...
function buildNewPageTemplate(pathValue: string): string {
  const title = (pathValue.split('/').pop() || 'Untitled')
    .replace(/\.md$/i, '')
    .replace(/-/g, ' ')
...
function buildWorkflowUrl(label: string, prompt: string): string {
  return `/chat?message=${encodeURIComponent(`${label}\n\n${prompt}`)}`
}
...
export function KnowledgeBrowserScreen() {
  ...
  const [createPath, setCreatePath] = useState('02_Notes/LLM/new-note.md')
  ...
  const healthQuery = useQuery({
    ...
    queryFn: () => readJson<KnowledgeHealthResponse>('/api/knowledge/health'),
  })
  ...
  const canEdit = source?.type === 'local'
  const health = healthQuery.data?.report ?? null
  ...
  const criticalIssueCount =
    (health?.totals.brokenLinks ?? 0) + (health?.totals.indexMissing ?? 0)
  const warningIssueCount =
    (health?.totals.orphanPages ?? 0) + (health?.totals.missingFrontmatter ?? 0)

  function openEditorForCurrentPage() {
    if (!page) return
  ...
  async function saveEditorContent() {
    ...
    setEditorError(null)
    try {
      await postJson('/api/knowledge/write', {
        path: editorPath,
        content: editorContent,
      })
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['knowledge'] }),
        queryClient.invalidateQueries({ queryKey: ['knowledge', 'read'] }),
      ])
  ...
  async function createNewPage() {
    ...
    setEditorPath(normalizedPath)
    setEditorContent(buildNewPageTemplate(normalizedPath))
    setEditorError(null)
    setCreateOpen(false)
    setEditorOpen(true)
  }
  ...
  async function deleteCurrentPage() {
    ...
    const confirmed = window.confirm(
      `Delete ${page.path}? This cannot be undone.`,
    )
    if (!confirmed) return
    await postJson('/api/knowledge/delete', { path: page.path })
      ...
      <div className="mx-auto flex w-full max-w-[1200px] min-h-0 flex-col px-4 py-6 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            >
              <HugeiconsIcon icon={BrainIcon} size={16} strokeWidth={1.7} />
              Maintain
              {criticalIssueCount + warningIssueCount > 0 ? (
            ...
            {canEdit ? (
              ...
              >
                <HugeiconsIcon icon={File01Icon} size={16} strokeWidth={1.7} />
                New page
              </button>
            ) : null}
        ...
        <div className="grid gap-3 px-3 py-3 md:grid-cols-4 md:px-4">
          <SummaryMetric
            label="Pages"
            value={String(health?.totals.pages ?? pages.length)}
            detail={`${health?.totals.formalPages ?? filteredPages.length} formal`}
          />
          <SummaryMetric
            label="Links"
            value={String(health?.totals.wikilinks ?? 0)}
            detail={`${health?.totals.brokenLinks ?? 0} broken`}
            tone={(health?.totals.brokenLinks ?? 0) > 0 ? 'bad' : 'good'}
          />
          <SummaryMetric
            label="Maintenance"
            value={String(criticalIssueCount + warningIssueCount)}
            detail="issues needing review"
            tone={
              criticalIssueCount > 0
                ? 'bad'
                : warningIssueCount > 0
          ...
          <SummaryMetric
            label="Source"
            value={source?.type === 'github' ? 'GitHub' : 'Local'}
            detail={source?.type === 'github' ? source.repo : knowledgeRoot}
          />
        </div>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  {canEdit ? (
                    <>
                      <button
                        type="button"
                        onClick={openEditorForCurrentPage}
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-md border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-600 transition-colors hover:bg-red-50 dark:border-red-900 dark:bg-neutral-900 dark:text-red-300 dark:hover:bg-red-950/30"
                      >
                        Delete
                      </button>
                    </>
        ...
        <DialogRoot open={maintenanceOpen} onOpenChange={setMaintenanceOpen}>
          <DialogContent className="w-[min(1040px,94vw)] max-w-none p-0">
            <div className="border-b border-primary-200 px-5 py-4 dark:border-neutral-800">
              <DialogTitle>llm-wiki maintenance console</DialogTitle>
              <DialogDescription>
                Health checks follow the skill rules: orient with schema, index
                and log; keep pages linked; update navigation after edits.
              </DialogDescription>
            </div>
            <div className="grid max-h-[78vh] gap-4 overflow-y-auto p-5 lg:grid-cols-[minmax(0,1fr)_320px]">
              <div className="space-y-4">
                <div className="grid gap-3 sm:grid-cols-3">
                  <SummaryMetric
                    label="Broken links"
                    value={String(health?.totals.brokenLinks ?? 0)}
                    detail="must fix first"
                    tone={
                      (health?.totals.brokenLinks ?? 0) > 0 ? 'bad' : 'good'
                    }
                  />
                  <SummaryMetric
                    label="Frontmatter"
                    value={String(health?.totals.missingFrontmatter ?? 0)}
                    detail="schema gaps"
                    tone={
                      (health?.totals.missingFrontmatter ?? 0) > 0
                        ? 'warn'
                        : 'good'
                    }
                  />
                  <SummaryMetric
                    label="Index gaps"
                    value={String(health?.totals.indexMissing ?? 0)}
                    detail="navigation debt"
                    tone={
                      (health?.totals.indexMissing ?? 0) > 0 ? 'bad' : 'good'
                    }
                  />
                </div>
                ...
                <IssueSection
                  ...
                  issues={health?.issues.brokenLinks ?? []}
                  onSelect={(pathValue) => {
                ...
                />
                <IssueSection
                  title="Schema and frontmatter gaps"
                  issues={health?.issues.missingFrontmatter ?? []}
                  onSelect={(pathValue) => {
                    setMaintenanceOpen(false)
                    handleSelectPath(pathValue)
                  }}
                ...
                <IssueSection
                  title="Index gaps"
                  issues={health?.issues.indexMissing ?? []}
                  onSelect={(pathValue) => {
                    setMaintenanceOpen(false)
                    handleSelectPath(pathValue)
                  }}
                />
                <IssueSection
                  title="Long or stale pages"
                  issues={[
                    ...(health?.issues.oversized ?? []),
                    ...(health?.issues.stale ?? []),
                  ]}
                  onSelect={(pathValue) => {
                    setMaintenanceOpen(false)
              ...
              <aside className="space-y-4">
                <section className="rounded-xl border border-primary-200 bg-primary-50/80 p-3 dark:border-neutral-800 dark:bg-neutral-900/60">
                  ...
                  <div className="mt-3 space-y-2 text-sm text-primary-700 dark:text-neutral-200">
                    <HealthFileRow
                      label="SCHEMA.md"
                      ok={health?.files.schema ?? false}
                    />
                    <HealthFileRow
                      label="index.md"
                      ok={health?.files.index ?? false}
                    />
                    <HealthFileRow
                      label="log.md"
                      ok={health?.files.log ?? false}
                    />
                  </div>
                </section>
                ...
                <section className="rounded-xl border border-primary-200 bg-primary-50/80 p-3 dark:border-neutral-800 dark:bg-neutral-900/60">
                  ...
                  <div className="mt-3 grid gap-2">
                    <WorkflowLink
                      label="Orient this wiki"
                      href={buildWorkflowUrl(
                        'Use llm-wiki',
                        `Run llm-wiki session orientation for ${knowledgeRoot}: read SCHEMA.md, index.md and recent log.md, then summarize the current structure.`,
                      )}
                    />
                    <WorkflowLink
                      label="Lint and repair plan"
                      href={buildWorkflowUrl(
                        'Use llm-wiki',
                        `Audit ${knowledgeRoot} with the llm-wiki lint checklist. Prioritize broken links, index gaps, frontmatter, orphan pages, stale pages and oversized pages.`,
                      )}
                    />
                    <WorkflowLink
                      label="Ask across the wiki"
                      href={buildWorkflowUrl(
                        'Use llm-wiki',
                        `Answer my question from the compiled knowledge in ${knowledgeRoot}. Search relevant pages first and cite wiki page names.`,
                      )}
                    />
                    <WorkflowLink
                      ...
                      href={buildWorkflowUrl(
                        'Use llm-wiki',
                        `Ingest a new source into ${knowledgeRoot}. Capture raw source when appropriate, update existing formal notes first, then update index.md and log.md.`,
                      )}
                    />
                  </div>
                </section>
                ...
                <section className="rounded-xl border border-primary-200 bg-primary-50/80 p-3 dark:border-neutral-800 dark:bg-neutral-900/60">
                  ...
                  <div className="mt-3 max-h-48 space-y-1 overflow-y-auto text-xs text-primary-600 dark:text-neutral-300">
                    {(health?.recentLog ?? []).map((line, index) => (
                      <div
                        key={`${line}:${index}`}
                        className="rounded-md bg-white/50 px-2 py-1 dark:bg-neutral-950/60"
                      >
                        {line}
                      </div>
                    ))}
        ...
        <DialogRoot open={createOpen} onOpenChange={setCreateOpen}>
          <DialogContent className="sm:max-w-lg">
            <div className="space-y-4">
              <div>
                <DialogTitle>Create llm-wiki page</DialogTitle>
                <DialogDescription>
                  Start from the vault schema template, then save it into the
                  local knowledge base.
                </DialogDescription>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium" htmlFor="new-page-path">
                  Markdown path
                </label>
                <input
                  id="new-page-path"
                  ...
                  className="w-full rounded-xl border border-primary-200 bg-primary-50 px-3 py-2 text-sm outline-none dark:border-neutral-800 dark:bg-neutral-950"
                />
              </div>
              <div className="flex justify-end gap-2">
                <button
                  ...
                  className="rounded-xl border border-primary-200 px-3 py-2 text-sm dark:border-neutral-700"
                >
                  Cancel
                </button>
                <button
        ...
        <DialogRoot open={editorOpen} onOpenChange={setEditorOpen}>
          <DialogContent className="w-[min(1040px,94vw)] max-w-none p-0">
            <div className="border-b border-primary-200 px-5 py-4 dark:border-neutral-800">
              <DialogTitle>{editorPath || 'Edit page'}</DialogTitle>
              <DialogDescription>
                Direct Markdown editor for local llm-wiki pages. Update index.md
                and log.md when the content becomes a formal note.
              </DialogDescription>
            </div>
            <div className="space-y-3 p-5">
              <input
                ...
                className="w-full rounded-xl border border-primary-200 bg-primary-50 px-3 py-2 font-mono text-xs outline-none dark:border-neutral-800 dark:bg-neutral-950"
              />
              <textarea
                value={editorContent}
                onChange={(event) => setEditorContent(event.target.value)}
                className="h-[52vh] w-full resize-none rounded-xl border border-primary-200 bg-primary-50 p-3 font-mono text-sm leading-6 outline-none dark:border-neutral-800 dark:bg-neutral-950"
                spellCheck={false}
              />
              {editorError ? (
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  className="rounded-xl bg-accent-500 px-4 py-2 text-sm font-medium text-white hover:bg-accent-600 disabled:opacity-60"
                >
    ...
    tone === 'good'
      ...
      : tone === 'warn'
        ...
        : tone === 'bad'
          ...
          : 'border-primary-200 bg-primary-50/80 dark:border-neutral-800 dark:bg-neutral-900/60'
  return (
    <div className={cn('min-w-0 rounded-xl border px-3 py-2.5', toneClass)}>
      <div className="text-[11px] font-semibold uppercase tracking-wide text-primary-500 dark:text-neutral-400">
        {label}
      </div>
      <div className="mt-1 truncate text-lg font-semibold text-primary-950 dark:text-neutral-50">
        {value}
      </div>
...
function IssueSection({
  ...
  title: string
  issues: Array<KnowledgeIssue>
  onSelect: (path: string) => void
}) {
  return (
    <section className="rounded-xl border border-primary-200 bg-primary-50/80 p-3 dark:border-neutral-800 dark:bg-neutral-900/60">
      <div className="flex items-center justify-between gap-3">
        <div className="text-sm font-semibold text-primary-950 dark:text-neutral-50">
          {title}
        </div>
        <span className="rounded-full border border-primary-200 px-2 py-0.5 text-xs text-primary-500 dark:border-neutral-700 dark:text-neutral-400">
          {issues.length}
        </span>
      </div>
      <div className="mt-3 space-y-2">
        {issues.length === 0 ? (
          <div className="rounded-lg border border-emerald-300/50 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300">
          ...
          issues.slice(0, 8).map((issue) => (
            <button
              ...
              className="block w-full rounded-lg border border-primary-200 bg-white/60 px-3 py-2 text-left transition-colors hover:border-primary-300 hover:bg-primary-100 dark:border-neutral-800 dark:bg-neutral-950/60 dark:hover:border-neutral-700 dark:hover:bg-neutral-900"
            >
              <div className="flex items-center justify-between gap-3">
                <span className="truncate text-sm font-medium text-primary-900 dark:text-neutral-100">
                  {issue.title}
                </span>
                <InlineBadge label={issue.severity} />
              </div>
        ...
        {issues.length > 8 ? (
          <div className="text-xs text-primary-500 dark:text-neutral-400">
            Showing 8 of {issues.length}. Search or open the page list for the
            rest.
          </div>
        ) : null}
      </div>
...
function HealthFileRow({ label, ok }: { label: string; ok: boolean }) {
  return (
    <div className="flex items-center justify-between rounded-lg border border-primary-200 bg-white/60 px-3 py-2 dark:border-neutral-800 dark:bg-neutral-950/60">
      <span>{label}</span>
      <span
        className={cn(
...
function WorkflowLink({ label, href }: { label: string; href: string }) {
  return (
    <a
      href={href}
      className="rounded-lg border border-primary-200 bg-white/60 px-3 py-2 text-sm font-medium text-primary-800 transition-colors hover:border-primary-300 hover:bg-primary-100 dark:border-neutral-800 dark:bg-neutral-950/60 dark:text-neutral-100 dark:hover:border-neutral-700 dark:hover:bg-neutral-900"
    >
      {label}
    </a>
  )
