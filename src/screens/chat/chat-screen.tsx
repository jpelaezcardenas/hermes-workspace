...
export function ChatScreen({
  ...
  // Don't show errors for new chats.
  ...
  const retryQueuedMessage = useCallback(
    function (message: ChatMessage, mode: 'manual' | 'auto') {
    ...
    (friendlyId: string, lastMessage: ChatMessage) => {
      ...
      queryClient.setQueryData(
        ...
        function upsert(existing: unknown) {
          const sessionList = Array.isArray(existing)
            ? (existing as Array<SessionMeta>)
            : []
          const now = Date.now()
          const existingIndex = sessionList.findIndex((session) => {
            return (
  ...
  useEffect(() => {
    function handleRunCommand(event: Event) {
      const detail = (event as CustomEvent<ChatRunCommandDetail | undefined>)
        .detail
  ...
  useEffect(() => {
    function handleSubmitSelection(event: Event) {
      const detail = (
        event as CustomEvent<ChatSubmitSelectionDetail | undefined>
      ).detail
