...
function ActiveModelCard() {
  ...
  const isLocal = LOCAL_PROVIDERS.some(
    (p) => p.id === (cfg?.provider ?? provider),
  )
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast(`Failed to save: ${e instanceof Error ? e.message : String(e)}`, {
        type: 'error',
      })
    } finally {
      setSaving(false)
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {[...CLOUD_PROVIDERS, ...LOCAL_PROVIDERS.map((p) => p.id)].map(
                (pid) => (
                  <button
                    key={pid}
                    type="button"
          ...
          {isLocal && (
            <div>
              ...
              <Input
                ...
                placeholder={
                  selectedLocal?.defaultUrl ?? 'http://localhost:11434/v1'
                }
          ...
          <div>
            ...
            <Input
              ...
              placeholder={
                isLocal ? 'qwen2.5:35b, llama3.2, etc.' : 'claude-sonnet-4-5'
              }
              className="font-mono text-sm"
            />
          </div>
