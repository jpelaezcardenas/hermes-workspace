...
import { kanbanPriorityColor, kanbanPriorityLabel } from '@/lib/hermes-kanban-types'
...
export function TaskCard({ task, assigneeLabels = {}, onClick, onDragStart, isDragging }: Props) {
  ...
  const isStale =
    ...
    Date.now() / 1000 - (task.last_heartbeat_at as number) > STALE_HEARTBEAT_MS / 1000

  const hasSpawnError = (task.spawn_failures ?? 0) > 0 || !!task.last_spawn_error
      ...
      <div className="flex items-center gap-1.5 flex-wrap mt-2">
        ...
        {hasSpawnError && (
          <span
            ...
            title={task.last_spawn_error ?? `${task.spawn_failures} spawn failure(s)`}
          >
            <HugeiconsIcon icon={Alert02Icon} size={10} />
            failed spawn
          </span>
