...
export function GuardrailsModal({ open, agentName, agentId, sessionKey, onOpenChange }: GuardrailsModalProps) {
  ...
  useEffect(() => {
    ...
    const parsed = normalizedId
      ? parseGuardrails(window.localStorage.getItem(`agentone:guardrails:${normalizedId}`))
      : DEFAULT_CONFIG
  ...
  function handleSave() {
    ...
    window.localStorage.setItem(`agentone:guardrails:${normalizedId}`, JSON.stringify(config))
