      ...
      POST: async ({ request }) => {
        ...
        const stream = new ReadableStream({
          async start(controller) {
            ...
            closeStream = () => {
              const wasStreamClosed = streamClosed
              streamClosed = true
              if (keepaliveTimer) {
          ...
          cancel() {
            ...
            // answer from session history. Mark this client stream closed and
            // stop browser-only timers, but keep the run id and upstream
            // abortController alive so the server can keep persisting progress.
            streamClosed = true
            if (keepaliveTimer) {
              clearInterval(keepaliveTimer)
              keepaliveTimer = null
