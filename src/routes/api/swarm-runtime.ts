...
function tmuxIsInstalled(): Promise<boolean> {
  // Honour AGENTONE_TMUX_BIN so a custom-path install isn't reported as
  // 'tmux not installed' just because PATH doesn't include it. See #244.
  const bin =
    process.env.AGENTONE_TMUX_BIN || process.env.HERMES_TMUX_BIN || process.env.CLAUDE_TMUX_BIN || 'tmux'
  return new Promise((resolve) => {
