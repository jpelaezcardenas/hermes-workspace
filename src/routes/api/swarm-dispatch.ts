...
function buildWorkerPrompt(input: {
  ...
  lines.push(
    ...
    'The FINAL message of your dispatched response MUST be exactly the 6-line checkpoint block below — and nothing else.',
    'No preamble, no trailing prose, no markdown fences, no narration of intent. The last assistant turn is the checkpoint, period.',
    'While you are still working you keep calling tools; reasoning/intent narration ("I\'ll start by...", "I\'m reviewing...", "Let me check...") is NEVER a substitute for the checkpoint and must NOT appear in the final assistant message.',
    'When you stop talking, your last reply must be these six labels, in this order, each on its own line:',
    '',
    'STATE: <DONE | BLOCKED | NEEDS_INPUT | HANDOFF | IN_PROGRESS>',
    'FILES_CHANGED: <exact repo-relative paths, comma-separated, or the literal word none>',
    'COMMANDS_RUN: <exact commands you executed, semicolon-separated, or the literal word none>',
    'RESULT: <concrete result or proof — what is now true that was not before>',
    'BLOCKER: <exact blocker, or the literal word none>',
    'NEXT_ACTION: <exact recommended next step>',
    '',
    'IN_PROGRESS with a concrete RESULT is a perfectly valid checkpoint when you are partway through; emit it instead of a free-form status update. All six labels are mandatory every time — partial blocks are rejected by the parser and you will be marked blocked.',
    '',
    'Example of an accepted final message (and the entire final message — nothing before or after):',
    'STATE: IN_PROGRESS',
    'FILES_CHANGED: docs/wiki/index.md',
    'COMMANDS_RUN: rg "swarm" docs/wiki',
    'RESULT: Surveyed existing wiki; identified 3 sections needing the new swarm topology page',
    'BLOCKER: none',
    'NEXT_ACTION: Draft docs/wiki/swarm-topology.md and link it from index.md',
    '',
    'Common failure to avoid: ending your dispatched response with reasoning prose like "I\'ll start by reviewing..." / "I\'m checking the wiki..." / "Let me look at...". That text, if it appears at all, belongs in tool-call turns BEFORE the final assistant message — never IN it. The final message is always and only the 6-line block above.',
  )
