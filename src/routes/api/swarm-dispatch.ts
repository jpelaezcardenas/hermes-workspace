...
export function buildWorkerPrompt(input: {
  ...
  lines.push(
    ...
    '',
    '## Operating Rules',
    '- Work in your persistent Agent-e1 worker session and preserve your profile context.',
...
async function ensureLiveTmuxSession(workerId: string): Promise<{ ok: true; tmuxBin: string; sessionName: string } | { ok: false; error: string }> {
  ...
  // Give the agent a moment to render its prompt before sending keys. If Agent-e1
  ...
  await sleep(1200)
  if (!(await tmuxHasSession(tmuxBin, sessionName))) {
    return { ok: false, error: `Agent-e1 worker tmux session ${sessionName} exited during startup` }
  }

  const startupOutput = await captureTmuxPane(tmuxBin, sessionName)
  if (startupOutput.includes('[Agent-e1 worker exited with status')) {
...
`, { flag: 'a' })
    return {
      ok: false,
      error: `Agent-e1 worker failed to start in tmux session ${sessionName}. Startup output saved to ${startupLogPath}: ${sanitizedOutput}`,
    }
  }
...
async function sendPromptToLiveSession(workerId: string, prompt: string): Promise<WorkerResult | null> {
  ...
  // Give the TUI enough time to ingest the paste before submitting. The Agent-e1
