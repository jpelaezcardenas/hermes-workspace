/**
 * Assignees endpoint for the hermes-tasks backend.
...
function getProfileNames(): string[] {
  try {
    return fs.readdirSync(PROFILES_PATH).filter(name => {
      try {
        return (
          fs.statSync(path.join(PROFILES_PATH, name)).isDirectory() &&
          fs.existsSync(path.join(PROFILES_PATH, name, 'config.yaml'))
        )
      } catch {
        return false
      }
