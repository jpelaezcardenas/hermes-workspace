...
const TAP_DEBUG_STORAGE_KEY = 'agentone:tap-debug'
const TAP_DEBUG_EVENT = 'agentone:tap-debug-change'
...
export function useTapDebug(
  ...
  useEffect(() => {
    ...
    const applyEnabled = (next: boolean) => {
      enabled = next
      debugWindow.__AGENTONE_TAP_DEBUG__ = next
      try {
