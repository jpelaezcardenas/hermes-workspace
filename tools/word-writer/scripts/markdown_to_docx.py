...
Usage:
    python3 scripts/markdown_to_docx.py input.md [output.docx] [--mode simple|full] [--toc|--no-toc] [--cover-json JSON]

Notes:
    --toc and --no-toc now control whether a TOC is inserted. Previously the
    script always injected a TOC; Word Writer relies on this flag for memos,
    meeting notes, and other short documents.
"""
...
from office_output_validation import OfficeValidationError, validate_docx_against_template, _sha256 as _oov_sha256
...
def _render_mermaid_to_image(code: str, output_dir: Path, index: int) -> Optional[Path]:
...
TEMPLATE_PATH = Path(__file__).resolve().parents[1] / "pandoc-templates" / "word.docx"
    ...
    for para in doc.paragraphs:
        if para.style and para.style.name:
            ...
            if sname.startswith('Heading '):
                ...
                hlevel = int(sname[1])
            if hlevel and 1 <= hlevel <= 3 and para.text.strip():
...
def _remove_existing_toc(doc):
    """Remove template/generated TOC blocks when TOC is explicitly disabled."""
    body = doc.element.body
    elements = list(body)
    to_remove = []

    def _is_toc_title_paragraph(elem):
    ...
    for elem in elements:
        ...
        if sdtPr is not None:
            ...
            if dpo is not None:
                ...
                if gallery is not None and 'Table of Contents' in gallery.get(qn('w:val'), ''):
                    is_toc = True
        if not is_toc and sdtContent is not None:
            for instr in sdtContent.findall('.//' + qn('w:instrText')):
                if instr.text and 'TOC' in instr.text:
                    is_toc = True
                    break
        if is_toc:
            ...
            if idx > 0 and _is_toc_title_paragraph(elements[idx - 1]):
                to_remove.append(elements[idx - 1])
    ...
    if to_remove:
        print(f"  TOC disabled: removed {len(to_remove)} TOC element(s)")
...
def convert(input_path: str, output_path: Optional[str] = None, cover_fields: Optional[dict] = None, document_mode: str = "simple", include_toc: bool = True) -> Path:
    src = Path(input_path)
    if not src.exists():
    ...
    # Fail closed: office deliverables must come from the canonical Word template.
    if not TEMPLATE_PATH.exists():
        raise OfficeValidationError(f"Canonical Word template not found: {TEMPLATE_PATH}")

    doc = Document(str(TEMPLATE_PATH))
    body = doc.element.body

    elements = list(body)
    if document_mode == "simple":
        # Keep only section properties, page setup, styles, headers, and footers.
        # Strip proposal-specific cover, TOC shell, and sample body content.
        to_remove = []
        for elem in elements:
            tag = elem.tag.split('}')[-1] if '}' in elem.tag else elem.tag
            if tag == 'sectPr':
        ...
        print(f"  Word Writer simple mode: stripped cover/TOC/sample content ({len(to_remove)} elements removed)")
    else:
        # Full mode preserves the formal title-page shell used by the shared Word template.
        page_break_count = 0
        keep_until = -1
        for idx, elem in enumerate(elements):
            tag = elem.tag.split('}')[-1] if '}' in elem.tag else elem.tag
        ...
        print(f"  Using template: {TEMPLATE_PATH.name} ({len(doc.styles)} styles)")
        print(f"  Full mode: cover page preserved ({keep_until + 1} elements), {len(to_remove)} sample elements removed")
    ...
    # Update cover page placeholder fields only in full-template mode
    if cover_fields and document_mode == "full":
    ...
    while i < len(lines):
        ...
        if img_match:
            ...
            # Resolve relative to source markdown file
            img_abs = (src.parent / img_rel_path).resolve()
            # Fallback: try one directory higher (handles ../../shared/ when shared/
            # is a sibling of the parent project dir, not inside it)
            if not img_abs.exists() and not img_abs.with_suffix('.jpg').exists():
                alt_rel = img_rel_path.replace("../../", "../../../", 1)
                alt_abs = (src.parent / alt_rel).resolve()
                if alt_abs.exists() or alt_abs.with_suffix('.jpg').exists():
                    img_abs = alt_abs
            # Try JPEG version first (optimized), fall back to original
            img_jpg = img_abs.with_suffix('.jpg')
            if img_jpg.exists():
                img_abs = img_jpg
            if img_abs.exists():
                doc.add_picture(str(img_abs), width=Inches(6.0))
                last_paragraph = doc.paragraphs[-1]
                last_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        if heading_match:
            ...
            # In simple mode, the very first H1 is the document title, not a semantic
            # Heading 1. Render it with Word's Title style so it looks like a title
            # without consuming the first outline heading slot.
            if level == 1 and not leading_title_consumed and document_mode != "full" and not first_content_emitted:
                try:
                    title_para = doc.add_paragraph(style="Title")
                except KeyError:
                    title_para = doc.add_paragraph()
                title_para.clear()
                title_para.add_run(heading_text)
                leading_title_consumed = True
                first_content_emitted = True
                i += 1
                continue

            # In full mode, the cover page already carries the document title, so any
            # leading H1 before body content should be skipped.
            if level == 1 and not leading_title_consumed and document_mode == "full" and not first_content_emitted:
                leading_title_consumed = True
                i += 1
                continue

            # Promote heading levels by 1 when the title has been consumed,
            # since body sections use H2 in markdown but should render as
            # outline level 1 ("1.", not "1.1") in Word.
            # H2→H1 (ilvl 0), H3→H2 (ilvl 1), H4→H3 (ilvl 2).
            # H1 body headings (after title) stay at H1/ilvl 0.
            effective_level = max(1, level - 1) if leading_title_consumed and level >= 2 else level
            spec = HEADING_SPEC.get(effective_level, HEADING_SPEC[4])

            # Apply the template heading style, but keep numbering in the heading
            # text itself. The source markdown already carries the contractual
            # section numbers (for example "1. Definitions" and "3.1 License Grant").
            # Letting Word add separate outline numbering causes the body and TOC to
            # diverge, because the TOC entries then drop the visible section numbers.
            ...
            heading.clear()
            heading.add_run(heading_text.strip())
        ...
        if "|" in line and line.strip().startswith("|"):
            ...
            if table_lines:
                ...
                for row_idx, cells in enumerate(table_lines):
                    ...
                    # Set row heights
                    if row_idx == 0:
                        row.height = Cm(0.86)  # header row (~15% increase for readability)
                        row.height_rule = 1   # WD_ROW_HEIGHT_RULE.AT_LEAST
                    else:
                        row.height = Cm(0.86)  # data row (~15% increase for readability)
                        row.height_rule = 1   # WD_ROW_HEIGHT_RULE.AT_LEAST
    ...
    # Find the insertion point for TOC.
    # - Full mode: after the cover shell (second page break)
    # - Simple mode / no cover shell: before the first body section heading so the
    #   title + subtitle stay above the TOC and content starts after it.
    elements = list(body)
    pb_count = 0
    ...
    if toc_insert_before is None:
        for elem in elements:
            ...
            if pStyle is None:
                continue
            style_val = pStyle.get(qn('w:val'), '')
            if style_val in {'H2', 'H3', 'H4', 'Heading2', 'Heading3', 'Heading4'}:
                toc_insert_before = elem
                break

    # Update or remove TOC based on caller choice
    if include_toc:
    ...
    doc.save(str(dst))
    # word/numbering.xml legitimately changes when the converter creates new
    # numId instances for bullet/numbered lists (_new_num_id), so we must
    # whitelist the resulting hash.  All other signature parts (styles,
    # settings, theme) remain unchanged from the locked template.
    with zipfile.ZipFile(dst) as _zf:
        _num_hash = _oov_sha256(_zf.read("word/numbering.xml"))
    validate_docx_against_template(
        dst,
        TEMPLATE_PATH,
        allowed_signature_overrides={"word/numbering.xml": {_num_hash}},
    )
    print(f"✓ Converted {src.name} → {dst.name} (template signature verified)")
    return dst
...
if __name__ == "__main__":
    ...
    if "--mode" in args:
        idx = args.index("--mode")
        document_mode = args[idx + 1].strip().lower()
        args = args[:idx] + args[idx + 2:]
    if "--toc" in args:
        args.remove("--toc")
        include_toc = True
