...
| Information request theme | Tokenized Lending |
| Reference | GOTO-FINANCIAL-RFI-202603 |
| Version | v1.0 |
...
GoTo Financial is conducting a structured market-sounding exercise in relation to **tokenized lending**. This Request for Information is issued for information-gathering purposes only. It does **not** constitute a commitment to procure, a binding tender, or an obligation to issue a subsequent RFP. That said, the institution is not looking for airy thought leadership either. It wants grounded information that can support internal option analysis, control design, sequencing decisions, and eventual investment cases.

The institution is seeking to understand current market maturity, viable operating models, regulatory implications, integration patterns, and delivery risks relevant to the target use case in Indonesia. Publicly visible market material reviewed during pre-RFI shaping suggests that the environment around GoTo Financial is being shaped by embedded finance at ecosystem scale, wallet and lending integration, Indonesian data and risk controls. Directional research considered includes the following:
...
- functional scope for tokenized lending;
...
| --- | --- | --- |
| IR-01 | End-to-end lifecycle support for tokenized lending, including initiation, approval, issuance/origination, servicing, reporting, exception handling, and closure. | Narrative |
...
| IR-20 | Approach for a future-state roadmap aligned to GoTo Financial strategic direction, while clearly distinguishing live capability from roadmap-only functionality. | Narrative |
...
|---|---|---|
| Strategic relevance to target use case | 20% | Degree to which the response addresses tokenized lending specifically in the context of GoTo Financial and Indonesia. |
...
The institution expects respondents to show awareness of market developments such as **embedded finance and lending digitisation**, but it does not require respondents to have participated directly in those programmes. What matters is whether the respondent can connect market learning to a credible control and operating model for tokenized lending in Indonesia.
...
The institution requests more detailed information on how respondents design and support the business-as-usual operating model for tokenized lending. Responses should describe first-line operations, exception management, user and participant support, control monitoring, reconciliation sign-off, reporting production, and escalation into compliance, risk, legal, or technology teams. GoTo Financial wants to understand whether respondents have a repeatable institutional model or whether every deployment still depends on a handful of specialist engineers hovering in the background.
...
The institution expects respondents to show awareness of how Indonesia market structure affects implementation choices for tokenized lending. Please explain how local regulatory treatment, domestic infrastructure, data-location expectations, outsourcing scrutiny, and the practical role of internal control functions influence your recommended architecture and delivery model.

The response should also connect, where relevant, to market initiatives such as embedded finance and lending digitisation. The point is not whether your firm participated directly in every initiative. The point is whether you understand what those initiatives have changed in buyer expectations around interoperability, programmability, governance, and operational transition.
