...
| Field | Value |
| --- | --- |
| Reference | `ADYEN-RFI-STABLECOIN-SETTLEMENT-202603` |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Adyen (Netherlands) |
| Information request subject | Stablecoin Settlement |
...
| Contact point | adyen-market-scan@issuer.example |
| Status | Non-binding market inquiry |

## 1. Procurement Context

Adyen is conducting this RFI to understand the current vendor landscape for **stablecoin settlement**. This is a market and capability scan, not an award-ready procurement. The purpose is to compare what vendors can do today, what can be configured quickly, what depends on partners, and what is still roadmap.
...
| Extension path | Whether the same foundation can support stablecoins, tokenized merchant balances, settlement orchestration, ledger and payout APIs over time |
...
This RFI covers functional capability, technical architecture, operating model, control framework, and indicative commercials for stablecoin settlement. The buyer is interested in both minimum viable launch support and extension potential. Vendors should answer based on current product reality, not aspiration.
...
| --- | --- | --- |
| CQ-01 | Describe your current support for stablecoin settlement. | Status label + 150-word summary |
...
Adyen expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Adyen approaches digital assets through the lens of payments and merchant settlement efficiency rather than speculative trading. Public market discussion has increasingly focused on stablecoins, digital cash, programmable settlement and merchant treasury optimization. A realistic RFP therefore centers on payout, treasury, reconciliation and merchant controls. In practical terms, that means the proposed platform must support stablecoin funding, treasury settlement, mint/burn controls, reserve reporting, sanctions controls and integration into payment orchestration while fitting into merchant acquiring stack, payout and treasury infrastructure, risk and fraud systems, ledgering, merchant reporting, APIs, IAM and observability platforms. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. evaluation would involve payments product, treasury, engineering, compliance, risk and enterprise security leaders. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, Adyen will care intensely about API reliability, merchant-operational simplicity and low-friction integration into its existing payments core. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by DNB, MiCA where crypto-asset services are implicated, DORA, PSD2/EMD-style payments controls, GDPR. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
For this procurement, bidders should assume that the first live use cases will be concrete rather than experimental. At minimum, the target operating model should support stablecoin funding, treasury settlement, mint/burn controls, reserve reporting, sanctions controls and integration into payment orchestration. The proposed platform should show how configuration, approvals, ledger updates, settlement instructions, exception handling and reporting are orchestrated across those use cases without requiring brittle one-off customization for each product or jurisdiction.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes merchant acquiring stack, payout and treasury infrastructure, risk and fraud systems, ledgering, merchant reporting, APIs, IAM and observability platforms. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
...
Because this is an RFI rather than a committed award process, Adyen is using this document to separate productized platforms from ambitious slide decks. Respondents should therefore describe the maturity of their solution for stablecoin funding, treasury settlement, mint/burn controls, reserve reporting, sanctions controls and integration into payment orchestration in very concrete terms: what is live today, what is in pilot, what is customer-specific, and what remains roadmap. The buyer is especially interested in evidence that a platform can survive production governance, exception volume, regulator scrutiny and multi-team operating ownership.
...
Respondents should conclude with a short statement of scope confidence, known boundaries, key dependencies and the minimum conditions required for a successful production deployment. The buyer is explicitly testing whether the vendor understands the difference between an interesting pilot and a controllable live service.
