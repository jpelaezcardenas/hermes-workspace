...
| Document Reference | STRIPE-RFP-202603-01 |
| Document Type | RFP |
...
| Workstream 2 | Configuration of the platform for tokenized settlement between merchants, platforms, and treasury accounts, including product templates, token or asset models, workflow approvals, entitlement structures, reporting schemas, data retention rules and control checkpoints. |
...
| FR-10 | Support phased rollout by entity, geography, participant segment, product or workflow while preserving shared control patterns for tokenized settlement between merchants, platforms, and treasury accounts. | Mandatory | Narrative + evidence | Rollout plan / tenancy model |
...
| FR-13 | Support integration patterns specific to the buyer environment, including payments orchestration, Connect/Treasury style APIs, stablecoin and Bridge rails, fraud monitoring, reconciliation and developer-facing platform tooling. | Mandatory | Narrative + evidence | Target-state integration blueprint |
...
|---|---|---|
| Functional fit and workflow coverage | 22% | Alignment to the target operating model for tokenized settlement between merchants, platforms, and treasury accounts and realism of end-to-end lifecycle coverage |
...
For Stripe, respondents should assume the institution will require a formal governance pack covering scope, controls, stakeholders, escalation paths, evidentiary outputs, operational ownership, release checkpoints, model/rules governance and board- or committee-level reporting before any production release. That pack must be capable of supporting audit and management review over time.

### 8.7 Integration inventory template
| Area | Illustrative scope |
|---|---|
| Internal systems | payments orchestration, Connect/Treasury style APIs, stablecoin and Bridge rails, fraud monitoring, reconciliation and developer-facing platform tooling. |
...
The bidder shall also address operational compliance design for tokenized settlement between merchants, platforms, and treasury accounts. This includes security master governance, entitlement calculations, collateral substitution logic, coupon or yield-event handling and cash-versus-asset reconciliation. Where the regulatory perimeter depends on business structure, the bidder shall present configurable options together with the residual responsibilities that remain with Stripe. Responses must identify what evidence is generated natively, what is derived via integration, how timestamps are normalised across components, and how legal-entity, desk or participant segmentation is preserved in both the live operating model and archived records.
...
The following scenarios are intended to test whether the proposed platform can keep Stripe's control environment intact while supporting tokenized settlement between merchants, platforms, and treasury accounts. Respondents shall describe native product behaviour, configuration points, integration dependencies, operational ownership, evidence outputs and fallback procedures for each scenario.
...
For avoidance of doubt, Stripe will treat unanswered delivery-risk questions as a material weakness even where the product appears functionally strong. The bidder should therefore identify non-standard assumptions, unresolved third-party dependencies, minimum-volume thresholds, jurisdictional constraints, roadmap items, implementation accelerators that require buyer resources, and any feature areas where manual controls remain necessary for tokenized settlement between merchants, platforms, and treasury accounts. This clarification section should also state what the buyer must provide to keep timeline, budget and control outcomes realistic.
