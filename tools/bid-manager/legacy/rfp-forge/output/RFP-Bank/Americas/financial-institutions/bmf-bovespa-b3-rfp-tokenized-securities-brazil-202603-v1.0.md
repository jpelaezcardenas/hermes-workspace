# BM&F Bovespa/B3 — Request for Proposal
...
| Document Reference | BMF-BOVESPA--RFP-202603-07 |
| Document Type | RFP |
...
| Workstream 2 | Configuration of the platform for tokenized securities issuance and market infrastructure in Brazil, including product templates, token or asset models, workflow approvals, entitlement structures, reporting schemas, data retention rules and control checkpoints. |
...
| FR-10 | Support phased rollout by entity, geography, participant segment, product or workflow while preserving shared control patterns for tokenized securities issuance and market infrastructure in Brazil. | Mandatory | Narrative + evidence | Rollout plan / tenancy model |
...
| FR-13 | Support integration patterns specific to the buyer environment, including exchange matching, clearing, central securities depository functions, B3 Digitas style tokenization services, participant gateways and Brazilian market surveillance. | Mandatory | Narrative + evidence | Target-state integration blueprint |
...
|---|---|---|
| Functional fit and workflow coverage | 22% | Alignment to the target operating model for tokenized securities issuance and market infrastructure in Brazil and realism of end-to-end lifecycle coverage |
...
For BM&F Bovespa/B3, respondents should assume the institution will require a formal governance pack covering scope, controls, stakeholders, escalation paths, evidentiary outputs, operational ownership, release checkpoints, model/rules governance and board- or committee-level reporting before any production release. That pack must be capable of supporting audit and management review over time.

### 8.7 Integration inventory template
| Area | Illustrative scope |
|---|---|
| Internal systems | exchange matching, clearing, central securities depository functions, B3 Digitas style tokenization services, participant gateways and Brazilian market surveillance. |
...
The bidder shall also address operational compliance design for tokenized securities issuance and market infrastructure in Brazil. This includes product configuration governance, approval workflows, reference-data quality, reconciliation evidence and controlled exception handling. Where the regulatory perimeter depends on business structure, the bidder shall present configurable options together with the residual responsibilities that remain with BM&F Bovespa/B3. Responses must identify what evidence is generated natively, what is derived via integration, how timestamps are normalised across components, and how legal-entity, desk or participant segmentation is preserved in both the live operating model and archived records.
...
The following scenarios are intended to test whether the proposed platform can keep BM&F Bovespa/B3's control environment intact while supporting tokenized securities issuance and market infrastructure in Brazil. Respondents shall describe native product behaviour, configuration points, integration dependencies, operational ownership, evidence outputs and fallback procedures for each scenario.
