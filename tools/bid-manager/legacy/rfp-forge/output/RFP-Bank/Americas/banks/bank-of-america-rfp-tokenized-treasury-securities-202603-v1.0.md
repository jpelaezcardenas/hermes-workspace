# Bank of America — Request for Proposal
...
| Document Reference | BANK-OF-AMER-RFP-202603-04 |
| Document Type | RFP |
...
| Workstream 2 | Configuration of the platform for tokenized treasury securities issuance, servicing, and collateral operations, including product templates, token or asset models, workflow approvals, entitlement structures, reporting schemas, data retention rules and control checkpoints. |
...
| FR-10 | Support phased rollout by entity, geography, participant segment, product or workflow while preserving shared control patterns for tokenized treasury securities issuance, servicing, and collateral operations. | Mandatory | Narrative + evidence | Rollout plan / tenancy model |
...
| FR-13 | Support integration patterns specific to the buyer environment, including CashPro and treasury services rails, core payments infrastructure, sanctions and AML controls, treasury booking systems, information security controls and enterprise observability. | Mandatory | Narrative + evidence | Target-state integration blueprint |
...
|---|---|---|
| Functional fit and workflow coverage | 22% | Alignment to the target operating model for tokenized treasury securities issuance, servicing, and collateral operations and realism of end-to-end lifecycle coverage |
...
For Bank of America, respondents should assume the institution will require a formal governance pack covering scope, controls, stakeholders, escalation paths, evidentiary outputs, operational ownership, release checkpoints, model/rules governance and board- or committee-level reporting before any production release. That pack must be capable of supporting audit and management review over time.

### 8.7 Integration inventory template
| Area | Illustrative scope |
|---|---|
| Internal systems | CashPro and treasury services rails, core payments infrastructure, sanctions and AML controls, treasury booking systems, information security controls and enterprise observability. |
...
The bidder shall also address operational compliance design for tokenized treasury securities issuance, servicing, and collateral operations. This includes security master governance, entitlement calculations, collateral substitution logic, coupon or yield-event handling and cash-versus-asset reconciliation. Where the regulatory perimeter depends on business structure, the bidder shall present configurable options together with the residual responsibilities that remain with Bank of America. Responses must identify what evidence is generated natively, what is derived via integration, how timestamps are normalised across components, and how legal-entity, desk or participant segmentation is preserved in both the live operating model and archived records.
...
The following scenarios are intended to test whether the proposed platform can keep Bank of America's control environment intact while supporting tokenized treasury securities issuance, servicing, and collateral operations. Respondents shall describe native product behaviour, configuration points, integration dependencies, operational ownership, evidence outputs and fallback procedures for each scenario.
...
For avoidance of doubt, Bank of America will treat unanswered delivery-risk questions as a material weakness even where the product appears functionally strong. The bidder should therefore identify non-standard assumptions, unresolved third-party dependencies, minimum-volume thresholds, jurisdictional constraints, roadmap items, implementation accelerators that require buyer resources, and any feature areas where manual controls remain necessary for tokenized treasury securities issuance, servicing, and collateral operations. This clarification section should also state what the buyer must provide to keep timeline, budget and control outcomes realistic.
