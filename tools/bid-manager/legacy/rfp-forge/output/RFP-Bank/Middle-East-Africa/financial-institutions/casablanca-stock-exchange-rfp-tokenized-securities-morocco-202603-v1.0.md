...
| Field | Value |
| --- | --- |
| Reference | CASABLANCA-STOCK-EXCHANGE-RFP-TOKENIZED-SECURITIES-MOROCCO-202603 |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Casablanca Stock Exchange (Morocco) |
| Document type | RFP |
| Subject | Tokenized Securities Issuance And Trading Framework |
...
Casablanca Stock Exchange is treating tokenized securities issuance and trading framework as a
...
securities issuance and trading framework in live operation, not just in a demonstration setting.
...
The second objective is to understand the operating model implied by each response. Casablanca Stock
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for tokenized securities issuance and trading framework. |
...
wants to understand how the work actually lands inside Casablanca Stock Exchange's organisation.
...
issuance and trading framework; the buyer must be able to identify who initiated a change or
...
securities issuance and trading framework programme.
...
For Casablanca Stock Exchange, this appendix should be written in a way that supports architecture
...
For Casablanca Stock Exchange, operational governance for Tokenized Securities Issuance And Trading
Framework must define named control owners across product management, first-line operations,
...
incident review, and management reporting. Casablanca Stock Exchange is especially interested in
...
A strong response should explain how data relating to Tokenized Securities Issuance And Trading
Framework moves between channels, orchestration layers, policy engines, ledgers, books-and-records,
reporting stacks, and external participants. Casablanca Stock Exchange expects the bidder to
...
Integration narratives should also address release management. Casablanca Stock Exchange wants to
...
Given the sensitivity of Tokenized Securities Issuance And Trading Framework, the buyer expects
...
Resilience discussion should go beyond backup existence. Casablanca Stock Exchange wants evidence of
...
configuration, interface build, testing, training, and cutover fit together. Casablanca Stock
Exchange does not expect a rigid waterfall plan, but it does expect formal acceptance criteria,
issue governance, milestone ownership, and explicit dependency management. If the bidder proposes
agile delivery, the governance model still needs to explain when decisions are frozen and what
constitutes readiness to progress.
...
to Tokenized Securities Issuance And Trading Framework, the more disciplined post-go-live change
control must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that Casablanca
Stock Exchange can build internal capability rather than remain dependent on a small external
delivery team.
...
third-party services grow. Casablanca Stock Exchange wants to know whether cost scales linearly,
...
core aspects of Tokenized Securities Issuance And Trading Framework depend on a single custody,
...
Casablanca Stock Exchange (Morocco) is not approaching **tokenized securities issuance and trading framework** as a generic blockchain exercise. Casablanca Stock Exchange is part of Morocco's broader push to deepen local capital markets and improve regional African relevance. Tokenized securities would most credibly begin as controlled digitisation of issuance and post-trade evidence with close AMMC and market-infrastructure oversight. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Likely integration points include depository and registrar functions, listing systems, surveillance, issuer reporting, and investor/KYC support processes. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Exchange management, market development, technology, legal, and regulator-liaison teams are likely sponsors. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. No direct public RFP was identified, but the market-modernisation posture makes a scoped evaluation credible. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For Casablanca Stock Exchange (Morocco), the most relevant workload pattern is **tokenized listing workflow, issuer evidence, investor registry linkage, and post-trade supervision**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how Casablanca Stock Exchange (Morocco) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, Casablanca Stock Exchange (Morocco) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Policy and market-infrastructure fit | 22% | Scored against tokenized securities issuance and trading framework in the specific context of Casablanca Stock Exchange (Morocco). |
| Technical architecture and interoperability | 18% | Scored against tokenized securities issuance and trading framework in the specific context of Casablanca Stock Exchange (Morocco). |
| Regulatory evidence and supervisory control support | 22% | Scored against tokenized securities issuance and trading framework in the specific context of Casablanca Stock Exchange (Morocco). |
| Security, resilience, and data-governance posture | 15% | Scored against tokenized securities issuance and trading framework in the specific context of Casablanca Stock Exchange (Morocco). |
| Delivery and implementation realism | 10% | Scored against tokenized securities issuance and trading framework in the specific context of Casablanca Stock Exchange (Morocco). |
| Commercial transparency | 6% | Scored against tokenized securities issuance and trading framework in the specific context of Casablanca Stock Exchange (Morocco). |
| Supplier capability and ecosystem alignment | 7% | Scored against tokenized securities issuance and trading framework in the specific context of Casablanca Stock Exchange (Morocco). |
...
A realistic proposal for Casablanca Stock Exchange (Morocco) should therefore provide a phased deployment hypothesis. Phase one should describe the minimum safe production scope, the volumes and participant types supported at launch, required client-side roles, major control gates, and objective acceptance criteria. Phase two should describe how the scope expands once controls, throughput, and reporting have been proven. If the bidder believes the requested use case should be sequenced differently, that is acceptable, but the answer must still show a credible route from contained launch to scaled operations.
...
Control assurance is an explicit selection criterion for Casablanca Stock Exchange (Morocco). The selected supplier will be expected to support walkthroughs with architecture, security, legal, compliance, operations, risk, and procurement stakeholders. Responses should therefore describe the artefacts available before contract signature and the deeper evidence available during implementation. Useful artefacts include configuration snapshots, role matrices, sample audit logs, example reconciliation outputs, incident-process documentation, environment diagrams, API specifications, backup/restore evidence, and sample operational reports. If these artefacts exist only in vendor-internal formats and cannot be shared with clients, that limitation should be declared clearly.

The rollout plan should distinguish between pilot value and production readiness. A pilot may validate that tokenized listing workflow, issuer evidence, investor registry linkage, and post-trade supervision is technically feasible, but the buyer is making a sourcing decision for a programme that will need governance, support, and repeatability. Bidders should therefore outline the minimum controls needed before first live processing, the metrics that would indicate the platform is ready for broader rollout, and the dependencies that must be closed before additional products, entities, markets, or participant groups are added. Where later phases depend on external partner readiness, regulatory comfort, or contract expansion, that dependency should be made visible in the plan rather than left implicit.

Finally, respondents should explain how the programme remains governable after go-live. The buyer expects structured release management, formal change approval, regression testing, periodic entitlement review, exception governance, and management reporting that tracks the real health of the service rather than vanity metrics. Examples of meaningful indicators include exception ageing, reconciliation break volumes, failed-settlement rates, approval turnaround, security incident trends, backlog of unresolved defects, accuracy of regulatory reporting, and frequency of manual overrides. The suppliers that score best will be the ones that show how tokenized securities issuance and trading framework becomes an owned institutional capability rather than a clever but fragile technology deployment.


### 8.18 Institution-specific market research and operating implications

Public market context reinforces that Casablanca Stock Exchange (Morocco) should be treated as a specific institutional
buyer, not as a placeholder for the region. It is a Morocco's primary exchange operator focused on capital-market development, issuer access, investor confidence, and coordination with domestic custody and regulatory institutions. Casablanca's digital-capital-markets agenda is shaped less by crypto-native trading and more by measured infrastructure modernization, greater capital-market depth, and interoperability with domestic supervision, custody, and payment rails. Morocco's cautious stance on crypto activity makes institutional tokenization questions especially sensitive and likely to require clear legal perimeter definition.

Responses must account for Moroccan capital-markets supervision, foreign-exchange control considerations, AML/CFT requirements, central-securities-depository interfaces, and any need to separate exploratory tokenization from prohibited unlicensed crypto activity.

For the target use case of **tokenized securities platform**, that public context changes the procurement lens.
...
contained innovation-lab assumption. The buyer will therefore look for structures that can support controlled pilots, closed user groups, or regulator-observed sandbox phases before broader market exposure, with especially strong evidence on investor eligibility and settlement finality.
...
- Support phased deployment beginning with private-market or qualified-investor instruments before any broader marketplace rollout.
- Enable bilingual operating artefacts where needed and preserve strong audit evidence for domestic regulators, CSD participants, and issuer governance committees.
- Provide explicit mechanisms to segregate tokenized-security workflows from any functionality that could be misconstrued as open crypto-asset dealing.

These requirements are additive to the baseline sections of this document. They are included because
Casablanca Stock Exchange (Morocco) operates with business, regulatory, and operational constraints that differ from a
