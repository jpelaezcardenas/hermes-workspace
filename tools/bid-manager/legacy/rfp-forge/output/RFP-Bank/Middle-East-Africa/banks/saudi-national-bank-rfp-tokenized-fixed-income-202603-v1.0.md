...
| Field | Value |
| --- | --- |
| Reference | SAUDI-NATIONAL-BANK-RFP-TOKENIZED-FIXED-INCOME-202603 |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Saudi National Bank (KSA) |
| Document type | RFP |
| Subject | Tokenized Fixed Income Origination And Investor Servicing |
...
Saudi National Bank is treating tokenized fixed income origination and investor servicing as a
...
The first objective is to identify a platform and supplier capable of supporting tokenized fixed
income origination and investor servicing in live operation, not just in a demonstration setting.
...
The second objective is to understand the operating model implied by each response. Saudi National
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for tokenized fixed income origination and investor servicing. |
...
wants to understand how the work actually lands inside Saudi National Bank's organisation.
...
| REQ-09 | Support issuance calendars, allocations, coupon or profit schedules, and maturity events. | High | Narrative + evidence reference |
| REQ-10 | Integrate with paying agent, registrar, depository, and settlement processes. | High | Narrative + evidence reference |
| REQ-11 | Support transfer restrictions, eligibility checks, and amendment governance. | High | Narrative + evidence reference |
...
Three issues will dominate this procurement. First is control integrity around tokenized fixed
income origination and investor servicing; the buyer must be able to identify who initiated a change
...
treasury, customer servicing, technology support, and management oversight for the tokenized fixed
income origination and investor servicing programme.
...
For Saudi National Bank, this appendix should be written in a way that supports architecture review,
...
For Saudi National Bank, operational governance for Tokenized Fixed Income Origination And Investor
Servicing must define named control owners across product management, first-line operations,
...
incident review, and management reporting. Saudi National Bank is especially interested in whether
...
A strong response should explain how data relating to Tokenized Fixed Income Origination And
Investor Servicing moves between channels, orchestration layers, policy engines, ledgers, books-and-
records, reporting stacks, and external participants. Saudi National Bank expects the bidder to
...
Integration narratives should also address release management. Saudi National Bank wants to know how
...
Given the sensitivity of Tokenized Fixed Income Origination And Investor Servicing, the buyer
...
Resilience discussion should go beyond backup existence. Saudi National Bank wants evidence of
...
configuration, interface build, testing, training, and cutover fit together. Saudi National Bank
does not expect a rigid waterfall plan, but it does expect formal acceptance criteria, issue
...
to Tokenized Fixed Income Origination And Investor Servicing, the more disciplined post-go-live
change control must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that Saudi
National Bank can build internal capability rather than remain dependent on a small external
delivery team.
...
third-party services grow. Saudi National Bank wants to know whether cost scales linearly, steps at
...
core aspects of Tokenized Fixed Income Origination And Investor Servicing depend on a single
...
Saudi National Bank (KSA) is not approaching **tokenized fixed income origination and investor servicing** as a generic blockchain exercise. SNB is positioned to benefit from Saudi capital-market deepening, digital financial-sector policy, and institutional demand for efficient fixed-income distribution. Tokenized fixed income should therefore be scoped as a scalable issuance and servicing capability aligned with SAMA, CMA, and local Sharia/governance requirements. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Likely integrations include treasury and capital-markets books, Tadawul-linked workflows, KYC/AML tooling, corporate and wealth channels, and Sharia governance processes for product approval. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Treasury, investment-banking, wealth, compliance, Sharia, and enterprise-architecture leaders are likely key decision-makers. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. Responses should assume a conservative bank procurement model with substantial attention on local hosting, security review, and controllable rollout. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For Saudi National Bank (KSA), the most relevant workload pattern is **tokenized notes and sukuk-style instruments, investor onboarding, settlement, servicing, and portfolio reporting**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how Saudi National Bank (KSA) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, Saudi National Bank (KSA) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Strategic business fit | 22% | Scored against tokenized fixed income origination and investor servicing in the specific context of Saudi National Bank (KSA). |
| Technical architecture and integration fit | 18% | Scored against tokenized fixed income origination and investor servicing in the specific context of Saudi National Bank (KSA). |
| Regulatory, compliance, and control evidence | 20% | Scored against tokenized fixed income origination and investor servicing in the specific context of Saudi National Bank (KSA). |
| Security, resilience, and operational readiness | 15% | Scored against tokenized fixed income origination and investor servicing in the specific context of Saudi National Bank (KSA). |
| Delivery credibility and partner model | 12% | Scored against tokenized fixed income origination and investor servicing in the specific context of Saudi National Bank (KSA). |
| Commercial transparency and TCO | 8% | Scored against tokenized fixed income origination and investor servicing in the specific context of Saudi National Bank (KSA). |
| Supplier viability and roadmap discipline | 5% | Scored against tokenized fixed income origination and investor servicing in the specific context of Saudi National Bank (KSA). |
...
A realistic proposal for Saudi National Bank (KSA) should therefore provide a phased deployment hypothesis. Phase one should describe the minimum safe production scope, the volumes and participant types supported at launch, required client-side roles, major control gates, and objective acceptance criteria. Phase two should describe how the scope expands once controls, throughput, and reporting have been proven. If the bidder believes the requested use case should be sequenced differently, that is acceptable, but the answer must still show a credible route from contained launch to scaled operations.
...
Control assurance is an explicit selection criterion for Saudi National Bank (KSA). The selected supplier will be expected to support walkthroughs with architecture, security, legal, compliance, operations, risk, and procurement stakeholders. Responses should therefore describe the artefacts available before contract signature and the deeper evidence available during implementation. Useful artefacts include configuration snapshots, role matrices, sample audit logs, example reconciliation outputs, incident-process documentation, environment diagrams, API specifications, backup/restore evidence, and sample operational reports. If these artefacts exist only in vendor-internal formats and cannot be shared with clients, that limitation should be declared clearly.

The rollout plan should distinguish between pilot value and production readiness. A pilot may validate that tokenized notes and sukuk-style instruments, investor onboarding, settlement, servicing, and portfolio reporting is technically feasible, but the buyer is making a sourcing decision for a programme that will need governance, support, and repeatability. Bidders should therefore outline the minimum controls needed before first live processing, the metrics that would indicate the platform is ready for broader rollout, and the dependencies that must be closed before additional products, entities, markets, or participant groups are added. Where later phases depend on external partner readiness, regulatory comfort, or contract expansion, that dependency should be made visible in the plan rather than left implicit.

Finally, respondents should explain how the programme remains governable after go-live. The buyer expects structured release management, formal change approval, regression testing, periodic entitlement review, exception governance, and management reporting that tracks the real health of the service rather than vanity metrics. Examples of meaningful indicators include exception ageing, reconciliation break volumes, failed-settlement rates, approval turnaround, security incident trends, backlog of unresolved defects, accuracy of regulatory reporting, and frequency of manual overrides. The suppliers that score best will be the ones that show how tokenized fixed income origination and investor servicing becomes an owned institutional capability rather than a clever but fragile technology deployment.



### 8.18 Institution-specific market signals and execution implications

SNB sits at the centre of Saudi corporate banking, debt capital markets, and large-scale transformation programmes. The likely buyer interest is not experimentation for its own sake but a platform that can support institutional debt workflows with better automation, evidence, and repeatability. In Saudi Arabia that also means strong scrutiny of third-party dependencies, local support, and formal governance. Saudi National Bank will evaluate proposals under SAMA, CMA, Saudi cyber and data rules, outsourcing expectations, and a capital-markets environment that is expanding rapidly under Vision 2030. Tokenized fixed-income proposals must therefore address issuance controls, investor classification, reporting, and domestic hosting or support assumptions where relevant.

This matters for the current procurement because For this procurement, bidders should show how the platform supports issuance approvals, book-building or allocation interfaces where applicable, investor entitlements, coupon servicing, maturity events, and integration to treasury, finance, and compliance systems. If the answer is heavy on generic blockchain language and light on operational debt-market mechanics, it will not land. The bank's evaluation team is unlikely to reward broad claims about future market transformation unless those claims are tied back to concrete controls, delivery phases, and accountable operating roles. In practice, a credible proposal will show how legal and policy assumptions are captured early, how architecture decisions are frozen at the right governance points, and how implementation evidence can be re-used across procurement, architecture review, information-security review, and internal-audit challenge.
