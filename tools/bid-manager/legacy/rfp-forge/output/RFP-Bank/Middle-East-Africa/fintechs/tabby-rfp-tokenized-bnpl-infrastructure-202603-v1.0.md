...
| Issuing organization | Tabby (UAE/KSA) |
| Document type | RFP |
| Subject | Tokenized Bnpl Funding And Receivables Infrastructure |
...
Tabby is treating tokenized BNPL funding and receivables infrastructure as a business-critical
...
Regional conditions in UAE/KSA also matter. The buyer expects bidders to anchor their response in
...
The first objective is to identify a platform and supplier capable of supporting tokenized BNPL
funding and receivables infrastructure in live operation, not just in a demonstration setting. The
...
The second objective is to understand the operating model implied by each response. Tabby needs to
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for tokenized BNPL funding and receivables infrastructure. |
...
wants to understand how the work actually lands inside Tabby's organisation.
...
Three issues will dominate this procurement. First is control integrity around tokenized BNPL
funding and receivables infrastructure; the buyer must be able to identify who initiated a change or
...
evidence posture are suitable for a regulated institution in UAE/KSA. The strongest responses will
...
Respondents must address the legal and supervisory context relevant to UAE/KSA, including combined
UAE and Saudi requirements, CBUAE/SCA/DFSA/ADGM rules, SAMA/CMA rules, AML/CFT, cloud and
outsourcing controls, Sharia governance where financing is Islamic. Applicability may differ by
...
treasury, customer servicing, technology support, and management oversight for the tokenized BNPL
funding and receivables infrastructure programme.
...
For Tabby, this appendix should be written in a way that supports architecture review, control
...
For Tabby, operational governance for Tokenized Bnpl Funding And Receivables Infrastructure must
...
incident review, and management reporting. Tabby is especially interested in whether those routines
...
A strong response should explain how data relating to Tokenized Bnpl Funding And Receivables
Infrastructure moves between channels, orchestration layers, policy engines, ledgers, books-and-
records, reporting stacks, and external participants. Tabby expects the bidder to identify
...
Integration narratives should also address release management. Tabby wants to know how API changes
...
Given the sensitivity of Tokenized Bnpl Funding And Receivables Infrastructure, the buyer expects
...
Resilience discussion should go beyond backup existence. Tabby wants evidence of recovery
...
configuration, interface build, testing, training, and cutover fit together. Tabby does not expect a
rigid waterfall plan, but it does expect formal acceptance criteria, issue governance, milestone
...
to Tokenized Bnpl Funding And Receivables Infrastructure, the more disciplined post-go-live change
control must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that Tabby can
...
third-party services grow. Tabby wants to know whether cost scales linearly, steps at thresholds, or
depends on discretionary supplier interpretation. A credible bid gives procurement and finance
...
core aspects of Tokenized Bnpl Funding And Receivables Infrastructure depend on a single custody,
...
Tabby (UAE/KSA) is not approaching **tokenized bnpl funding and receivables infrastructure** as a generic blockchain exercise. Tabby has major BNPL scale in the Gulf. Tokenized BNPL infrastructure would be justified only if it improves funding, receivables financing, investor transparency, and cross-market control across UAE and KSA operations. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Expected dependencies include merchant systems, lender/funding partners, KYC/AML tooling, collections/servicing, treasury and warehouse-facility reporting, and local regulatory reporting. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Treasury, capital markets, risk, compliance, data/platform engineering, and operations leaders are the likely decision-makers. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. The use case aligns with receivables financing and structured funding rather than consumer-facing crypto products. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For Tabby (UAE/KSA), the most relevant workload pattern is **tokenized BNPL receivables, funding-vehicle administration, servicing events, investor reporting, and cross-jurisdiction control**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how Tabby (UAE/KSA) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, Tabby (UAE/KSA) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Use-case fit and operating leverage | 24% | Scored against tokenized bnpl funding and receivables infrastructure in the specific context of Tabby (UAE/KSA). |
| Integration and API maturity | 18% | Scored against tokenized bnpl funding and receivables infrastructure in the specific context of Tabby (UAE/KSA). |
| Compliance, controls, and auditability | 18% | Scored against tokenized bnpl funding and receivables infrastructure in the specific context of Tabby (UAE/KSA). |
| Settlement, treasury, and resilience design | 15% | Scored against tokenized bnpl funding and receivables infrastructure in the specific context of Tabby (UAE/KSA). |
| Delivery speed and configurability | 12% | Scored against tokenized bnpl funding and receivables infrastructure in the specific context of Tabby (UAE/KSA). |
| Commercial scalability | 8% | Scored against tokenized bnpl funding and receivables infrastructure in the specific context of Tabby (UAE/KSA). |
| Supplier viability and dependency transparency | 5% | Scored against tokenized bnpl funding and receivables infrastructure in the specific context of Tabby (UAE/KSA). |
...
Public market context reinforces that Tabby (Saudi Arabia / UAE fintech) should be treated as a specific institutional
buyer, not as a placeholder for the region. It is a buy-now-pay-later and consumer commerce platform operating with merchant integrations, underwriting workflows, collections, and regulated payment or finance structures across the Gulf. Tabby's operating reality is embedded consumer finance at checkout, merchant settlement, and disciplined risk management. Tokenization would most likely be relevant for receivables funding, investor participation, treasury optimisation, or structured distribution of BNPL assets rather than public crypto features.

Relevant context includes Saudi and UAE consumer-finance regulation, merchant disclosures, data and credit-governance obligations, Sharia considerations where applicable, AML/CFT duties, and investor-eligibility rules for receivable-backed structures.

For the target use case of **tokenized BNPL infrastructure**, that public context changes the procurement lens.
...
contained innovation-lab assumption. The buyer should therefore focus on receivables-data quality, funding waterfalls, delinquency and charge-off handling, merchant settlement interfaces, and governance over special-purpose structures or partner funding vehicles.
...
- Support receivables-pool formation, eligibility rules, amortisation tracking, delinquency buckets, and investor reporting for tokenized funding structures.
- Provide strong controls around merchant disputes, refunds, restructures, and write-offs so funding or token representations stay aligned with underlying asset performance.
- Allow phased deployment beginning with institutional funding participants before any broader distribution of tokenized receivable interests.

These requirements are additive to the baseline sections of this document. They are included because
Tabby (Saudi Arabia / UAE fintech) operates with business, regulatory, and operational constraints that differ from a
