...
| Field | Value |
| --- | --- |
| Reference | CHIPPER-CASH-RFP-CROSS-BORDER-TOKENIZED-REMITTANCE-202603 |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Chipper Cash (Africa) |
| Document type | RFP |
| Subject | Cross-Border Tokenized Remittance Settlement |
...
Chipper Cash is treating cross-border tokenized remittance settlement as a business-critical
...
Regional conditions in Africa also matter. The buyer expects bidders to anchor their response in
...
The first objective is to identify a platform and supplier capable of supporting cross-border
tokenized remittance settlement in live operation, not just in a demonstration setting. The buyer
...
The second objective is to understand the operating model implied by each response. Chipper Cash
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for cross-border tokenized remittance settlement. |
...
wants to understand how the work actually lands inside Chipper Cash's organisation.
...
Three issues will dominate this procurement. First is control integrity around cross-border
tokenized remittance settlement; the buyer must be able to identify who initiated a change or
...
evidence posture are suitable for a regulated institution in Africa. The strongest responses will be
...
Respondents must address the legal and supervisory context relevant to Africa, including multi-
country African payment and securities obligations, AML/CFT and sanctions requirements, data
transfer governance, partner bank and PSP controls, FX and settlement boundaries, cross-border
resilience expectations. Applicability may differ by legal entity, product perimeter, client
segment, and whether the target structure is treated as a payment, security, custody arrangement,
deposit analogue, investment product, or supervised market activity. The bidder must therefore state
its assumptions clearly rather than implying universal applicability.
...
treasury, customer servicing, technology support, and management oversight for the cross-border
tokenized remittance settlement programme.
...
For Chipper Cash, this appendix should be written in a way that supports architecture review,
...
For Chipper Cash, operational governance for Cross-Border Tokenized Remittance Settlement must
...
incident review, and management reporting. Chipper Cash is especially interested in whether those
...
A strong response should explain how data relating to Cross-Border Tokenized Remittance Settlement
moves between channels, orchestration layers, policy engines, ledgers, books-and-records, reporting
stacks, and external participants. Chipper Cash expects the bidder to identify authoritative data
...
Integration narratives should also address release management. Chipper Cash wants to know how API
...
Given the sensitivity of Cross-Border Tokenized Remittance Settlement, the buyer expects layered
...
Resilience discussion should go beyond backup existence. Chipper Cash wants evidence of recovery
...
configuration, interface build, testing, training, and cutover fit together. Chipper Cash does not
...
to Cross-Border Tokenized Remittance Settlement, the more disciplined post-go-live change control
must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that Chipper
Cash can build internal capability rather than remain dependent on a small external delivery team.
...
third-party services grow. Chipper Cash wants to know whether cost scales linearly, steps at
...
core aspects of Cross-Border Tokenized Remittance Settlement depend on a single custody, exchange,
...
Chipper Cash (Africa) is not approaching **cross-border tokenized remittance settlement** as a generic blockchain exercise. Chipper Cash is a cross-border fintech where tokenization would be valuable only if it materially improves settlement speed, treasury efficiency, FX management, and compliance evidence across African corridors. The operating context is payments-first, not crypto-first. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Expected dependencies include local payment rails, banking partners, FX providers, sanctions/KYC vendors, treasury wallets/accounts, and merchant/consumer channels. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Payments, treasury, compliance, risk, and platform-engineering leadership are the likely sponsors. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. Public procurement evidence is limited, but cross-border remittance economics make settlement innovation commercially relevant. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For Chipper Cash (Africa), the most relevant workload pattern is **cross-border remittance settlement, prefunding optimisation, FX visibility, compliance traceability, and wallet/account interoperability**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
The regulatory perimeter for Chipper Cash (Africa) is shaped by multiple African jurisdictions with local central-bank, capital-markets, AML/CFT, sanctions, FX-control, and data-protection obligations that vary by country and booking model. Bidders must not treat that list as a box-ticking reference. They need to translate the perimeter into platform behaviour: data-location choices, audit-log retention periods, approval routing, customer classification, reporting outputs, outsourcing controls, incident escalation, access recertification, and evidentiary support during supervisory review.

Where the requested use case intersects capital-markets activity, payments, custody, digital-asset exchange operations, or tokenized deposit analogues, respondents should explain the assumed legal characterisation and the resulting control posture. If a proposed architecture depends on a regulated partner, foreign booking model, special-purpose issuer, bankruptcy-remote vehicle, or external wallet/custodian, that structure must be declared explicitly together with the residual obligations that remain with Chipper Cash (Africa). If Sharia governance is relevant, the bidder should explain how product rules, approval lineage, prohibited activity restrictions, and board or committee evidence are captured and reported.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how Chipper Cash (Africa) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, Chipper Cash (Africa) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Use-case fit and operating leverage | 24% | Scored against cross-border tokenized remittance settlement in the specific context of Chipper Cash (Africa). |
| Integration and API maturity | 18% | Scored against cross-border tokenized remittance settlement in the specific context of Chipper Cash (Africa). |
| Compliance, controls, and auditability | 18% | Scored against cross-border tokenized remittance settlement in the specific context of Chipper Cash (Africa). |
| Settlement, treasury, and resilience design | 15% | Scored against cross-border tokenized remittance settlement in the specific context of Chipper Cash (Africa). |
| Delivery speed and configurability | 12% | Scored against cross-border tokenized remittance settlement in the specific context of Chipper Cash (Africa). |
| Commercial scalability | 8% | Scored against cross-border tokenized remittance settlement in the specific context of Chipper Cash (Africa). |
| Supplier viability and dependency transparency | 5% | Scored against cross-border tokenized remittance settlement in the specific context of Chipper Cash (Africa). |
...
A realistic proposal for Chipper Cash (Africa) should therefore provide a phased deployment hypothesis. Phase one should describe the minimum safe production scope, the volumes and participant types supported at launch, required client-side roles, major control gates, and objective acceptance criteria. Phase two should describe how the scope expands once controls, throughput, and reporting have been proven. If the bidder believes the requested use case should be sequenced differently, that is acceptable, but the answer must still show a credible route from contained launch to scaled operations.
...
Public market context reinforces that Chipper Cash (Pan-African fintech) should be treated as a specific institutional
buyer, not as a placeholder for the region. It is a consumer-focused cross-border payments fintech serving multiple African corridors with wallet, transfer, and card-related services. Chipper Cash's business reality is multi-jurisdiction retail payments, corridor economics, wallet operations, and compliance-intensive cross-border movement. A tokenized remittance platform would only be credible if it reduces prefunding, settlement latency, or treasury friction without creating regulatory problems in each corridor.

The regulatory context spans money-transfer licensing, AML/CFT screening, sanctions, FX controls in certain markets, safeguarding of customer funds, partner-bank dependencies, and consumer-protection expectations across multiple African jurisdictions.

For the target use case of **cross-border tokenized remittance platform**, that public context changes the procurement lens.
...
contained innovation-lab assumption. The buyer should expect heavy emphasis on corridor segmentation, treasury and liquidity controls, reversals and complaints handling, and resilience where one local payout rail or banking partner is unavailable.
...
- Support per-corridor rules for source of funds, payout eligibility, FX controls, payout-bank constraints, and local consumer disclosures.
- Provide operational tooling for failed payouts, name mismatches, sanctions interventions, and customer-support evidence across multiple jurisdictions.
- Allow treasury teams to monitor prefunding, netting, settlement cycles, and partner-bank exposure before extending tokenized settlement across new corridors.

These requirements are additive to the baseline sections of this document. They are included because
Chipper Cash (Pan-African fintech) operates with business, regulatory, and operational constraints that differ from a
