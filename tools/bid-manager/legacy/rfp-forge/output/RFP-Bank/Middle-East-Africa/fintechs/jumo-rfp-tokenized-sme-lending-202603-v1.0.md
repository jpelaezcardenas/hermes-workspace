...
| Issuing organization | Jumo (South Africa) |
| Document type | RFP |
| Subject | Tokenized Sme Lending And Servicing Infrastructure |
...
Jumo is treating tokenized SME lending and servicing infrastructure as a business-critical
...
The first objective is to identify a platform and supplier capable of supporting tokenized SME
lending and servicing infrastructure in live operation, not just in a demonstration setting. The
...
The second objective is to understand the operating model implied by each response. Jumo needs to
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for tokenized SME lending and servicing infrastructure. |
...
wants to understand how the work actually lands inside Jumo's organisation.
...
Three issues will dominate this procurement. First is control integrity around tokenized SME lending
and servicing infrastructure; the buyer must be able to identify who initiated a change or
...
treasury, customer servicing, technology support, and management oversight for the tokenized SME
lending and servicing infrastructure programme.
...
For Jumo, this appendix should be written in a way that supports architecture review, control
...
For Jumo, operational governance for Tokenized Sme Lending And Servicing Infrastructure must define
named control owners across product management, first-line operations, compliance oversight,
...
incident review, and management reporting. Jumo is especially interested in whether those routines
...
A strong response should explain how data relating to Tokenized Sme Lending And Servicing
Infrastructure moves between channels, orchestration layers, policy engines, ledgers, books-and-
records, reporting stacks, and external participants. Jumo expects the bidder to identify
...
Integration narratives should also address release management. Jumo wants to know how API changes
...
Given the sensitivity of Tokenized Sme Lending And Servicing Infrastructure, the buyer expects
...
Resilience discussion should go beyond backup existence. Jumo wants evidence of recovery objectives,
...
configuration, interface build, testing, training, and cutover fit together. Jumo does not expect a
...
to Tokenized Sme Lending And Servicing Infrastructure, the more disciplined post-go-live change
control must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that Jumo can
...
third-party services grow. Jumo wants to know whether cost scales linearly, steps at thresholds, or
...
core aspects of Tokenized Sme Lending And Servicing Infrastructure depend on a single custody,
...
Jumo (South Africa) is not approaching **tokenized sme lending and servicing infrastructure** as a generic blockchain exercise. Jumo operates in digital lending and SME finance. Tokenized SME lending would make sense as a way to structure receivables, participations, or funding pools with stronger data lineage and investor/reporting controls, especially where multiple funders or banking partners are involved. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Likely integrations include lending decision engines, telco or bank distribution channels, data providers, treasury/funding ledgers, AML/KYC, and investor/funder reporting. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Credit, treasury, risk, data science, platform engineering, and compliance leadership are likely participants. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. Expect careful scrutiny on responsible-lending controls, data provenance, and multi-funder governance rather than pure blockchain novelty. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For Jumo (South Africa), the most relevant workload pattern is **tokenized loan participations, funding-pool administration, borrower data lineage, servicing events, and investor reporting**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how Jumo (South Africa) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, Jumo (South Africa) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Use-case fit and operating leverage | 24% | Scored against tokenized sme lending and servicing infrastructure in the specific context of Jumo (South Africa). |
| Integration and API maturity | 18% | Scored against tokenized sme lending and servicing infrastructure in the specific context of Jumo (South Africa). |
| Compliance, controls, and auditability | 18% | Scored against tokenized sme lending and servicing infrastructure in the specific context of Jumo (South Africa). |
| Settlement, treasury, and resilience design | 15% | Scored against tokenized sme lending and servicing infrastructure in the specific context of Jumo (South Africa). |
| Delivery speed and configurability | 12% | Scored against tokenized sme lending and servicing infrastructure in the specific context of Jumo (South Africa). |
| Commercial scalability | 8% | Scored against tokenized sme lending and servicing infrastructure in the specific context of Jumo (South Africa). |
| Supplier viability and dependency transparency | 5% | Scored against tokenized sme lending and servicing infrastructure in the specific context of Jumo (South Africa). |
...
A realistic proposal for Jumo (South Africa) should therefore provide a phased deployment hypothesis. Phase one should describe the minimum safe production scope, the volumes and participant types supported at launch, required client-side roles, major control gates, and objective acceptance criteria. Phase two should describe how the scope expands once controls, throughput, and reporting have been proven. If the bidder believes the requested use case should be sequenced differently, that is acceptable, but the answer must still show a credible route from contained launch to scaled operations.
...
Public market context reinforces that Jumo (Pan-African fintech) should be treated as a specific institutional
buyer, not as a placeholder for the region. It is a embedded finance and lending technology provider working with banks, mobile operators, and other partners to originate and manage credit products. Jumo's business revolves around credit decisioning, partner-distributed lending, and disciplined portfolio operations. Tokenization would most plausibly be used for funding, receivables participation, warehouse efficiency, or investor reporting rather than consumer-facing crypto distribution.

Relevant considerations include local lending and consumer-protection rules, data-use permissions, securitisation or receivables-assignment constraints, AML/CFT duties, partner-bank oversight, and investor-eligibility requirements where funding interests are digitised.

For the target use case of **tokenized SME lending platform**, that public context changes the procurement lens.
...
contained innovation-lab assumption. The buyer will care about loan-data lineage, consent and data-governance boundaries, servicing-event accuracy, and the ability to trace tokenized funding interests back to underlying receivable pools without operational ambiguity.
...
- Support pool-level and loan-level data segregation, waterfall logic, and investor reporting for tokenized receivables or funding structures.
- Provide rigorous controls for data privacy, borrower-level information minimisation, and partner permissioning across banks, mobile operators, and funders.
- Enable lifecycle handling for restructures, write-offs, recoveries, delinquency changes, and corrected servicing events without breaking investor or audit evidence.

These requirements are additive to the baseline sections of this document. They are included because
Jumo (Pan-African fintech) operates with business, regulatory, and operational constraints that differ from a
