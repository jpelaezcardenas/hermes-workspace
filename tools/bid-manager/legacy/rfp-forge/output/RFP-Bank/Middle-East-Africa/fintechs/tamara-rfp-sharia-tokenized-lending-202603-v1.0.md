...
| Issuing organization | Tamara (KSA) |
| Document type | RFP |
| Subject | Sharia-Compliant Tokenized Lending And Funding Rails |
...
Tamara is treating Sharia-compliant tokenized lending and funding rails as a business-critical
...
tokenized lending and funding rails in live operation, not just in a demonstration setting. The
buyer wants confidence that the selected solution can sustain growth in users, volumes, products,
and audit scrutiny without fundamental rework.

The second objective is to understand the operating model implied by each response. Tamara needs to
know which activities are automated, which are configurable, which require manual oversight, and
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for Sharia-compliant tokenized lending and funding rails. |
...
wants to understand how the work actually lands inside Tamara's organisation.
...
tokenized lending and funding rails; the buyer must be able to identify who initiated a change or
...
tokenized lending and funding rails programme.
...
For Tamara, this appendix should be written in a way that supports architecture review, control
...
For Tamara, operational governance for Sharia-Compliant Tokenized Lending And Funding Rails must
...
incident review, and management reporting. Tamara is especially interested in whether those routines
can be supported through native reporting and workflow evidence or whether they rely on external
manual tracking. If offline controls are required, they should be called out explicitly so the buyer
can judge staffing impact.
...
A strong response should explain how data relating to Sharia-Compliant Tokenized Lending And Funding
Rails moves between channels, orchestration layers, policy engines, ledgers, books-and-records,
reporting stacks, and external participants. Tamara expects the bidder to identify authoritative
data sources, event sequencing assumptions, idempotency treatment, reference data ownership, and how
...
Integration narratives should also address release management. Tamara wants to know how API changes
are versioned, how consumers are notified, whether backward compatibility is maintained, and how
test environments reflect production behaviour. Responses that ignore lifecycle management of
interfaces create avoidable delivery risk.

### 8.8 Security, resilience, and assurance detail

Given the sensitivity of Sharia-Compliant Tokenized Lending And Funding Rails, the buyer expects
...
Resilience discussion should go beyond backup existence. Tamara wants evidence of recovery
...
configuration, interface build, testing, training, and cutover fit together. Tamara does not expect
a rigid waterfall plan, but it does expect formal acceptance criteria, issue governance, milestone
ownership, and explicit dependency management. If the bidder proposes agile delivery, the governance
model still needs to explain when decisions are frozen and what constitutes readiness to progress.
...
to Sharia-Compliant Tokenized Lending And Funding Rails, the more disciplined post-go-live change
control must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that Tamara can
build internal capability rather than remain dependent on a small external delivery team.
...
third-party services grow. Tamara wants to know whether cost scales linearly, steps at thresholds,
or depends on discretionary supplier interpretation. A credible bid gives procurement and finance
enough information to model multiple operating scenarios.

Supplier transparency also includes product strategy, concentration risk, and partner dependency. If
core aspects of Sharia-Compliant Tokenized Lending And Funding Rails depend on a single custody,
...
Tamara (KSA) is not approaching **sharia-compliant tokenized lending and funding rails** as a generic blockchain exercise. Tamara's scale in BNPL and merchant finance makes Sharia-aware tokenized lending or receivables structures commercially relevant if they improve warehouse funding, transparency, and governance. The programme must align with SAMA, CMA, PDPL, and Sharia review where applicable. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Likely integrations include merchant platforms, funding partners, treasury and receivables systems, collections and servicing, AML/KYC, and Sharia governance processes. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Treasury, funding, risk, compliance, product, platform engineering, and Sharia advisers are the likely stakeholders. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. Expect funding and control economics to matter more than novelty; proposals should be framed around scalable receivables administration. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For Tamara (KSA), the most relevant workload pattern is **Sharia-aware tokenized receivables, funding-programme administration, servicing events, merchant and investor reporting, and control evidence**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how Tamara (KSA) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, Tamara (KSA) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Use-case fit and operating leverage | 24% | Scored against sharia-compliant tokenized lending and funding rails in the specific context of Tamara (KSA). |
| Integration and API maturity | 18% | Scored against sharia-compliant tokenized lending and funding rails in the specific context of Tamara (KSA). |
| Compliance, controls, and auditability | 18% | Scored against sharia-compliant tokenized lending and funding rails in the specific context of Tamara (KSA). |
| Settlement, treasury, and resilience design | 15% | Scored against sharia-compliant tokenized lending and funding rails in the specific context of Tamara (KSA). |
| Delivery speed and configurability | 12% | Scored against sharia-compliant tokenized lending and funding rails in the specific context of Tamara (KSA). |
| Commercial scalability | 8% | Scored against sharia-compliant tokenized lending and funding rails in the specific context of Tamara (KSA). |
| Supplier viability and dependency transparency | 5% | Scored against sharia-compliant tokenized lending and funding rails in the specific context of Tamara (KSA). |
...
A realistic proposal for Tamara (KSA) should therefore provide a phased deployment hypothesis. Phase one should describe the minimum safe production scope, the volumes and participant types supported at launch, required client-side roles, major control gates, and objective acceptance criteria. Phase two should describe how the scope expands once controls, throughput, and reporting have been proven. If the bidder believes the requested use case should be sequenced differently, that is acceptable, but the answer must still show a credible route from contained launch to scaled operations.
...
Public market context reinforces that Tamara (Saudi Arabia fintech) should be treated as a specific institutional
buyer, not as a placeholder for the region. It is a Saudi-origin BNPL and consumer-finance platform with merchant integrations, checkout distribution, and increasing focus on compliant funding and scalable credit operations. Tamara operates in a market where Sharia alignment, consumer transparency, and disciplined funding structures matter enormously. A tokenized lending or receivables platform would need to respect that context by making structure, asset linkage, and governance explicit rather than layering vague digital-asset language over consumer-finance exposures.

Key context includes Saudi consumer-finance and fintech regulation, CMA considerations if funding interests are distributed as securities, AML/CFT obligations, data and cyber governance, and Sharia-board oversight where financing structures require formal approval.

For the target use case of **Sharia tokenized lending platform**, that public context changes the procurement lens.
...
contained innovation-lab assumption. This means the platform must support receivables-level traceability, funder and SPV governance, clear profit or fee treatment, and evidence that product rules remain aligned with approved Sharia structure through servicing and collections.
...
- Support Sharia-governance artefacts including board or scholar approvals, restricted activity rules, document versioning, and evidence of ongoing structural compliance.
- Provide lifecycle support for refunds, cancellations, early settlements, collections, restructures, and write-offs without breaking funding-pool accounting or investor evidence.
- Allow careful segmentation between customer-facing BNPL operations and institutional funding or tokenized participation layers so regulatory and reputational risk stay contained.

These requirements are additive to the baseline sections of this document. They are included because
Tamara (Saudi Arabia fintech) operates with business, regulatory, and operational constraints that differ from a
