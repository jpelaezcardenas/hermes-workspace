...
| Procurement theme | Tokenized Lending Platform |
| Reference | ZIP-CO-RFP-202603 |
| Version | v1.0 |
| Issue month | March 2026 |
| Indicative currency | AUD |
| Primary regulatory context | APRA-adjacent expectations, ASIC, Privacy Act Australia |
| Relevant market initiatives | consumer credit platform modernization |
| Classification | Confidential – invited bidders only |

## 1. Procurement Context

Zip Co is issuing this Request for Proposal to identify a qualified technology provider and implementation partner for a regulated **tokenized lending platform** programme in Australia. This is not a beauty parade and it is not an invitation to recycle a generic digital-asset deck. The institution expects bidders to respond as if the programme will be scrutinised by risk, internal audit, cyber security, legal, compliance, operations, and senior management from day one. Production readiness, governance clarity, and the ability to survive boring operational reality matter more than glossy pilot language.

The procurement context is informed by publicly visible market activity around BNPL portfolio funding and receivables management, securitisation adjacency, Australia consumer credit oversight. Market signals reviewed during pre-procurement analysis indicate that Zip Co operates in an environment where digital asset initiatives are moving from exploratory proof points toward controlled institutional deployment. Recent public material reviewed by the institution includes the following directional intelligence:
...
- establish a controlled and reusable operating model for tokenized lending platform that can move from pilot scope to business-as-usual operations without a re-platform;
...
- ensure regulatory and audit readiness in line with APRA-adjacent expectations, ASIC, Privacy Act Australia;
...
- position Zip Co to participate credibly in market initiatives connected to consumer credit platform modernization without creating stranded technology or governance debt.
...
- institution-specific context matters: responses must show an understanding of Australia market structure, operating constraints, and the programme relevance of consumer credit platform modernization;
...
The selected vendor shall provide software, implementation services, configuration support, documentation, training, and transition assistance required to establish a production-ready tokenized lending platform environment. The scope includes business workflows, technical architecture, controls, integration services, operational tooling, support arrangements, and the artefacts necessary for audit and risk sign-off.
...
Respondents should assume integration into the institution's existing enterprise landscape, including finance ledgers, payment and settlement systems, data warehouses or lakes, risk and compliance tooling, identity and access management, and service management / observability platforms. For Zip Co, the integration pattern must also reflect current modernization priorities around tokenized lending platform and the public-market direction signalled through consumer credit platform modernization.

### 2.2 Technical requirements table

| Req ID | Requirement | Priority | Response Type | Evaluation Notes |
| --- | --- | --- | --- | --- |
| TR-01 | End-to-end lifecycle support for tokenized lending platform, including initiation, approval, issuance/origination, servicing, reporting, exception handling, and closure. | Must | Comply with narrative | Institution expects lifecycle completeness, not a narrow pilot demo. |
...
| TR-20 | Approach for a future-state roadmap aligned to Zip Co strategic direction, while clearly distinguishing live capability from roadmap-only functionality. | Should | Narrative | No hand-wavy roadmap theatre. |
...
| --- | --- | --- |
| Strategic and use-case fit | 18% | Depth of alignment to Zip Co's stated objectives in tokenized lending platform, including realism of business case, target operating model, and phased execution approach. |
...
| Security, regulatory, and control maturity | 17% | Alignment to APRA-adjacent expectations, ASIC, Privacy Act Australia, auditability, change control, data governance, and cyber posture. |
...
- executive summary and solution overview tailored to Zip Co and tokenized lending platform;
...
The institution requires bidders to describe a security architecture appropriate for high-trust regulated environments. Responses should cover identity federation, privileged access management, device or session controls for administrators, key management and HSM/KMS integration, certificate lifecycle management, vulnerability management, patching cadence, code-signing, configuration hardening, logging integrity, and alerting thresholds. For tokenized lending platform, the institution is especially concerned with how administrative override events are controlled, evidenced, and independently reviewable.
...
Responses should further address cyber-resilience expectations relevant to Australia, including alignment with APRA-adjacent expectations, ASIC, Privacy Act Australia. Vendors should describe secure software development practices, penetration testing scope and frequency, independent assurance results, backup encryption, immutable logging options, evidence retention, and post-incident review workflows. The winning bidder will need to support scrutiny not only from security teams but also from risk committees, audit functions, and external regulators or supervisors.

### Appendix K – Data, reporting, and audit evidence expectations

A recurring failure mode in digital-asset programmes is that transaction workflows are built first and reporting is improvised later. Zip Co is explicitly trying to avoid that mistake. Respondents shall therefore describe how the platform captures, stores, enriches, reconciles, and exports operational data, accounting data, and control evidence across the full lifecycle of tokenized lending platform. The institution expects support for event lineage, field-level traceability, time synchronization, report reproducibility, and controlled data retention.
...
Responses should demonstrate clear awareness of how Australia market structure affects solution design for tokenized lending platform. That includes the interplay between local regulatory expectations, domestic payment or settlement infrastructure, data location constraints, outsourcing scrutiny, and the practical role of internal control functions. Bidders should discuss how their platform and delivery model align with APRA-adjacent expectations, ASIC, Privacy Act Australia, and how those obligations affect deployment choices, access controls, resilience design, evidence retention, and third-party contracting.

The institution also expects respondents to connect their answer to the market initiatives most relevant to this procurement, namely consumer credit platform modernization. The goal is not to repeat press releases. The goal is to show that the bidder understands how those initiatives influence buyer expectations around interoperability, programmability, governance, and production transition. Where the vendor believes a market initiative is not directly relevant, it should say so and explain why.
...
Cutover planning must include go/no-go criteria, command structure, decision rights, communications, backout triggers, manual contingency procedures, and post-cutover observation periods. Respondents should set out how reconciliation will be validated on day one, how open exceptions will be triaged, how access will be controlled during the high-risk cutover window, and how business, risk, compliance, and technology stakeholders will be kept aligned. Because Zip Co operates in a regulated environment, cutover is not merely a technical switch; it is a controlled business event that must withstand retrospective scrutiny.
...
Bidders should also address future-state scenarios. Examples include expansion to new legal entities, new business lines, more counterparties, additional jurisdictions, or broader participation in initiatives related to consumer credit platform modernization. The institution wants to understand whether the proposed architecture can scale through configuration and governed rollout, or whether each expansion will trigger a mini reimplementation. A scalable answer will discuss parameterization, reusable templates, tenant or entity isolation, policy inheritance, and structured approval workflows for new product variants.
...
Ninth, bidders should explain how future roadmap items are governed. Zip Co does not object to roadmaps; every serious platform has one. What matters is whether roadmap items are separated cleanly from committed deliverables, how client influence is handled, how security and regulatory implications are reviewed before release, and how backwards compatibility is managed. The institution does not want a strategic dependency on promises that are still floating around a product meeting somewhere.
