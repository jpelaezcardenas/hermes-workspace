...
- establish a controlled and reusable operating model for tokenized settlement that can move from pilot scope to business-as-usual operations without a re-platform;
...
- ensure regulatory and audit readiness in line with ASIC, RBA, APRA-adjacent expectations;
...
- position ASX to participate credibly in market initiatives connected to post-trade modernization, DLT settlement analysis without creating stranded technology or governance debt.
...
- institution-specific context matters: responses must show an understanding of Australia market structure, operating constraints, and the programme relevance of post-trade modernization, DLT settlement analysis;
...
The selected vendor shall provide software, implementation services, configuration support, documentation, training, and transition assistance required to establish a production-ready tokenized settlement environment. The scope includes business workflows, technical architecture, controls, integration services, operational tooling, support arrangements, and the artefacts necessary for audit and risk sign-off.
...
Respondents should assume integration into the institution's existing enterprise landscape, including finance ledgers, payment and settlement systems, data warehouses or lakes, risk and compliance tooling, identity and access management, and service management / observability platforms. For ASX, the integration pattern must also reflect current modernization priorities around tokenized settlement and the public-market direction signalled through post-trade modernization, DLT settlement analysis.

### 2.2 Technical requirements table

| Req ID | Requirement | Priority | Response Type | Evaluation Notes |
| --- | --- | --- | --- | --- |
| TR-01 | End-to-end lifecycle support for tokenized settlement, including initiation, approval, issuance/origination, servicing, reporting, exception handling, and closure. | Must | Comply with narrative | Institution expects lifecycle completeness, not a narrow pilot demo. |
...
| TR-20 | Approach for a future-state roadmap aligned to ASX strategic direction, while clearly distinguishing live capability from roadmap-only functionality. | Should | Narrative | No hand-wavy roadmap theatre. |
...
| --- | --- | --- |
| Strategic and use-case fit | 18% | Depth of alignment to ASX's stated objectives in tokenized settlement, including realism of business case, target operating model, and phased execution approach. |
...
| Security, regulatory, and control maturity | 17% | Alignment to ASIC, RBA, APRA-adjacent expectations, auditability, change control, data governance, and cyber posture. |
...
- executive summary and solution overview tailored to ASX and tokenized settlement;
...
The institution requires bidders to describe a security architecture appropriate for high-trust regulated environments. Responses should cover identity federation, privileged access management, device or session controls for administrators, key management and HSM/KMS integration, certificate lifecycle management, vulnerability management, patching cadence, code-signing, configuration hardening, logging integrity, and alerting thresholds. For tokenized settlement, the institution is especially concerned with how administrative override events are controlled, evidenced, and independently reviewable.
...
Responses should further address cyber-resilience expectations relevant to Australia, including alignment with ASIC, RBA, APRA-adjacent expectations. Vendors should describe secure software development practices, penetration testing scope and frequency, independent assurance results, backup encryption, immutable logging options, evidence retention, and post-incident review workflows. The winning bidder will need to support scrutiny not only from security teams but also from risk committees, audit functions, and external regulators or supervisors.

### Appendix K – Data, reporting, and audit evidence expectations

A recurring failure mode in digital-asset programmes is that transaction workflows are built first and reporting is improvised later. ASX is explicitly trying to avoid that mistake. Respondents shall therefore describe how the platform captures, stores, enriches, reconciles, and exports operational data, accounting data, and control evidence across the full lifecycle of tokenized settlement. The institution expects support for event lineage, field-level traceability, time synchronization, report reproducibility, and controlled data retention.
...
Responses should demonstrate clear awareness of how Australia market structure affects solution design for tokenized settlement. That includes the interplay between local regulatory expectations, domestic payment or settlement infrastructure, data location constraints, outsourcing scrutiny, and the practical role of internal control functions. Bidders should discuss how their platform and delivery model align with ASIC, RBA, APRA-adjacent expectations, and how those obligations affect deployment choices, access controls, resilience design, evidence retention, and third-party contracting.

The institution also expects respondents to connect their answer to the market initiatives most relevant to this procurement, namely post-trade modernization, DLT settlement analysis. The goal is not to repeat press releases. The goal is to show that the bidder understands how those initiatives influence buyer expectations around interoperability, programmability, governance, and production transition. Where the vendor believes a market initiative is not directly relevant, it should say so and explain why.
...
Cutover planning must include go/no-go criteria, command structure, decision rights, communications, backout triggers, manual contingency procedures, and post-cutover observation periods. Respondents should set out how reconciliation will be validated on day one, how open exceptions will be triaged, how access will be controlled during the high-risk cutover window, and how business, risk, compliance, and technology stakeholders will be kept aligned. Because ASX operates in a regulated environment, cutover is not merely a technical switch; it is a controlled business event that must withstand retrospective scrutiny.
...
Bidders should also address future-state scenarios. Examples include expansion to new legal entities, new business lines, more counterparties, additional jurisdictions, or broader participation in initiatives related to post-trade modernization, DLT settlement analysis. The institution wants to understand whether the proposed architecture can scale through configuration and governed rollout, or whether each expansion will trigger a mini reimplementation. A scalable answer will discuss parameterization, reusable templates, tenant or entity isolation, policy inheritance, and structured approval workflows for new product variants.
...
Ninth, bidders should explain how future roadmap items are governed. ASX does not object to roadmaps; every serious platform has one. What matters is whether roadmap items are separated cleanly from committed deliverables, how client influence is handled, how security and regulatory implications are reviewed before release, and how backwards compatibility is managed. The institution does not want a strategic dependency on promises that are still floating around a product meeting somewhere.
