...
| Region | APAC |
| Country / market | Hong Kong |
| Procurement theme | E-Hkd Infrastructure |
| Reference | HONG-KONG-MONETARY-AUTHORITY-RFP-202603 |
| Version | v1.0 |
| Issue month | March 2026 |
| Indicative currency | HKD |
| Primary regulatory context | HKMA, SFC coordination, PDPO Hong Kong |
| Relevant market initiatives | e-HKD pilots, Project Ensemble, mBridge |
| Classification | Confidential – invited bidders only |

## 1. Procurement Context

Hong Kong Monetary Authority is issuing this Request for Proposal to identify a qualified technology provider and implementation partner for a regulated **e-hkd infrastructure** programme in Hong Kong. This is not a beauty parade and it is not an invitation to recycle a generic digital-asset deck. The institution expects bidders to respond as if the programme will be scrutinised by risk, internal audit, cyber security, legal, compliance, operations, and senior management from day one. Production readiness, governance clarity, and the ability to survive boring operational reality matter more than glossy pilot language.

The procurement context is informed by publicly visible market activity around e-HKD pilot, Project Ensemble, tokenized deposit and interbank settlement exploration. Market signals reviewed during pre-procurement analysis indicate that Hong Kong Monetary Authority operates in an environment where digital asset initiatives are moving from exploratory proof points toward controlled institutional deployment. Recent public material reviewed by the institution includes the following directional intelligence:



The institution has also considered publicly visible executive and governance signals. These do not replace direct stakeholder confirmation, but they do indicate where sponsorship and challenge are likely to sit. In practical terms, senior sponsorship is likely to sit across fintech facilitation, payments, and banking supervision leadership Relevant public references include:
...
- establish a controlled and reusable operating model for e-hkd infrastructure that can move from pilot scope to business-as-usual operations without a re-platform;
...
- ensure regulatory and audit readiness in line with HKMA, SFC coordination, PDPO Hong Kong;
...
- position Hong Kong Monetary Authority to participate credibly in market initiatives connected to e-HKD pilots, Project Ensemble, mBridge without creating stranded technology or governance debt.
...
- institution-specific context matters: responses must show an understanding of Hong Kong market structure, operating constraints, and the programme relevance of e-HKD pilots, Project Ensemble, mBridge;
...
The selected vendor shall provide software, implementation services, configuration support, documentation, training, and transition assistance required to establish a production-ready e-hkd infrastructure environment. The scope includes business workflows, technical architecture, controls, integration services, operational tooling, support arrangements, and the artefacts necessary for audit and risk sign-off.
...
Respondents should assume integration into the institution's existing enterprise landscape, including finance ledgers, payment and settlement systems, data warehouses or lakes, risk and compliance tooling, identity and access management, and service management / observability platforms. For Hong Kong Monetary Authority, the integration pattern must also reflect current modernization priorities around e-hkd infrastructure and the public-market direction signalled through e-HKD pilots, Project Ensemble, mBridge.

### 2.2 Technical requirements table

| Req ID | Requirement | Priority | Response Type | Evaluation Notes |
| --- | --- | --- | --- | --- |
| TR-01 | End-to-end lifecycle support for e-hkd infrastructure, including initiation, approval, issuance/origination, servicing, reporting, exception handling, and closure. | Must | Comply with narrative | Institution expects lifecycle completeness, not a narrow pilot demo. |
...
| TR-04 | Alignment with HKMA supervisory expectations, SFC virtual asset and tokenized product guidance, PDPO, cyber resilience framework, e-HKD and Project Ensemble related supervisory learnings including audit evidence and control mapping. | Must | Narrative + control mapping | Regulatory fit is a board-level issue. |
...
| TR-13 | Integration approach for institution-specific infrastructure, including ERP/GL, treasury, CRM, case management, observability stack, and domestic payment or market infrastructure in Hong Kong. | Must | Attachment | The institution expects a realistic connectivity plan. |
...
| TR-20 | Approach for a future-state roadmap aligned to Hong Kong Monetary Authority strategic direction, while clearly distinguishing live capability from roadmap-only functionality. | Should | Narrative | No hand-wavy roadmap theatre. |
...
The proposed solution shall align with the legal, supervisory, and risk-management expectations applicable to Hong Kong. At minimum, respondents must describe how their offering supports requirements arising under **HKMA, SFC coordination, PDPO Hong Kong**. Where the proposed architecture spans multiple jurisdictions, the respondent shall distinguish domestic controls from cross-border controls and identify any legal, licensing, tax, sanctions, or data-transfer assumptions.
...
| --- | --- | --- |
| Strategic and use-case fit | 18% | Depth of alignment to Hong Kong Monetary Authority's stated objectives in e-hkd infrastructure, including realism of business case, target operating model, and phased execution approach. |
...
| Security, regulatory, and control maturity | 17% | Alignment to HKMA, SFC coordination, PDPO Hong Kong, auditability, change control, data governance, and cyber posture. |
...
- executive summary and solution overview tailored to Hong Kong Monetary Authority and e-hkd infrastructure;
...
The institution requires bidders to describe a security architecture appropriate for high-trust regulated environments. Responses should cover identity federation, privileged access management, device or session controls for administrators, key management and HSM/KMS integration, certificate lifecycle management, vulnerability management, patching cadence, code-signing, configuration hardening, logging integrity, and alerting thresholds. For e-hkd infrastructure, the institution is especially concerned with how administrative override events are controlled, evidenced, and independently reviewable.
...
Responses should further address cyber-resilience expectations relevant to Hong Kong, including alignment with HKMA, SFC coordination, PDPO Hong Kong. Vendors should describe secure software development practices, penetration testing scope and frequency, independent assurance results, backup encryption, immutable logging options, evidence retention, and post-incident review workflows. The winning bidder will need to support scrutiny not only from security teams but also from risk committees, audit functions, and external regulators or supervisors.

### Appendix K – Data, reporting, and audit evidence expectations

A recurring failure mode in digital-asset programmes is that transaction workflows are built first and reporting is improvised later. Hong Kong Monetary Authority is explicitly trying to avoid that mistake. Respondents shall therefore describe how the platform captures, stores, enriches, reconciles, and exports operational data, accounting data, and control evidence across the full lifecycle of e-hkd infrastructure. The institution expects support for event lineage, field-level traceability, time synchronization, report reproducibility, and controlled data retention.
...
Responses should demonstrate clear awareness of how Hong Kong market structure affects solution design for e-hkd infrastructure. That includes the interplay between local regulatory expectations, domestic payment or settlement infrastructure, data location constraints, outsourcing scrutiny, and the practical role of internal control functions. Bidders should discuss how their platform and delivery model align with HKMA, SFC coordination, PDPO Hong Kong, and how those obligations affect deployment choices, access controls, resilience design, evidence retention, and third-party contracting.

The institution also expects respondents to connect their answer to the market initiatives most relevant to this procurement, namely e-HKD pilots, Project Ensemble, mBridge. The goal is not to repeat press releases. The goal is to show that the bidder understands how those initiatives influence buyer expectations around interoperability, programmability, governance, and production transition. Where the vendor believes a market initiative is not directly relevant, it should say so and explain why.
...
Cutover planning must include go/no-go criteria, command structure, decision rights, communications, backout triggers, manual contingency procedures, and post-cutover observation periods. Respondents should set out how reconciliation will be validated on day one, how open exceptions will be triaged, how access will be controlled during the high-risk cutover window, and how business, risk, compliance, and technology stakeholders will be kept aligned. Because Hong Kong Monetary Authority operates in a regulated environment, cutover is not merely a technical switch; it is a controlled business event that must withstand retrospective scrutiny.
...
Bidders should also address future-state scenarios. Examples include expansion to new legal entities, new business lines, more counterparties, additional jurisdictions, or broader participation in initiatives related to e-HKD pilots, Project Ensemble, mBridge. The institution wants to understand whether the proposed architecture can scale through configuration and governed rollout, or whether each expansion will trigger a mini reimplementation. A scalable answer will discuss parameterization, reusable templates, tenant or entity isolation, policy inheritance, and structured approval workflows for new product variants.
...
Ninth, bidders should explain how future roadmap items are governed. Hong Kong Monetary Authority does not object to roadmaps; every serious platform has one. What matters is whether roadmap items are separated cleanly from committed deliverables, how client influence is handled, how security and regulatory implications are reviewed before release, and how backwards compatibility is managed. The institution does not want a strategic dependency on promises that are still floating around a product meeting somewhere.
