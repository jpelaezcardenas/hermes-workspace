...
# ING Group — Request for Proposal
## Tokenized Trade Finance Platform
...
- Digitise trade finance assets and related documentary workflows without compromising credit and compliance controls.
- Link tokenized exposures to existing transaction banking, kyc, sanctions, and collateral frameworks.
- Create a reusable platform for receivables, guarantees, and supply-chain finance use cases.

The target environment should support both bank-originated and ecosystem-participant workflows with clear data ownership and permission boundaries. The buyer expects vendors to respond in a disciplined, evidence-backed manner and to distinguish clearly between delivered functionality, configurable functionality, partner-enabled capability, and roadmap items.

### 1.2 Procurement objectives and intended outcomes
The objective of this procurement is to identify one or more vendors able to provide a platform and associated implementation services suitable for the bank's target operating model for tokenized trade finance. The desired outcome is not simply a technology deployment; it is a control-aligned operating capability that can be governed by the buyer as a regulated institution. Vendors must therefore demonstrate not only product breadth but also implementation realism, control design maturity, and credible long-term supportability.
...
The governing regulatory context includes MiCA, MiFID II where securities treatment applies, DORA, GDPR, Dutch financial supervision requirements, trade finance AML and sanctions obligations. Vendors remain responsible for identifying any additional regulatory dependencies relevant to their proposed architecture, hosting model, and service delivery structure.
...
|---|---|---|---|---|
| TR-01 | The platform shall support end-to-end lifecycle management for tokenized trade finance including origination, onboarding, issuance, servicing, transfer, redemption, and archival record retention. | Mandatory | Comply + narrative | Solution architecture and workflow diagrams |
...
The buyer requires proposals that are workable within the Dutch and EU regulatory perimeter. Responses shall explain how the proposed platform supports a regulated financial institution operating under MiCA, MiFID II where securities treatment applies, DORA, GDPR, Dutch financial supervision requirements, trade finance AML and sanctions obligations. The response must distinguish between obligations addressed by product design, obligations addressed through buyer policy or operating procedure, and obligations dependent on third-party service providers.
