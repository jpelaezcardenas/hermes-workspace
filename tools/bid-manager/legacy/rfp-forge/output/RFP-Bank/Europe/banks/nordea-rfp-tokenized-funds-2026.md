...
date: 2026-03-14
version: v1.0
reference: NOR-2026-001
---

# Request for Proposal: Tokenized Fund Servicing Platform for Nordic Institutional Distribution

## Section 1: Context, Scope, and Timeline

### 1.1 Issuing Organization
Nordea operates a Nordic banking franchise spanning multiple jurisdictions and customer segments, with strong obligations around resilience, data governance and cross-border operating consistency. The Bank is assessing digital asset infrastructure from the perspective of institutional servicing and transaction banking practicality rather than speculative market participation. We require technology and operating models that can work across a Nordic control environment and support multi-entity governance. Primary drivers include more efficient fund distribution and servicing, better transparency across transfer restrictions and investor categories, and the possibility of linking tokenized fund operations with modern payment and liquidity workflows across the Nordic region. The Bank is especially focused on cross-border interoperability, harmonised controls and operational simplicity.
...
Applicable considerations include ECB and local Nordic supervisory expectations, PSD2 / PSD3 trajectory where relevant, AML/CFT controls, GDPR, DORA, NIS2, EBA outsourcing guidance, settlement finality analysis, and cross-border data handling obligations across Finland and the wider Nordic footprint.

### 1.2 Scope and Key Scenarios
This RFP is for a tokenized fund platform supporting institutional and professional investor servicing. Capabilities in scope include issuance of fund units or shares, subscription and redemption workflow support, cut-off handling, eligibility controls, transfer restrictions, registrar-style position records, NAV-driven servicing events, distribution event support, and integration with fund accounting, transfer agency, custody and payments processes.
...
| --- | --- | --- |
| NOR-B1 | Support controlled creation, approval and release of the target asset lifecycle with configurable maker-checker controls. | Mandatory |
| NOR-B2 | Provide investor or participant eligibility rules aligned to jurisdiction, product and distribution constraints. | Mandatory |
| NOR-B3 | Support lifecycle servicing events including distributions, redemptions, transfers, amendments and exceptions. | Mandatory |
| NOR-B4 | Provide operational dashboards, exception queues and status tracking for front-to-back teams. | Scored |
| NOR-B5 | Support configurable metadata and document linkage for legal, operational and reporting artefacts. | Scored |
| NOR-B6 | Enable scenario-based simulation or test workflows before production release of new products. | Desirable |
...
| ID | Requirement description | Priority |
| --- | --- | --- |
| NOR-T1 | Expose documented APIs and event interfaces suitable for enterprise integration and automation. | Mandatory |
| NOR-T2 | Support deployment patterns compatible with bank-controlled hosting and segregated environments. | Mandatory |
| NOR-T3 | Provide comprehensive audit logs, configuration history and environment-specific controls. | Mandatory |
| NOR-T4 | Support integration with IAM, key management, monitoring and enterprise workflow tooling. | Scored |
| NOR-T5 | Provide non-functional evidence for scalability, resilience and performance under peak event loads. | Scored |
| NOR-T6 | Offer migration tooling for reference data, positions, static data or configuration baselines. | Desirable |

The target environment must connect to fund accounting, transfer agency interfaces, custody records, transaction banking systems, sanctions screening, liquidity management, ISO 20022 message processing, data lake/reporting environments, enterprise IAM and central observability tooling. Where third-party managed components are required, respondents must specify which party operates them, how incidents are handled, and what evidence can be supplied to satisfy internal security review and outsourcing governance.
...
| ID | Requirement description | Priority |
| --- | --- | --- |
| NOR-S1 | Support role-based access control, least privilege and segregation of duties across administration and operations. | Mandatory |
| NOR-S2 | Provide immutable or tamper-evident audit evidence for critical actions and lifecycle events. | Mandatory |
| NOR-S3 | Support data residency, retention and deletion controls aligned to GDPR and local obligations. | Mandatory |
| NOR-S4 | Support security monitoring, incident investigation and evidence export to bank SOC tooling. | Scored |
| NOR-S5 | Demonstrate operational resilience and third-party risk controls consistent with DORA or equivalent expectations. | Scored |
| NOR-S6 | Provide independent assurance artefacts and vulnerability-management processes. | Desirable |

Applicable considerations include ECB and local Nordic supervisory expectations, PSD2 / PSD3 trajectory where relevant, AML/CFT controls, GDPR, DORA, NIS2, EBA outsourcing guidance, settlement finality analysis, and cross-border data handling obligations across Finland and the wider Nordic footprint. Respondents should disclose any known control gaps, roadmap dependencies or jurisdictional limitations. Evidence such as certifications, assurance reports, penetration testing summaries and secure development lifecycle documentation may be attached as appendices.
...
Nordea expects clear Nordic support coverage, strong documentation standards, multilingual readiness where needed, testability for cross-entity scenarios, and a vendor operating model that can support phased rollout across business lines without excessive bespoke engineering. Respondents shall indicate the level of support available during critical events such as issuance dates, payment windows, maturity events, period-end reporting or contingency recovery exercises. Documentation must be suitable for business users, support teams, architects, security reviewers and auditors.
...
Nordea expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Nordea has maintained a measured stance on digital assets while evaluating how tokenization and digital cash could improve institutional payments, funds and post-trade efficiency. The realistic procurement posture is therefore exploratory but serious: the bank wants concrete answers on cross-border payments, liquidity, compliance and integration without committing to speculative business models. In practical terms, that means the proposed platform must support fund tokenization, transfer-agency processes, subscriptions, redemptions, NAV-related workflows, investor restrictions and distributor connectivity while fitting into cross-border payments infrastructure, correspondent banking controls, treasury and liquidity systems, KYC/sanctions screening, reporting platforms and Nordic regional operating environments. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. ownership would likely span transaction banking, payments, technology, compliance, operations and architecture leadership. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, Nordea is likely to weight resilience, cross-border controls and interoperability across multiple legal entities and regional operating models. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by ECB supervision, national Nordic regulators including Finansinspektionen where applicable, MiCA, DORA, GDPR and cross-border payments controls. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes cross-border payments infrastructure, correspondent banking controls, treasury and liquidity systems, KYC/sanctions screening, reporting platforms and Nordic regional operating environments. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
