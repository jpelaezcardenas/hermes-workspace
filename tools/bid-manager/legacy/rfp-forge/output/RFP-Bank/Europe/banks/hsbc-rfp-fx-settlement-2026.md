...
document-type: rfp
asset-class: fx settlement
date: 2026-03-14
version: v1.0
reference: HSBC-2026-001
---

# Request for Proposal: FX Settlement Orchestration Platform

## Section 1: Context, Scope, and Timeline

### 1.1 Issuing Organization
HSBC is assessing a platform for controlled modernization of FX settlement workflows where programmability, intra-day visibility and conditional transfer mechanisms may improve selected treasury and institutional payment corridors. The Bank also seeks non-binding market information on tokenized bond capabilities relevant to its broader securities services and wholesale banking activities. Any shortlisted solution must support a highly controlled UK operating environment, robust resilience, clear legal role allocation and integration with treasury, payment operations and sanctions screening functions. The Bank will prioritize practical reduction of operational friction over purely experimental blockchain features.
...
From a market standpoint, drivers include shorter settlement expectations, demand for improved transparency over ownership and lifecycle events, pressure to reduce manual reconciliations, and the need to support future delivery-versus-payment or programmable servicing patterns. The selected platform, if any, must prove that these benefits can be realized without compromising legal certainty, investor protection, information security or operational resilience. Priority is payment workflow certainty, sanctions and screening integration, intraday controls, and credible reconciliation between digital transfer events and existing nostro/vostro and treasury processes.

### 1.2 Scope and Key Scenarios
This RFP concerns fx settlement as the primary asset class. The Bank is seeking binding proposals for an implementation-capable platform. The in-scope scenarios include Selected internal treasury and institutional FX settlement scenarios involving conditional PvP-style orchestration, prefunding visibility, exception management and auditable reconciliation between trade instruction, settlement confirmation and accounting events.
...
|---|---|---|
| HSBC-BR1 | Support configurable asset lifecycle workflows with maker-checker approval and complete audit trail | Mandatory |
| HSBC-BR2 | Provide participant, holding and entitlement views suitable for operations, compliance and reporting users | Mandatory |
| HSBC-BR3 | Handle scheduled and exception-based events, including corrections and reversals | Mandatory |
| HSBC-BR4 | Allow business users to configure core rules, templates and approval paths without code changes | Scored |
| HSBC-BR5 | Support multilingual document and notification generation where relevant to operating entities | Desirable |
...
| ID | Requirement | Priority |
|---|---|---|
| HSBC-TR1 | Expose versioned APIs and event interfaces for workflow, holdings, transactions and reporting data | Mandatory |
| HSBC-TR2 | Support segregated environments for development, testing, pre-production and production | Mandatory |
| HSBC-TR3 | Provide high availability and documented disaster recovery objectives | Mandatory |
| HSBC-TR4 | Offer deployment options compatible with the Bank's cloud and network security policies | Scored |
| HSBC-TR5 | Provide infrastructure-as-code or equivalent automated deployment support | Desirable |

The current system landscape includes Treasury management, sanctions screening, payment engines, SWIFT, market operations platforms, finance controls, service management tooling and enterprise identity. Bidders must state whether integration accelerators exist for these or equivalent categories, and how they would manage message translation, canonical data modelling and operational monitoring across the interface estate.

### 2.3 Security/Compliance Requirements
Security and compliance requirements are central to evaluation. The solution must support operation in a regulated banking environment subject to FCA, PRA, Bank of England expectations, UK operational resilience rules, UK GDPR and cross-border payment controls. Respondents should describe how the platform supports segregation of duties, privileged access management, encryption in transit and at rest, immutable or tamper-evident logs, incident response, vulnerability management and evidence generation for internal audit and regulators. The Bank expects alignment with modern control standards, including defensible software development and third-party risk practices.
...
| ID | Requirement | Priority |
|---|---|---|
| HSBC-SC1 | Support role-based access control, MFA integration and privileged activity logging | Mandatory |
| HSBC-SC2 | Provide immutable or tamper-evident audit logs covering configuration and operational events | Mandatory |
| HSBC-SC3 | Document data residency, retention and deletion controls aligned with applicable privacy laws | Mandatory |
| HSBC-SC4 | Provide security certifications, penetration test summaries and secure SDLC evidence | Scored |
| HSBC-SC5 | Support policy-based export of audit evidence for regulator and internal audit requests | Scored |
...
HSBC UK Bank plc expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. HSBC has one of the clearest public institutional digital asset programs among large banks, including HSBC Orion for digital bond issuance, tokenized gold offerings in some markets, and participation in digital cash and settlement pilots. For an FX or bonds procurement, the bank would expect vendors to understand that HSBC is past the pure-experiment stage and is testing how digital asset flows integrate with a very large global banking estate. In practical terms, that means the proposed platform must support cross-currency settlement, PvP / synchronized settlement, treasury prefunding, nostro optimization and exception handling while fitting into global payments, FX and treasury infrastructure, securities services, custody and settlement tooling, SWIFT, client channels, cloud and cyber controls, and extensive regional operations platforms. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. sponsorship would likely span Markets & Securities Services, innovation, digital assets, treasury, technology, cyber and regional compliance teams. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, HSBC will expect credible global scalability, regional segmentation, data-residency options and disciplined operational-resilience commitments. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by FCA, PRA, Bank of England, plus cross-border obligations; for EEA servicing also MiCA and DORA considerations. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes global payments, FX and treasury infrastructure, securities services, custody and settlement tooling, SWIFT, client channels, cloud and cyber controls, and extensive regional operations platforms. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
