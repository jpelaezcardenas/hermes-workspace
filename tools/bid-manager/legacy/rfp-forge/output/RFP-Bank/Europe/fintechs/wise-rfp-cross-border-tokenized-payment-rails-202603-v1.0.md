...
| Field | Value |
| --- | --- |
| Reference | `WISE-RFP-CROSS-BORDER-TOKENIZED-PAYMENT-RAILS-202603` |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Wise (United Kingdom) |
| Procurement subject | Cross-Border Tokenized Payment Rails |
...
Wise operates as a cross-border payments platform obsessed with transparent pricing, settlement performance, and operational resilience. That operating model shapes this RFP in practical ways: API quality matters more than slideware, operating overhead matters more than broad but shallow feature lists, and support for controlled expansion matters more than jurisdiction-agnostic claims. The selected vendor must fit into an environment where customer experience, automation, and resilience are all expected at the same time.

The initial business case centers on cross-border tokenized payment rails, but the evaluation will also consider whether the proposed architecture creates a reusable foundation for adjacent digital asset services. Vendors should therefore distinguish between the minimum production scope, near-term extension options, and capabilities that require substantial roadmap or partner dependency.
### 1.1 Objectives and success measures

- Deliver a production-ready capability for cross-border tokenized payment rails with measurable readiness for launch within 6-12 months of contract signature.
...
The solution must support the end-to-end business and operational scenarios required for cross-border tokenized payment rails. At minimum, the buyer expects support for customer onboarding hooks, asset setup and governance, transaction or order processing, ledger and entitlement updates, reconciliation, reporting, exception handling, and controlled rollout into new jurisdictions or customer segments.
...
| Core platform capability | Native support for cross-border tokenized payment rails, including lifecycle and control workflows | Mandatory |
...
Evaluation will be performed by a cross-functional team spanning Payments Platform, Treasury, Engineering, Financial Crime, Operations, Product. The buyer may request clarification sessions, architecture walkthroughs, control deep-dives, or reference calls. Scores may be adjusted where written responses are contradicted by demo evidence, undocumented dependencies, or unclear operating assumptions.
...
Wise expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Wise is most plausibly evaluating digital assets through cross-border payments, stablecoins and treasury settlement efficiency rather than capital-markets issuance. The realistic procurement question is whether tokenized payment rails can improve speed, cut trapped liquidity and preserve the transparency and compliance profile Wise already markets heavily. In practical terms, that means the proposed platform must support cross-border settlement, liquidity management, sanctions and screening, ledger synchronization and payment transparency while fitting into cross-border payment engine, treasury and liquidity systems, correspondent banking connections, customer onboarding, sanctions screening, ledgers, analytics and monitoring. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. payments, treasury, infrastructure, compliance and security leaders would shape requirements. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, Wise will likely demand practical evidence on settlement finality, liquidity management, sanctions controls and reconciliation under high-volume cross-border load. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by FCA, EEA payment regulators, MiCA if crypto-asset rails are brought in, DORA and GDPR. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes cross-border payment engine, treasury and liquidity systems, correspondent banking connections, customer onboarding, sanctions screening, ledgers, analytics and monitoring. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
