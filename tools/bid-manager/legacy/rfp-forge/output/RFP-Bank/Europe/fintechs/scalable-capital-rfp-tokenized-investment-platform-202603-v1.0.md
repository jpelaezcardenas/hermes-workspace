...
| Field | Value |
| --- | --- |
| Reference | `SCALABLE-CAPITAL-RFP-TOKENIZED-INVESTMENT-PLATFORM-202603` |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Scalable Capital (Germany) |
| Procurement subject | Tokenized Investment Platform |
...
Scalable Capital operates as a digital wealth and brokerage platform with emphasis on automated investing, modular investment products, and scalable compliance operations. That operating model shapes this RFP in practical ways: API quality matters more than slideware, operating overhead matters more than broad but shallow feature lists, and support for controlled expansion matters more than jurisdiction-agnostic claims. The selected vendor must fit into an environment where customer experience, automation, and resilience are all expected at the same time.

The initial business case centers on tokenized investment platform, but the evaluation will also consider whether the proposed architecture creates a reusable foundation for adjacent digital asset services. Vendors should therefore distinguish between the minimum production scope, near-term extension options, and capabilities that require substantial roadmap or partner dependency.
### 1.1 Objectives and success measures

- Deliver a production-ready capability for tokenized investment platform with measurable readiness for launch within 6-12 months of contract signature.
...
The solution must support the end-to-end business and operational scenarios required for tokenized investment platform. At minimum, the buyer expects support for customer onboarding hooks, asset setup and governance, transaction or order processing, ledger and entitlement updates, reconciliation, reporting, exception handling, and controlled rollout into new jurisdictions or customer segments.
...
| Core platform capability | Native support for tokenized investment platform, including lifecycle and control workflows | Mandatory |
...
Evaluation will be performed by a cross-functional team spanning Wealth Platform, Product, Compliance, Operations, Engineering, Finance. The buyer may request clarification sessions, architecture walkthroughs, control deep-dives, or reference calls. Scores may be adjusted where written responses are contradicted by demo evidence, undocumented dependencies, or unclear operating assumptions.
...
Scalable Capital expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Scalable Capital is closer to digital wealth distribution than wholesale market infrastructure. A tokenized fund distribution procurement would logically focus on order lifecycle, investor reporting, fractional holdings, custody dependencies, cost efficiency and smooth integration into existing brokerage and wealth journeys. In practical terms, that means the proposed platform must support institutional-grade digital-asset issuance, servicing, settlement, reporting and control workflows while fitting into brokerage and wealth-management platform, portfolio systems, client reporting, custody interfaces, market data, CRM, AML and analytics. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. wealth product, brokerage operations, compliance, platform engineering and partner-management leaders would be involved. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, Scalable is likely to value low-operational-friction infrastructure that supports scale without introducing institutional-complexity theater. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by BaFin, MiCA where relevant, DORA, investor-protection and outsourcing rules. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes brokerage and wealth-management platform, portfolio systems, client reporting, custody interfaces, market data, CRM, AML and analytics. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
