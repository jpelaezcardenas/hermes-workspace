...
| Field | Value |
| --- | --- |
| Reference | `KLARNA-RFP-TOKENIZED-LENDING-INFRASTRUCTURE-202603` |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Klarna (Sweden) |
| Procurement subject | Tokenized Lending Infrastructure |
...
Klarna operates as a consumer payments and credit fintech with large merchant distribution, underwriting workflows, and servicing operations. That operating model shapes this RFP in practical ways: API quality matters more than slideware, operating overhead matters more than broad but shallow feature lists, and support for controlled expansion matters more than jurisdiction-agnostic claims. The selected vendor must fit into an environment where customer experience, automation, and resilience are all expected at the same time.

The initial business case centers on tokenized lending infrastructure, but the evaluation will also consider whether the proposed architecture creates a reusable foundation for adjacent digital asset services. Vendors should therefore distinguish between the minimum production scope, near-term extension options, and capabilities that require substantial roadmap or partner dependency.
### 1.1 Objectives and success measures

- Deliver a production-ready capability for tokenized lending infrastructure with measurable readiness for launch within 6-12 months of contract signature.
...
The solution must support the end-to-end business and operational scenarios required for tokenized lending infrastructure. At minimum, the buyer expects support for customer onboarding hooks, asset setup and governance, transaction or order processing, ledger and entitlement updates, reconciliation, reporting, exception handling, and controlled rollout into new jurisdictions or customer segments.
...
| Core platform capability | Native support for tokenized lending infrastructure, including lifecycle and control workflows | Mandatory |
...
Evaluation will be performed by a cross-functional team spanning Lending Platform, Risk, Merchant Solutions, Engineering, Compliance, Finance. The buyer may request clarification sessions, architecture walkthroughs, control deep-dives, or reference calls. Scores may be adjusted where written responses are contradicted by demo evidence, undocumented dependencies, or unclear operating assumptions.
...
Klarna expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Klarna is not primarily known for digital-asset infrastructure, so a realistic procurement around digital credit instruments would be exploratory and heavily risk-filtered. The company would be asking whether tokenization can improve funding, receivables management or programmable credit servicing without compromising consumer-credit governance. In practical terms, that means the proposed platform must support institutional-grade digital-asset issuance, servicing, settlement, reporting and control workflows while fitting into lending and BNPL systems, consumer risk engines, treasury and funding, collections, reporting, CRM, IAM and data platforms. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.
