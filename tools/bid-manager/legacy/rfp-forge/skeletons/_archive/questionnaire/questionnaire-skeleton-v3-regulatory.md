# Questionnaire Skeleton v3 — Regulatory Compliance Assessment

**Document ID:** RB-QS-03-v1.0
**Codename:** REGULATOR
**Category:** Vendor Assessment Questionnaire
**Buyer type:** Compliance, legal, policy, and regulatory affairs teams
**Primary intent:** Structured questionnaire for regulatory control maturity and jurisdictional fit
**Default response model:** Structured vendor response with explicit evidence requests
**Tailoring variables:** jurisdictions, investor categories, reporting regime, asset type, governance model
...
- Write about: regulatory controls, eligibility, transfer restrictions, identity claims, audit evidence, reporting, governance workflows, data handling, and jurisdiction-sensitive operating constraints
...
- Use domain groupings by control objective rather than by regulation alone so the questionnaire remains reusable across regions
- Require a matrix for native platform controls, configurable controls, external dependencies, and manual processes
- Add region-specific annexes where one or more named frameworks must be mapped explicitly
