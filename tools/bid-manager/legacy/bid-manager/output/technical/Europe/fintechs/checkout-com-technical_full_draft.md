...
DALP's transaction pipeline manages 11 explicit states: Initiated, Validated, Compliance Checked, Approved, Signed, Broadcast, Confirmed, Failed, Retried, Dead Letter, and Complete. Each state transition is durable through the Restate execution engine, persisted to PostgreSQL, and emits a structured event. Failed transactions do not disappear; they enter the Failed state and surface in the operations exception queue. Dead-letter transactions require explicit operator decision (retry, abandon, or escalate). Reversal is supported through the transfer reversion workflow for applicable transaction types.
...
graph TB
    ...
    subgraph Middleware["Middleware Layer"]
        restate["Restate: Durable Payment Workflows\nExactly-once semantics, persisted state"]
...
|---|---|---|
| Safeguarding (PSR 2017 Reg. 23) | Supported by configuration | Reserve assets held through configured custody arrangement |
| Transaction monitoring | Supported | DALP event stream; Checkout.com owns AML monitoring logic |
| Audit trail | Supported | Tamper-evident transaction log; configurable retention |
| Outsourcing controls | Supported | SettleMint ISO 27001 and SOC 2 Type II; DORA-aligned ICT documentation |
...
| BR-02 | Supported | 11-state transaction pipeline, Restate durability | Section 6 |
...
**DALP:** Digital Asset Lifecycle Platform. SettleMint's production-grade platform for tokenized asset lifecycle management.
