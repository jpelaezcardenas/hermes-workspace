...
SettleMint builds institutional digital asset lifecycle infrastructure for regulated financial markets and sovereign use cases. Founded in 2017 and headquartered in Antwerp, Belgium, SettleMint operates with teams in Europe, the Middle East, Asia, and North America. The company combines over 200 years of combined banking and blockchain experience.
...
On-chain token balances are authoritative. DALP's blockchain event indexing service continuously projects on-chain state into a PostgreSQL read model with sub-second latency between on-chain confirmation and API availability. N26's mobile banking core reads customer savings note balances through DALP's REST API. Omnibus product accounts managed through DALP's multi-wallet operations. Treasury position visibility through product-level balance aggregation APIs.
...
flowchart TB
    ...
    subgraph DALP_Services["DALP Digital Asset Services Layer"]
        ...
        indexer_svc["blockchain event indexing service\n(Real-time balance projection)"]
    end
...
sequenceDiagram
    ...
    participant Indexer as blockchain event indexing service
...
The modular capability described above depends on DALP's layered architecture -- a design that separates product logic from network execution and compliance enforcement from custody. Section 7 explains how those layers work and why they matter for N26's cloud-native engineering environment.
...
graph TB
    ...
    subgraph MiddlewareLayer["Middleware Layer"]
        ...
        chain_idx["blockchain event indexing service\n(Real-time event projection)"]
...
graph TB
    subgraph N26_K8s["N26 Kubernetes Cluster - Frankfurt (3 AZ)"]
        subgraph App_Pods["Application Pods"]
            ...
            indexer_pods["blockchain event indexing service Pods\n(2 pods, active-passive)"]
        end
...
| BaFin audit trail requirements (BaFin Circular 05/2023) | Supported | Tamper-evident event log, export APIs | 10-year retention; circular references outsourced IT systems |
| BaFin MiCA implementation guidance (BAFIN-Q 25-DIG) | Supported | Compliance modules aligned with 2024 guidance | Consumer eligibility, transfer limits, disclosures |
| BaFin AML/CFT screening (GwG) | Supported with partner | OnchainID claim integration with N26's AML system | N26's AML provider issues OpenID Connect-compatible claims; claim adapter microservice available if provider is not OIDC-native (2-3 days effort) |
...
| BR-03 | Supported | On-chain authoritative balances, blockchain event indexing service projection |
