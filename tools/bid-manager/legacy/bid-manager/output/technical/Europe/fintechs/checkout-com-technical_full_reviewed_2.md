...
OpenAPI 3.1 specification generated directly from procedure definitions, so documentation cannot drift from implementation. TypeScript SDK with typed client factory and automatic serialization of blockchain value types. The CLI with 301 commands across 26 groups is the primary interface for operations teams running batch jobs and automation scripts; the API serves Checkout.com's backend integrations; and the Asset Console provides the GUI for compliance and configuration teams. 534 structured error codes with metadata and SDK mirror. API versioning through path versioning (v1, v2 in current release). Deprecation policy: 12 months notice before removing deprecated endpoints.
...
Every merchant participating in digital asset settlement receives a managed wallet through DALP's Key Guardian system. Wallets are created programmatically through the DALP API, custody provider-assigned, and linked to the merchant's OnchainID for compliance verification.

Batch merchant onboarding: the batch identity registration API processes up to 100 merchants per call. For Checkout.com's merchant migration programme, this enables efficient onboarding of existing merchants without per-merchant manual operations, a single API call enrolls a merchant cohort and issues the eligibility claims that activate their access to tokenized settlement.

Multi-wallet operations: merchants can hold multiple wallets for different purposes (settlement receipt, treasury holding, reserve management). Each wallet operates under the same compliance rules applied to the merchant's OnchainID, regardless of which wallet receives the payment leg.
...
graph TB
    subgraph Application["Application Layer"]
        ...
        cli["DALP CLI\n301 commands, 26 groups\nOps automation & batch scripting"]
    end
