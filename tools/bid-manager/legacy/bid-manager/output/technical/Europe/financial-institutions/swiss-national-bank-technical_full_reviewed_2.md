...
The third principle is immutability. The SNB's audit obligations under the National Bank Act require a permanent, unalterable record of every wCBDC issuance, transfer, and redemption. DALP's permissioned blockchain provides an immutable, unalterable record of every wCBDC transaction: once a block is finalised under IBFT 2.0, its contents cannot be modified by any party, including SettleMint or SNB administrators. No SettleMint staff member, no infrastructure provider, and no SNB administrator can modify the historical record of wCBDC transactions.
...
| TR-019 | SIC integration for cash leg | Comply | API integration with SIC payment infrastructure; payment initiation on ADMITTED state; confirmation webhook triggers FINAL; integration protocol (SWIFT ISO 20022 or SIC-proprietary) confirmed during Discovery based on SNB's current SIC interface specification |
...
| TR-028 | BIS Innovation Hub observer access | Comply | REGULATOR_ROLE read-only access for BIS observers; zero write capability; real-time Chain Indexer access; SNB controls scope of data visible to BIS observers, consistent with Swiss bank secrecy obligations; individual participant data can be excluded from observer view at SNB's discretion |
