...
**PRISMA integration boundary:** PRISMA calculates margin requirements. DALP enforces margin collateral eligibility at the smart contract layer. The two systems communicate via DALP's REST API using HTTP POST for margin call instructions and webhook callbacks for confirmation events. PRISMA posts margin call instructions as REST API calls; DALP verifies on-chain that eligible collateral has been received and returns confirmation via webhook. The margin calculation model and all EMIR risk model governance remain entirely within PRISMA; DALP enforces the collateral eligibility rules configured in the compliance modules.
...
3. **Dress rehearsal (Phase 4, Week 24-25):** Full end-to-end default management simulation (1 day) including PRISMA notification, account freeze, position export, and bridge transfer under time pressure. Simulation is timed against EMIR default management deadlines. Dress rehearsal completion is a go-live prerequisite.
...
| TR-003 | P1 | Initial margin, variation margin, intraday margin with deterministic controls | Full | PRISMA is the margin calculation authority and EMIR risk model governance authority. DALP enforces margin collateral eligibility at the contract layer per PRISMA-derived parameters. Margin calculation models remain entirely within PRISMA. DALP does not perform margin calculation. |
