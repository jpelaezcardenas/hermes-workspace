...
**Netting model:** Bilateral netting within a settlement cycle reduces gross settlement volume. DALP's execution engine manages batched XvP settlement for netted positions. Each net position is settled atomically; the XvP addon handles the net delivery instruction.
...
Cross-CSD settlement uses a bridge protocol that coordinates atomic lock-release across the two Besu networks. The bridge is a Phase 2+ feature; initial deployment covers Euroclear Bank ICSD settlement only. The extension path to other Euroclear entities is defined but not in initial scope.
...
graph LR
    ...
    REST_API <-->|Cash leg confirmations| TARGET2
...
| Reconciliation with legacy book-entry | | Responsible | Integration: SettleMint |
...
- Normal processing: standard DvP, bilateral netting, EOD settlement
...
| ID | Status | Notes |
|---|---|---|
| REG-001 | Full | CSDR, DORA, MiCA, GDPR, AMLD, NIS2, EMIR interfaces: all mapped |
