...
## 14a. Target Operating Model (BAU: Commodity Finance)

### 14a.1 Operational Ownership

**ANZ Trade Finance Operations Team owns:**
- Daily LC documentary review queue management: reviewing document presentations, confirming documentary condition satisfaction, authorizing LC settlement
- Settlement queue monitoring: pending XvP settlements, HTLC expiry tracking, counterparty confirmation status
- A$DC reserve reconciliation: confirming on-chain A$DC balances match TMS reserve records
- AUSTRAC reporting: daily threshold transaction reporting, suspicious matter alert management
- Counterparty onboarding coordination: identity registration for new commodity producers, traders, and buyers

**ANZ Compliance and Financial Crime owns:**
- AML/CTF alert adjudication: reviewing transaction monitoring alerts, approving or escalating suspicious matters
- DFAT/OFAC sanctions screening: managing country blocklist updates, enhanced due diligence approvals
- AUSTRAC regulatory reporting sign-off

**ANZ Treasury Operations owns:**
- A$DC reserve management: maintaining adequate A$DC reserves for settlement obligations
- Cross-currency settlement coordination: FX rate confirmation for USD/AUD cross-chain settlements

**ANZ Technology owns:**
- Platform health monitoring (shared with SettleMint Enterprise Support)
- NPP connectivity monitoring
- Commodity price feed health monitoring
...
### 14a.2 Daily Commodity Finance Procedures

**Start of Day (07:00-08:00 AEST):**
1. Confirm A$DC reserve adequacy: on-chain A$DC balance versus TMS reserve record
2. Review LC documentary queue: new document presentations overnight requiring trade desk review
3. Check settlement queue: pending XvP settlements, HTLC expiry within 24 hours requiring counterparty action
4. Confirm commodity price feed freshness: staleness alerts for margin-linked instruments
5. Review AML/CTF alerts from overnight activity: screening matches requiring adjudication
6. Confirm AUSTRAC reporting schedule: any TTR or IFTI submissions due

**Intraday Operations:**
- LC document presentation review within 2 business hours of presentation
- Settlement confirmation within 1 business hour of HTLC counterparty confirmation
- Margin call processing within 4 business hours of price move triggering threshold
- AML/CTF freeze execution within 15 minutes of compliance team authorization

**End of Day (17:00-18:00 AEST):**
- A$DC reserve reconciliation: confirm on-chain balance matches TMS close
- AUSTRAC daily reporting pack: export and submit required reports
- Open exception review: unresolved documentary conditions, pending counterparty confirmations, open AML cases
- Settlement position close: confirm all intraday settlements reached final state

### 14a.3 A$DC Programme Operational Model

ANZ's A$DC stablecoin programme is the cash leg for commodity finance settlement. The operational model for A$DC within the DALP platform:

**A$DC Issuance (by ANZ Treasury):** ANZ Treasury mints A$DC against AUD reserve backing. The Supply Management role for the A$DC deposit token is held by ANZ Treasury. Each A$DC minting event is recorded on-chain and reconciled against the AUD reserve in TMS.

**Reserve Monitoring:** DALP's collateral requirement module monitors the A$DC collateral ratio continuously. If the ratio falls below the configured minimum (100% AUD backing), the compliance module blocks new A$DC issuance until reserve adequacy is restored. Operations team is alerted via monitoring dashboard.

**Settlement Reserve Pre-Funding:** For large LC settlements, ANZ Treasury pre-funds the settlement trust account with A$DC before the settlement window opens. This ensures settlement liquidity is available for the atomic DvP execution without requiring real-time Treasury intervention.

**Post-Settlement Reserve Release:** After LC settlement (A$DC transferred to exporter), the reserve corresponding to the settled amount is released. The exporter can then redeem A$DC for AUD through ANZ's exchange mechanism, or hold A$DC for subsequent commodity transactions.

### 14a.4 AUSTRAC Reporting Workflow

DALP provides the transaction data layer for ANZ's AUSTRAC reporting obligations:

**Threshold Transaction Reports (TTR):** For commodity finance transactions exceeding AUD 10,000 in cash equivalent, DALP's event export API provides the transaction data. ANZ's AML/CTF platform consumes DALP event webhooks and automatically generates TTR submissions where applicable.

**International Funds Transfer Instructions (IFTI):** For cross-border commodity finance settlements (AUD vs. foreign currency stablecoins), DALP's event log captures the full transaction details including amount, currency, sender and receiver identifiers, and settlement timestamp. The IFTI reporting pipeline maps these details to AUSTRAC's submission format.

**Suspicious Matter Reports (SMR):** When ANZ's transaction monitoring platform identifies a suspicious matter from DALP event data, the compliance team uses DALP's freeze API to restrict the relevant wallet before filing the SMR. The on-chain freeze event provides the evidence of immediate action taken by ANZ consistent with AML/CTF Act obligations.

---

## 14b. Scenario Narratives for Commodity Finance

### Scenario 1: Standard LC Settlement on Document Presentation

ANZ has issued a tokenized letter of credit for an iron ore shipment from a Pilbara producer to a Japanese steel mill. The LC is denominated in A$DC, with standard documentary conditions: commercial invoice, bill of lading from a recognized carrier, inspection certificate from SGS, and marine insurance certificate.

The vessel departs Port Hedland. The carrier's agent uploads the bill of lading to the trade finance platform, which delivers the document event to DALP via API. DALP's documentary condition module records the BL submission. The trade desk reviews the BL within 2 business hours, confirming it is a compliant set of documents. The trade desk approval is recorded on-chain.

ANZ's trade operations team receives the full document set from the exporter's presenting bank: commercial invoice, inspection certificate, and insurance certificate. Each document presentation is delivered to DALP via API, updating the settlement condition status. When all four documentary conditions are satisfied, the settlement condition module marks the LC as settlement-eligible.

ANZ's settlement operation executes the A$DC DvP: the exporter's wallet receives the A$DC settlement amount while the LC token is simultaneously retired. The atomic execution means the exporter cannot receive A$DC without the LC being retired, and the LC cannot be retired without the A$DC payment. The settlement event generates webhooks to the core banking system (GL posting for the LC payment liability) and TMS (A$DC reserve release).

Total elapsed time from document presentation to settlement: under 4 business hours, versus the 3-7 days typical for traditional LC settlement.

### Scenario 2: Cross-Border XvP: AUD/USD Commodity Settlement

A Singapore commodity trader purchases iron ore from an Australian producer, with payment in USD stablecoins against delivery of the eBL token. ANZ facilitates the settlement as the LC issuing bank.

ANZ creates an XvP settlement in DALP:
- Leg 1: Australian producer delivers eBL token to Singapore trader (on the Australian EVM network)
- Leg 2: Singapore trader delivers USD stablecoin to ANZ (on the Singapore EVM network, via Project Guardian-connected network)

The HTLC hashlock is set by ANZ and shared with the Singapore trader's bank. When the Singapore trader confirms Leg 2 (USD stablecoin delivery), ANZ reveals the pre-image secret to claim the eBL on the Australian network. The Singapore trader simultaneously receives the eBL token. Both legs settle atomically within seconds.

All compliance checks run independently on each token: the eBL transfer validates that the Singapore trader has verified counterparty identity and is in an approved trading jurisdiction. The USD stablecoin transfer validates that ANZ is an eligible recipient on the Singapore network.

The settlement event on the Australian network generates an IFTI-required webhook to ANZ's AML/CTF platform (international funds transfer instruction, cross-border settlement). ANZ's AUSTRAC reporting pipeline processes the event automatically.

### Scenario 3: Commodity Price Margin Call Trigger

ANZ has financed AUD 50 million of copper inventory held in a bonded warehouse in Melbourne, against on-chain warehouse receipt tokens. The collateral ratio is configured at 130% (AUD 65 million collateral value required for AUD 50 million financing). The copper price is linked to LME copper via DALP's Feeds System.

LME copper price falls 10% over a trading session. DALP's Feeds System updates the copper price on-chain. The Collateral Ratio compliance module recalculates: at the new price, the collateral value has fallen to AUD 58.5 million, giving a coverage ratio of 117%, below the 130% minimum.

The compliance module immediately blocks any additional financing against the copper warehouse receipt tokens and generates an alert to the operations dashboard. ANZ's trade finance team sees the alert and contacts the borrower: additional margin (AUD 6.5 million in A$DC or approved collateral) must be posted within 4 business hours to restore the ratio to 130%.

The borrower posts additional A$DC collateral via DALP's collateral deposit mechanism. The Collateral Ratio module recalculates: 135% coverage, above the minimum. The financing block is automatically lifted when the ratio is confirmed as compliant.

The entire margin call event, price feed update, ratio calculation, financing block, alert, collateral posting, ratio restoration, is recorded on-chain with timestamps and operator identity. The complete event chain is available for audit review through DALP's events API.
