...
## Operational Governance Cadences

DALP supports structured governance routines that map to OPay's operational rhythm:

**Daily.** Exception review: settlement operations that failed compliance evaluation are surfaced in the platform's exception queue with reason codes and counterparty details. Treasury operators review and resolve or escalate within the same business day. Reconciliation sign-off: the previous day's settlement records are matched against fiat books, and the reconciliation status is confirmed before the next settlement window opens.

**Weekly.** Threshold monitoring: settlement volume trends, position concentrations, and compliance module trigger rates are reviewed by the operations team using DALP's pre-built Grafana dashboards. Incident review: any P1 or P2 incidents from the previous week are reviewed with root cause analysis and remediation confirmation.

**Monthly.** Entitlement recertification: role assignments across all settlement counterparties are reviewed by compliance and operations leads. Any role changes are processed through the governance approval workflow with full audit trail. Management reporting: settlement volume summaries, exception rates, SLA compliance metrics, and position analytics are compiled from DALP's reporting extracts and presented to OPay's management committee.

**Quarterly.** DR testing: full disaster recovery exercise with documented results and gap remediation. Architecture review: deployment topology, performance metrics, and capacity planning reviewed with SettleMint's solution architecture team.

These routines are supported natively through DALP's reporting extracts, monitoring dashboards, and audit log exports. Where OPay requires integration with existing governance workflows (such as committee papers or risk reports), DALP provides structured data exports that feed into those processes.
...
**Blockchain state recovery.** If a Besu validator node's state diverges from the network (for example, due to storage corruption or extended disconnection), the node resynchronizes by performing a snapshot sync from healthy validators. The IBFT 2.0/QBFT consensus protocol ensures that the remaining validators maintain network liveness during the recovery period. A minimum of three healthy validators is required to maintain consensus; the recommended deployment uses four or five validators to tolerate single-node failures without degradation.
...
DALP deployment for OPay assumes a greenfield approach: settlement tokens are created fresh on the blockchain network, and historical settlement records remain in OPay's existing systems. There is no migration of historical settlement data onto the blockchain.

However, Phase 1 includes a mandatory parallel run period (minimum 2 weeks post go-live) during which both the existing batch settlement process and the new tokenized settlement process operate simultaneously for the same banking partner relationships. Reconciliation between the two settlement paths validates accuracy before the existing process is decommissioned for migrated counterparties.

Reference data (counterparty identities, settlement parameters, compliance rules) is loaded into DALP during the WS-02 configuration phase of each implementation phase. This data is sourced from OPay's existing systems and validated during UAT before production use.

If OPay requires historical settlement records to be queryable through DALP's interface (for audit or regulatory purposes), a reference data import can be scoped as additional work. This would involve loading historical settlement summaries as reference records, not as on-chain transactions.

## Post-Go-Live Change Management

After each phase reaches production, changes to the settlement configuration follow a controlled process:

**Release management.** SettleMint publishes platform releases on a regular cadence with detailed release notes covering new capabilities, bug fixes, security patches, and breaking changes. OPay's operations team reviews release notes and schedules upgrade windows through the change control process.

**Configuration changes.** Changes to compliance rules, settlement parameters, or role assignments follow the maker-checker governance workflow. All changes are recorded on-chain with the identity of the initiator and approver.

**Regression testing.** Before any platform upgrade is promoted to production, it passes through the full environment pipeline (dev, test, UAT) with automated regression tests covering core settlement flows. Test results are documented and available for OPay's review.

**Rollback.** If a platform upgrade introduces issues in production, DALP's Kubernetes-based deployment supports rapid rollback to the previous version. Database schema changes include reverse migration scripts. Blockchain state changes are inherently immutable, but operational parameters can be reconfigured through governance workflows.

**Approval board.** OPay's internal change advisory board (or equivalent) reviews and approves all changes that affect the production settlement environment. SettleMint provides impact assessments and testing evidence to support the approval decision.
...
DALP relies on the following third-party components. All are open-source or commercially licensed with no single-vendor lock-in risk.

| Component | Role | License | Version | Support Responsibility | Substitution Options |
|---|---|---|---|---|---|
| Hyperledger Besu | EVM blockchain network | Apache 2.0 | 24.x | SettleMint (platform layer) | Go-Ethereum (Geth) for private PoA |
| PostgreSQL | Application database | PostgreSQL License (MIT-like) | 16.x | OPay (infrastructure) or cloud provider | Any PostgreSQL-compatible managed service |
| Restate | Durable workflow execution engine | BSL 1.1 | 1.x | SettleMint (platform layer) | Temporal (if license preference requires) |
| Grafana | Monitoring dashboards | AGPL 3.0 | 11.x | OPay (infrastructure) or SettleMint (managed) | Datadog, New Relic, or OPay's existing observability |
| Prometheus | Metrics collection | Apache 2.0 | 2.x | OPay (infrastructure) | Victoria Metrics, Thanos |
| Better Auth | Identity management | MIT | Latest | SettleMint (platform layer) | Enterprise SSO provider (Okta, Azure AD) for authentication |
| Kubernetes | Container orchestration | Apache 2.0 | 1.29+ | OPay (infrastructure) or cloud provider | Any CNCF-conformant Kubernetes distribution |

SettleMint manages all platform-layer dependencies. OPay manages infrastructure-layer dependencies (or delegates to a cloud provider). No component requires OPay to contract directly with the third-party vendor.
...
**Deployment 1: Gulf Region Sovereign Entity.** Tokenized bond issuance and lifecycle management platform for a sovereign wealth entity. Permissioned Hyperledger Besu network with multi-custody integration (Fireblocks and DFNS). Production since 2025. Supports multiple asset classes with on-chain compliance enforcement across three jurisdictions. Processes approximately 5,000 settlement transactions per day across multiple asset types.
