...
**Performance baseline:** Under a representative test profile of 200 concurrent settlement instructions on a 4-validator Besu network (Azure Standard_D4s_v3, UAE North region), the platform achieved median transaction confirmation latency of 2.1 seconds with P99 at 4.8 seconds. Peak throughput under sustained load reached 180 settlement-grade transactions per second. These figures reflect the instrument complexity profile appropriate for a digital securities marketplace (not high-frequency trading). Performance is configurable: block period, gas limit, and network topology parameters are tuned during Phase 2 based on ADX's confirmed transaction volume targets.
...
## 8.4a UAE AML/CFT Law Mapping

UAE Federal Decree-Law No. 20 of 2018 on Anti-Money Laundering and Combating the Financing of Terrorism (AML/CFT law) establishes screening, transaction monitoring, and suspicious activity reporting obligations that apply to digital asset activities under SCA and ADGM FSRA frameworks. DALP's compliance integration model addresses these requirements as follows:

- **Screening obligation:** DALP's AML integration module triggers a real-time screening call to the configured AML provider at investor onboarding and at each material state change (new wallet address, jurisdiction change). Screening results are stored with the investor record and the timestamp of the last check.
- **Transaction monitoring:** The webhook event stream delivers all transfer events in structured format to ADX's transaction monitoring system or AML provider's monitoring engine. Events include: transfer party identities, amounts, timestamps, instrument type, and compliance check outcomes.
- **Suspicious activity reporting:** DALP does not replace the regulated entity's SAR obligations; it provides the structured data and audit log required to support the SAR process efficiently. Alert management hooks notify ADX compliance officers of screening alerts requiring review.
- **Beneficial ownership:** Record-date snapshots and beneficial owner position data are exportable in formats suitable for AML monitoring and regulatory reporting under UAE AML/CFT law Article 12 obligations.
...
**ADX effort by phase (estimated person-days):**

| Phase | Programme Director | Technology Team | Compliance Team | Operations Team | Total ADX |
|-------|-------------------|----------------|----------------|----------------|-----------|
| Phase 1. Discovery | 5 | 3 | 3 | 0 | 11 |
| Phase 2. Foundation | 3 | 10 | 1 | 0 | 14 |
| Phase 3. Configuration | 3 | 5 | 8 | 2 | 18 |
| Phase 4. Integration | 3 | 15 | 3 | 2 | 23 |
| Phase 5. Go-Live | 5 | 8 | 5 | 10 | 28 |
| Phase 6. Hypercare | 2 | 5 | 2 | 8 | 17 |
| **Total** | **21** | **46** | **22** | **22** | **111** |

**VARA applicability note:** Where specific digital securities instruments issued through the ADX marketplace fall within the VARA (Virtual Asset Regulatory Authority) perimeter, for example, instruments that may be characterized as virtual assets under the Abu Dhabi Emirate's regulatory framework. DALP's compliance module configuration can be extended to enforce VARA-specific transfer restrictions, investor classification requirements, and documentation obligations. The compliance module framework supports runtime addition of jurisdiction-specific rules without contract redevelopment. ADX's legal and compliance teams would determine the applicable classification; SettleMint provides the technical configuration capability to implement the resulting requirements.
...
**Reconciliation break SLA:** Breaks identified in the daily reconciliation run are surfaced in the Operations Dashboard within 1 business hour of the reconciliation completing. Operations teams are expected to review and classify each break within 1 business day: data quality issue (correction and resubmit), timing issue (pending confirmation), or genuine discrepancy requiring escalation to Compliance. Genuine discrepancies are escalated and resolved within 3 business days with mandatory documentation.
