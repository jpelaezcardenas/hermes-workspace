...
## 8.2 Morocco Regulatory Compliance: AML/CFT and Data Protection

**AML/CFT:** Bank Al-Maghrib Circular No. 14/W/17 establishes AML/CFT obligations for financial institutions in Morocco, including customer due diligence, transaction monitoring, suspicious activity reporting, and correspondent banking controls. DALP's compliance integration addresses these obligations through: real-time screening at investor onboarding, transfer event monitoring with structured output to CSE's transaction monitoring system, alert management hooks for suspicious activity triage, and complete audit log export for regulatory reporting. For Pan-African investors, FATF-aligned screening providers (Accuity, Refinitiv, or equivalent) are configured to cover the relevant African jurisdiction risk profiles.

**OHADA cross-border framework:** For institutional investors from OHADA member states (17 West and Central African countries), the Uniform Act on Commercial Companies and Economic Interest Groups provides the corporate law framework governing investor entity eligibility. DALP's OnchainID claim system supports jurisdiction-specific entity classification attributes, enabling CSE compliance officers to configure per-corridor eligibility rules for OHADA-jurisdiction institutional investors without creating separate onboarding workflows.
