...
## Appendix P: Additional Architecture Diagrams (Review 1 Additions)

### P.1 Security Model Diagram
```mermaid
flowchart TD
    subgraph SecurityPerimeter["ICBC Security Perimeter"]
        HSM["HSM Cluster\nThales Luna\nFIPS 140-2 L3"]
        KG["Key Guardian\nDual-Control Policy"]
        AUTH["Step-Up Auth\nWebAuthn FIDO2\nMaker + Checker"]
        VAULT_SC["Vault Smart Contract\nOn-chain Segregation"]
    end
    ...
    subgraph AuditLayer["Audit Layer"]
        CHAIN_AUDIT["On-Chain Events\nImmutable Record"]
        IDX_AUDIT["Chain Indexer\nStructured Export"]
        HSM_LOG["HSM Signing Log\nKey-Level Audit"]
    end
    extThreat["External Attack"] -.->|Blocked: No Public Endpoint| VAULT_SC
    insiderThreat["Insider Threat"] -.->|Blocked: Maker-Checker| AUTH
    HSM --> KG
    KG --> AUTH
    AUTH --> VAULT_SC
    VAULT_SC --> CHAIN_AUDIT
    RBAC --> AccessControl
    ABAC --> AccessControl
    CHAIN_AUDIT --> IDX_AUDIT
```

### P.2 Regulatory Reporting Diagram
```mermaid
flowchart LR
    SC["Smart Contract\nCustody Events"] --> IDX["Chain Indexer\nEvent Processing"]
    IDX --> PBOC_FEED["PBOC Supervisory\nFeed: Daily Aggregate"]
    IDX --> CSRC_RPT["CSRC Sandbox\nDaily Report"]
    IDX --> AUDIT_API["Audit Export API\nFull Trail: ICBC Auditors"]
    IDX --> CBS_HOOK["CBS Webhook\nReal-time Position Update"]
    PBOC_FEED --> PBOC["PBOC Platform"]
    CSRC_RPT --> CSRC["CSRC Sandbox Monitor"]
    AUDIT_API --> INT_AUDIT["ICBC Internal Audit"]
    CBS_HOOK --> CBS["Core Banking System"]
```

### P.3 Integration Architecture Diagram
```mermaid
flowchart LR
    subgraph External["External / ICBC Internal Systems"]
        AML_SYS["AML Screening\nActimize / NICE"]
        CBS_SYS["Core Banking System"]
        CMS_SYS["Custody Management System"]
        AD["Active Directory\nLDAP"]
    end
    ...
    AML_SYS <-->|Pre-Custody Screen| GW
    CBS_SYS <-->|Position Webhooks| IDX
    CMS_SYS <-->|Instruction Import / Position Feed| GW
    AD -->|LDAP Auth| GW
    GW --> EE
    EE --> KG
    KG --> BESU
...
*Review 1 additions: 3 additional mermaid diagrams (security model, regulatory reporting, integration architecture).*
*ICBC Technical Proposal. Reviewed Version 1. Score: 83/100*
