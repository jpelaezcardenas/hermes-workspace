...
**Infrastructure scaling note.** If settlement volumes require infrastructure scaling beyond the estimated range (for example, adding Besu validator nodes, scaling database capacity, or expanding monitoring retention), those costs are infrastructure-layer expenses managed by OPay. DALP platform licensing does not change with volume. SettleMint provides capacity planning guidance as part of the quarterly architecture review included in Enterprise support.
...
Implementation fees exclude: OPay's internal team effort, infrastructure provisioning (OPay-managed), third-party system changes required for integration, and regulatory engagement (OPay-led with SettleMint technical support).

**OPay internal team requirements by phase:**

| Role | Phase 1 | Phase 2 | Phase 3 |
|---|---|---|---|
| Business Analyst (settlement operations) | 0.5 FTE | 0.5 FTE | 0.25 FTE |
| Integration Developer (API and middleware) | 1.0 FTE | 1.0 FTE | 0.5 FTE |
| Testing Coordinator | 0.5 FTE | 0.5 FTE | 0.5 FTE |
| Compliance Officer (CBN and AML liaison) | 0.25 FTE | 0.25 FTE | 0.25 FTE |
| Infrastructure Engineer (Kubernetes and networking) | 0.5 FTE | 0.25 FTE | 0.25 FTE |
| **Total** | **2.75 FTE** | **2.5 FTE** | **1.75 FTE** |

## Partner Bank Cost Sharing

Phase 3 (Partner Bank Connectivity) involves integration with partner banks' reconciliation systems. The DALP platform license covers OPay's settlement operations. Partner banks do not require separate DALP licenses, as they interact with the settlement layer through API interfaces exposed by OPay's DALP deployment.

If a partner bank wishes to deploy its own DALP instance for bilateral settlement (creating a direct node-to-node settlement model), that would require a separate commercial engagement between SettleMint and the partner bank. This is an optional enhancement, not a prerequisite for Phase 3 delivery.
