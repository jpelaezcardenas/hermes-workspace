...
DALP's stablecoin asset class provides the foundation for representing fiat-pegged digital currencies. Each of the six corridor currencies (USD, AED, SAR, INR, PKR, BDT) would be configured as a separate stablecoin token, each with its own minting authority, burning rules, and compliance module set.
...
DALP provides atomic settlement through its XvP (Exchange-versus-Payment) settlement engine. Settlement is all-or-nothing: when a remittance settlement executes, the source currency debit and destination currency credit either both complete or both revert. There is no intermediate state where one leg has settled and the other has not.
...
DALP enforces compliance ex-ante, meaning every transaction is validated against eligibility rules, identity claims, and jurisdictional constraints before execution. This is not a post-trade compliance check; non-compliant transactions are prevented from creating immutable on-chain evidence of violations.
...
DALP provides comprehensive audit trail data and transaction history that forms the basis for regulatory reports. The API surface exposes all settlement records, participant activity, compliance decisions, and identity verification events in structured, queryable formats. The platform's indexer maintains 18+ analytics views providing pre-aggregated reporting data.
...
DALP provides a comprehensive API surface through its Durable API Service (DAPI):
...
For RemitLink, the monitoring stack would be accessible to designated Consortium operators, providing real-time visibility into settlement volumes, compliance module activity, participant balances, and system health.
