...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # ── SaaS patch: slash-command dispatch ─────────────────────────────
        # When the user message is a known slash command from COMMAND_REGISTRY,
        # route it through the gateway's full _handle_message (the same path
        # Telegram/Discord/Slack use) so /status, /sessions, /retry, /undo,
        # /agents, /fast, /yolo, /help, etc. work via the OpenAI API. Without
        # this, the LLM just hallucinates plausible-looking responses.
        # See /root/hermes-workspace/patches/README.md for context.
        if isinstance(user_message, str) and user_message.lstrip().startswith("/") and self._message_handler:
            _cmd_text_full = user_message.lstrip()
            _first_word = _cmd_text_full.split(maxsplit=1)[0]
            _cmd_name = _first_word[1:].split("@", 1)[0].lower()
            if _cmd_name and "/" not in _cmd_name:
                _cmd_def = None
                try:
                    from hermes_cli.commands import resolve_command as _resolve_command
                    _cmd_def = _resolve_command(_cmd_name)
                except Exception as _exc:
                    logger.debug("Slash patch: resolve_command(%s) failed: %s", _cmd_name, _exc)
                if _cmd_def is not None and not getattr(_cmd_def, "cli_only", False):
                    try:
                        from gateway.platforms.base import MessageEvent as _MsgEvent, MessageType as _MsgType
                        from gateway.session import SessionSource as _SrcType
                        _chat_id = session_id or "api_server"
                        _evt = _MsgEvent(
                            text=_cmd_text_full,
                            message_type=_MsgType.COMMAND,
                            source=_SrcType(
                            ...
                            internal=True,  # bypass auth — already auth'd via API key
                        )
                        _result = await self._message_handler(_evt)
                        _content = _result if isinstance(_result, str) else (getattr(_result, "text", "") or "")
                        if _content:
                            _resp = {
                        ...
                        logger.warning(
                            "Slash patch: dispatch failed for /%s (%s) — falling through to LLM",
                            _cmd_name, _exc,
                        )
