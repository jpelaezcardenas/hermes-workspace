#!/bin/bash
# =============================================================================
# mythos-v2.sh — Agentic orchestrator (Opus 4.7)
#
#   Phase 0: Language detection
#   Phase 1: Sink-guided slicing
#   Phase 2: File ranking (sink density × language heuristic)
#   Phase 3: Parallel agentic hunter per top file
#            (each hunter has Read/Grep/Glob tool access in target cwd)
#   Phase 4: Validation pass (skeptical reviewer, also with tool access)
#   Phase 5: Aggregate + report
#
# Usage:
#   mythos-v2.sh <target-dir> [--max-files N] [--budget USD] [--language X]
# =============================================================================
...
SCAN_ID="mythos2_$(date +%Y%m%d_%H%M%S)_$$"
...
if [ -z "$TARGET_DIR" ]; then
    ...
    echo "Usage: mythos-v2.sh <target-dir> [--max-files N] [--budget USD] [--language X]" >&2
    exit 1
fi
...
# vuln-yield, hard to reach by accident.
...
while IFS= read -r file; do
    [ -z "$file" ] && continue
    INDEX=$((INDEX + 1))
    log "  launch $INDEX/$SELECTED : ${file#$TARGET_DIR/}"
    jlog "hunt_start" "file=${file#$TARGET_DIR/}"
    hunt_file "$file" &
    PIDS+=($!)
done <<< "$TOP_FILES"
...
for pid in "${PIDS[@]}"; do
    wait "$pid" || true
    DONE=$((DONE + 1))
    log "  progress $DONE/$SELECTED hunters complete"
done
