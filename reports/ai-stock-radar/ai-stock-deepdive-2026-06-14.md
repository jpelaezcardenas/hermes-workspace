# AI Stock Radar Weekly Calibration - 2026-06-14

## Kurzfazit
- Weekly calibration reviews false positives, stale candidates, and quality-rule effects.
- Watchlist candidates reviewed: 30
- This report does not modify the watchlist automatically.

## Grade Summary
- S: 0
- A: 0
- B: 2
- C: 1
- X: 27

## Firewall Summary
- pass: 1
- caution: 13
- reject: 16

## CEO Control Summary
- focus: 0
- monitor: 1
- manual_review: 2
- reject: 27

## False Positive Memory
- name-only-ai-watch: 21
- delisting-watch: 16
- dilution-trend-watch: 12
- cash-runway-watch: 9
- offering-watch: 9
- revenue-decline-watch: 8
- cash-burn-watch: 1
- clinical-risk: 1

## Shadow Backtest Summary
- total_snapshots: 330
- assessed: 0
- pending: 330
- unavailable: 0
- constructive: 0
- risk_confirmed: 0
- inconclusive: 0

## Paper Portfolio Summary
- total_positions: 0
- open_positions: 0
- PAPER_ENTRY_REVIEW: 0
- PAPER_EXIT_REVIEW: 0
- THESIS_INTACT: 0
- EXIT_RISK_REVIEW: 0

## Advanced Signal Summary
- BANGER_CANDIDATE_REVIEW: 0
- EARLY_BUT_THIN: 0
- WAIT: 1
- RISK_TRAP: 29
- review_queue: none
- relative_strength_unavailable: 30
- ownership_unavailable: 30
- liquidity_unavailable: 30

## Thesis Intelligence Summary
- THESIS_CONFIRMED_REVIEW: 0
- WATCH_THESIS: 1
- WEAK_THESIS: 0
- BROKEN_THESIS: 29
- critical_negative_catalysts: 29
- serious_negative_catalysts: 0
- verified_ai_revenue: 0
- reality_risk: 21
- top_gaps: ownership context unavailable (30), price volume context unavailable (30), relative strength unavailable (30), source confidence has missing data (30), AI revenue proof missing (24), AI revenue split not verified (6), fundamentals unavailable (1)

## Alpha Memory Summary
- TRACK_HYPOTHESIS: 0
- WATCH_ONLY: 0
- CONTRADICTION_REVIEW: 1
- RISK_PATTERN: 29
- critical_contradictions: 29
- serious_contradictions: 0
- top_contradiction_labels: broken_thesis (29), critical_negative_catalyst (29), advanced_signal_risk_trap (29), ai_story_without_revenue_proof (24)
- recurring_risk_patterns: name_only_ai_watch (21), delisting_watch (16), dilution_trend_watch (12), cash_runway_watch (9), offering_watch (9)
- fresh_catalyst: 0
- late_or_risk: 29
- constructive_assessments: 0
- risk_confirmed_assessments: 0
- false_positive_assessments: 0

## Keep Review
- GMEX (GMEX ROBOTICS CORPORATION): Grade B, Early Watch, Score 65, Datenqualitaet A, Status fresh. Notes: no quality notes
- NBIS (Nebius Group N.V.): Grade B, Early Watch, Score 69, Datenqualitaet A, Status fresh. Notes: foreign issuer; limited us gaap facts
- DUKR (DUKE Robotics Corp.): Grade C, Early Watch, Score 63, Datenqualitaet A, Status fresh. Notes: cash runway watch

## Downgrade Review
- AMCI (AMC Robotics Corporation): Grade X, Avoid, Score 47, Datenqualitaet A, Status fresh. Notes: delisting watch
- RXRX (Recursion Pharmaceuticals, Inc.): Grade X, Avoid, Score 44, Datenqualitaet A, Status fresh. Notes: clinical risk; cash burn watch; offering watch; dilution trend watch; revenue decline watch
- RR (Richtech Robotics Inc.): Grade X, Avoid, Score 35, Datenqualitaet A, Status fresh. Notes: delisting watch; dilution trend watch
- ARBE (Arbe Robotics Ltd.): Grade X, Avoid, Score 31, Datenqualitaet A, Status fresh. Notes: cash runway watch; dilution trend watch; revenue decline watch
- PLTR (Palantir Technologies Inc.): Grade X, Avoid, Score 25, Datenqualitaet A, Status fresh. Notes: crowded large cap; overheated watch; delisting watch; offering watch
- KITT (Nauticus Robotics, Inc.): Grade X, Avoid, Score 18, Datenqualitaet A, Status fresh. Notes: delisting watch; offering watch; cash runway watch

## Archive Review
- Keine Kandidaten im Archive Review.

## False Positive Review
- AISP (Airship AI Holdings, Inc): Grade X, Avoid, Score 52, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch
- GMM (Global Mofy AI Limited): Grade X, Avoid, Score 40, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch
- KDK (Kodiak AI, Inc.): Grade X, Avoid, Score 37, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; offering watch
- GFAI (Guardforce AI Co., Limited): Grade X, Avoid, Score 35, Datenqualitaet B, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; SEC companyfacts fundamentals unavailable
- AIIO (Robo.ai Inc.): Grade X, Avoid, Score 32, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; revenue decline watch
- ODYS (Odysight.ai Inc.): Grade X, Avoid, Score 29, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; offering watch; revenue decline watch
- HPAI (Helport AI Limited): Grade X, Avoid, Score 28, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; dilution trend watch
- RZLV (Rezolve AI PLC): Grade X, Avoid, Score 28, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; dilution trend watch
- HCAI (Huachen AI Parking Management Technology Holding Co., Ltd.): Grade X, Avoid, Score 18, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; cash runway watch; revenue decline watch
- JTAI (Jet.AI Inc.): Grade X, Avoid, Score 14, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; revenue decline watch
- KIDZ (KIDZ AI Inc.): Grade X, Avoid, Score 14, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; revenue decline watch
- ARAI (Arrive AI Inc.): Grade X, Avoid, Score 10, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; dilution trend watch
- BFRG (Bullfrog AI Holdings, Inc.): Grade X, Avoid, Score 10, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; dilution trend watch
- MYSE (Myseum.AI, Inc.): Grade X, Avoid, Score 8, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; cash runway watch
- FABC (Fabric.AI, Inc.): Grade X, Avoid, Score 7, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; offering watch
- MSAI (MultiSensor AI Holdings, Inc.): Grade X, Avoid, Score 7, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; offering watch
- MDAI (Spectral AI, Inc.): Grade X, Avoid, Score 2, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; dilution trend watch; revenue decline watch
- AIFC (AI Financial Corporation): Grade X, Avoid, Score 0, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; offering watch; cash runway watch; dilution trend watch
- DVLT (Datavault AI Inc.): Grade X, Avoid, Score 0, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; cash runway watch; dilution trend watch
- GXAI (Gaxos.ai Inc.): Grade X, Avoid, Score 0, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; cash runway watch; dilution trend watch
- PDYN (Palladyne AI Corp.): Grade X, Avoid, Score 0, Datenqualitaet A, Status fresh. Notes: name-only AI evidence; needs manual substance check; name only ai watch; delisting watch; offering watch; cash runway watch; dilution trend watch

## Scoring Adjustment Candidate
- Der einzige sinnvolle Kalibrierungskandidat ist aktuell: `name_only_ai_watch` noch strenger gewichten oder frueher in `X`/`Avoid` routen, weil 21 von 30 Kandidaten im False-Positive-Muster landen.
- Aber: Keine sofortige Regelanpassung empfohlen, solange Shadow-Backtest-Outcomes noch ausstehen. Erst pruefen, ob echte False Positives bestaetigt werden oder ob die Engine nur konservativ markiert.

## Datenqualitaet Und Luecken
- Market data: free_price_data_unavailable
- Filings: available
- News: public_sources_only
- Wichtigste Datenluecke fuer die naechste Verbesserung: verlaessliche freie Price/Volume-Historie, damit Momentum, relative Staerke, Liquiditaet und Paper-Entry-Readiness nicht pauschal `unavailable` bleiben.
- S-Inspektion: kein S-Kandidat vorhanden.
- X-Inspektion: 27 X-Kandidaten wurden durch Qualitaets-/Risk-Gates begrenzt; Hauptursachen sind name-only AI, Delisting-/Listing-Risiken, Dilution/Offering, Cash-Runway und fehlender AI-Revenue-Proof. X bleibt Forschungsqualitaetslabel, keine Handlungsanweisung.
- Deep-Dive-Check: keine Kandidaten erfuellen aktuell die konservative Deep-Dive-Huerde aus A/B-Datenqualitaet plus harter aktueller These ohne aktive harte Risiko-/Caution-Gates.
- Watchlist-Hygiene: Keine stille Loeschung. Kandidaten mit Avoid/X bleiben dokumentiert fuer manuelle Archive-Review statt automatischer Entfernung.

## Befehlskarte
- Chris 5-Minuten-Befehl: Pruefe einen einzigen False-Positive-Fall aus der Liste, bevorzugt `AISP` oder `GFAI`, und entscheide nur: `weiter beobachten`, `Avoid bestaetigen` oder `Archiv-Review vormerken`.
- Hermes-Pruefbefehl: `AI_STOCK_RADAR_DATE=2026-06-14 node /Users/zondrius/hermes-workspace/scripts/ai-stock-radar-weekly-calibration.mjs` und danach diesen Report gegenlesen.
- Stop-/Park-Befehl: Wenn keine bessere freie Price/Volume-Quelle verfuegbar ist, keine Momentum- oder Entry-Readiness-Regel verschaerfen.
- Nicht-ausfuehren: Keine Trades, keine Broker- oder API-Key-Einrichtung, keine Optionen/Leverage/Margin, keine automatischen Alerts mit Trade-Charakter, keine stillen Watchlist-Loeschungen.

## Decision Inbox
- Signal: Yellow
- SOFORT_MACHEN: nichts
- CHRIS_ENTSCHEIDET: Ob Archivierungs- oder Downgrade-Vorschlaege manuell uebernommen werden.
- BEOBACHTEN: False-positive Muster, stale Kandidaten und wiederkehrende name-only AI Treffer.
- SPAETER: Backtesting der Qualitaetsregeln nach mehreren Wochenlaeufen.
- BLOCKIERT: Keine automatische Watchlist-Aenderung ohne dokumentierte Review-Entscheidung.
- NICHT_TUN: Keine automatischen Trades; keine stillen Loeschungen aus der Watchlist.
- Naechste kleinste Aktion: Einen False-Positive-Kandidaten manuell pruefen und Entscheidung dokumentieren.
- Beleg / Datei: /Users/zondrius/hermes-workspace/reports/ai-stock-radar/ai-stock-deepdive-2026-06-14.md
