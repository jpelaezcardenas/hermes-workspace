...
**Why:** Anthropic explicitly identifies this as the pattern that makes agents reliable. As of 2026-03-09 single-pass fails silently.
...
**Why:** As of 2026-03-09 agents start each task cold. Handoffs are first-class in OpenAI Agents SDK — explicit control transfer with context. This is what keeps overnight runs coherent.
...
**Why:** As of 2026-03-09 a rejection leaves broken code that the next agent inherits.
...
**What:** Pass previous run context (what worked, what failed, git history) to agent at start of each task. Agents as of 2026-03-09 start blind even when re-running.
