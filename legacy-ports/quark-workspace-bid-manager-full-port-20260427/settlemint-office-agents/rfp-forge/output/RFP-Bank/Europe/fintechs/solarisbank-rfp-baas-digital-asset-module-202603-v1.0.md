...
| Issuing organization | Solarisbank (Germany) |
| Procurement subject | Baas Digital Asset Module |
...
The solution must support the end-to-end business and operational scenarios required for BaaS digital asset module. At minimum, the buyer expects support for customer onboarding hooks, asset setup and governance, transaction or order processing, ledger and entitlement updates, reconciliation, reporting, exception handling, and controlled rollout into new jurisdictions or customer segments.
...
| Core platform capability | Native support for BaaS digital asset module, including lifecycle and control workflows | Mandatory |
...
Evaluation will be performed by a cross-functional team spanning BaaS Product, Partner Engineering, Compliance, Risk, Operations, Security. The buyer may request clarification sessions, architecture walkthroughs, control deep-dives, or reference calls. Scores may be adjusted where written responses are contradicted by demo evidence, undocumented dependencies, or unclear operating assumptions.
...
Solaris expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Solaris has had public involvement in crypto custody and embedded digital-asset capabilities alongside its BaaS model. A realistic procurement around tokenized deposits or digital-asset modules would focus on modular APIs, partner onboarding, safeguarding, ledger segregation, compliance tooling and controllable rollout across multiple clients. In practical terms, that means the proposed platform must support institutional-grade digital-asset issuance, servicing, settlement, reporting and control workflows while fitting into BaaS APIs, partner management, account ledgering, payments, KYC/AML, custody dependencies, reporting and security controls. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. platform product, partner operations, compliance, risk, cyber and architecture teams would all matter. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, Solaris will likely favor configurable tenant isolation, embedded-finance controls and strong evidence that one client rollout cannot destabilize others. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by BaFin, MiCA, DORA, AML/CFT and outsourcing regulations applicable to banking-as-a-service and embedded-finance models. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes BaaS APIs, partner management, account ledgering, payments, KYC/AML, custody dependencies, reporting and security controls. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
