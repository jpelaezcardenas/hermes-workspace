...
date: 2026-03-14
version: v1.0
reference: BNPP-2026-001
---

# Request for Proposal: Tokenized Funds Platform

## Section 1: Context, Scope, and Timeline

### 1.1 Issuing Organization
BNP Paribas operates across asset servicing, securities services, global markets and fund administration businesses that are directly affected by the evolution of fund tokenization and digitally represented capital markets instruments. The Bank is seeking a platform that can support tokenized fund share issuance and servicing within a European operating model that respects fund governance, transfer restrictions, investor eligibility and administrator control requirements. In parallel, the Bank seeks market information regarding digital bond platform maturity to inform cross-business roadmap decisions. Any future solution must fit a French and pan-European control environment, align with operational risk expectations under DORA, and integrate with existing registrar, transfer agency, custody and reporting functions.
...
From a market standpoint, drivers include shorter settlement expectations, demand for improved transparency over ownership and lifecycle events, pressure to reduce manual reconciliations, and the need to support future delivery-versus-payment or programmable servicing patterns. The selected platform, if any, must prove that these benefits can be realized without compromising legal certainty, investor protection, information security or operational resilience. Priority is fund administration workflow integrity, investor onboarding alignment, NAV-driven processing and multi-entity operating models spanning management companies, depositaries and distributors.

### 1.2 Scope and Key Scenarios
This RFP concerns tokenized funds as the primary asset class. The Bank is seeking binding proposals for an implementation-capable platform. The in-scope scenarios include Money market, private market feeder and selected alternative fund share classes represented in tokenized form, with subscription, redemption, transfer restriction and holdings reporting workflows tightly governed by fund rules and transfer agency controls.
...
|---|---|---|
| BNPP-BR1 | Support configurable asset lifecycle workflows with maker-checker approval and complete audit trail | Mandatory |
| BNPP-BR2 | Provide participant, holding and entitlement views suitable for operations, compliance and reporting users | Mandatory |
| BNPP-BR3 | Handle scheduled and exception-based events, including corrections and reversals | Mandatory |
| BNPP-BR4 | Allow business users to configure core rules, templates and approval paths without code changes | Scored |
| BNPP-BR5 | Support multilingual document and notification generation where relevant to operating entities | Desirable |
...
| ID | Requirement | Priority |
|---|---|---|
| BNPP-TR1 | Expose versioned APIs and event interfaces for workflow, holdings, transactions and reporting data | Mandatory |
| BNPP-TR2 | Support segregated environments for development, testing, pre-production and production | Mandatory |
| BNPP-TR3 | Provide high availability and documented disaster recovery objectives | Mandatory |
| BNPP-TR4 | Offer deployment options compatible with the Bank's cloud and network security policies | Scored |
| BNPP-TR5 | Provide infrastructure-as-code or equivalent automated deployment support | Desirable |

The current system landscape includes Fund accounting platforms, transfer agency tools, custody platforms, distributor connectivity, SWIFT, AML screening, investor portals, pricing/NAV engines and enterprise data governance services. Bidders must state whether integration accelerators exist for these or equivalent categories, and how they would manage message translation, canonical data modelling and operational monitoring across the interface estate.

### 2.3 Security/Compliance Requirements
Security and compliance requirements are central to evaluation. The solution must support operation in a regulated banking environment subject to AMF, ACPR, ECB SSM oversight, ESMA guidance, GDPR and French outsourcing expectations. Respondents should describe how the platform supports segregation of duties, privileged access management, encryption in transit and at rest, immutable or tamper-evident logs, incident response, vulnerability management and evidence generation for internal audit and regulators. The Bank expects alignment with modern control standards, including defensible software development and third-party risk practices.
...
| ID | Requirement | Priority |
|---|---|---|
| BNPP-SC1 | Support role-based access control, MFA integration and privileged activity logging | Mandatory |
| BNPP-SC2 | Provide immutable or tamper-evident audit logs covering configuration and operational events | Mandatory |
| BNPP-SC3 | Document data residency, retention and deletion controls aligned with applicable privacy laws | Mandatory |
| BNPP-SC4 | Provide security certifications, penetration test summaries and secure SDLC evidence | Scored |
| BNPP-SC5 | Support policy-based export of audit evidence for regulator and internal audit requests | Scored |
...
BNP Paribas expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. BNP Paribas has been active across digital bonds, tokenized fund servicing, on-chain collateral and digital cash experimentation through its securities services and capital markets businesses. The group has publicly engaged in distributed ledger initiatives for post-trade modernization, fund tokenization and digital asset servicing, often through controlled institutional pilots rather than open retail crypto distribution. In practical terms, that means the proposed platform must support fund tokenization, transfer-agency processes, subscriptions, redemptions, NAV-related workflows, investor restrictions and distributor connectivity while fitting into securities services platforms, issuer services, fund administration, custody, payment rails, ISO 20022 messaging, client onboarding and KYC, market data, finance and reconciliation systems. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. program sponsorship is expected from BNP Paribas CIB, Securities Services, Digital Assets and Operations leadership working jointly with architecture, compliance and procurement. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, BNP Paribas typically expects clear control ownership, alignment with regulated operational models and multilingual cross-border servicing readiness. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by ACPR, AMF, ECB banking supervision, MiCA, DORA and French outsourcing / data protection requirements. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes securities services platforms, issuer services, fund administration, custody, payment rails, ISO 20022 messaging, client onboarding and KYC, market data, finance and reconciliation systems. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
