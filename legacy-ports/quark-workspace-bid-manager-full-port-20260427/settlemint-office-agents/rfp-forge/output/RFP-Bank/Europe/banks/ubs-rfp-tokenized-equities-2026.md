...
date: 2026-03-14
version: v1.0
reference: UBS-2026-001
---

# Request for Proposal: Tokenized Equities Platform

## Section 1: Context, Scope, and Timeline

### 1.1 Issuing Organization
UBS is evaluating a platform capable of supporting tokenized equity instruments in a manner consistent with Swiss market infrastructure, private banking control expectations and institutional post-trade discipline. The Bank requires high standards of confidentiality, entitlement control and corporate action accuracy. Separately, UBS is seeking information on the applicability of digital lifecycle tooling to selected structured product use cases where payoff complexity, suitability controls and distributor workflow needs are materially higher than vanilla securities. Solutions must demonstrate readiness for Swiss legal documentation standards, integration with existing booking and custody environments, and disciplined operating controls for high-net-worth, institutional and cross-border client contexts.
...
From a market standpoint, drivers include shorter settlement expectations, demand for improved transparency over ownership and lifecycle events, pressure to reduce manual reconciliations, and the need to support future delivery-versus-payment or programmable servicing patterns. The selected platform, if any, must prove that these benefits can be realized without compromising legal certainty, investor protection, information security or operational resilience. Priority is precise cap table and corporate action handling, private market or restricted distribution support, and strong confidentiality architecture. UBS will differentiate between credible production controls and innovation theatre.

### 1.2 Scope and Key Scenarios
This RFP concerns tokenized equities as the primary asset class. The Bank is seeking binding proposals for an implementation-capable platform. The in-scope scenarios include Private shares, employee participation structures and selected eligible investment instruments requiring issuance, transfer approval, restrictions management, dividend events and shareholder registry alignment.
...
|---|---|---|
| UBS-BR1 | Support configurable asset lifecycle workflows with maker-checker approval and complete audit trail | Mandatory |
| UBS-BR2 | Provide participant, holding and entitlement views suitable for operations, compliance and reporting users | Mandatory |
| UBS-BR3 | Handle scheduled and exception-based events, including corrections and reversals | Mandatory |
| UBS-BR4 | Allow business users to configure core rules, templates and approval paths without code changes | Scored |
| UBS-BR5 | Support multilingual document and notification generation where relevant to operating entities | Desirable |
...
| ID | Requirement | Priority |
|---|---|---|
| UBS-TR1 | Expose versioned APIs and event interfaces for workflow, holdings, transactions and reporting data | Mandatory |
| UBS-TR2 | Support segregated environments for development, testing, pre-production and production | Mandatory |
| UBS-TR3 | Provide high availability and documented disaster recovery objectives | Mandatory |
| UBS-TR4 | Offer deployment options compatible with the Bank's cloud and network security policies | Scored |
| UBS-TR5 | Provide infrastructure-as-code or equivalent automated deployment support | Desirable |

The current system landscape includes Avaloq and related wealth platforms, custody and corporate actions systems, compliance screening, document management, market data feeds, enterprise IAM and Swiss reporting workflows. Bidders must state whether integration accelerators exist for these or equivalent categories, and how they would manage message translation, canonical data modelling and operational monitoring across the interface estate.

### 2.3 Security/Compliance Requirements
Security and compliance requirements are central to evaluation. The solution must support operation in a regulated banking environment subject to FINMA requirements, Swiss banking secrecy, revised Swiss DLT framework, data residency controls and relevant cross-border securities rules. Respondents should describe how the platform supports segregation of duties, privileged access management, encryption in transit and at rest, immutable or tamper-evident logs, incident response, vulnerability management and evidence generation for internal audit and regulators. The Bank expects alignment with modern control standards, including defensible software development and third-party risk practices.
...
| ID | Requirement | Priority |
|---|---|---|
| UBS-SC1 | Support role-based access control, MFA integration and privileged activity logging | Mandatory |
| UBS-SC2 | Provide immutable or tamper-evident audit logs covering configuration and operational events | Mandatory |
| UBS-SC3 | Document data residency, retention and deletion controls aligned with applicable privacy laws | Mandatory |
| UBS-SC4 | Provide security certifications, penetration test summaries and secure SDLC evidence | Scored |
| UBS-SC5 | Support policy-based export of audit evidence for regulator and internal audit requests | Scored |
...
UBS AG expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. UBS has been publicly active across tokenized money market funds, digital bonds, structured products and Project Guardian-related industry initiatives. A UBS procurement in tokenized equities or structured products would expect mature responses around issuance controls, investor restrictions, secondary-market handling and integration into institutional wealth and market infrastructure. In practical terms, that means the proposed platform must support equity issuance or representation, corporate actions, transfer restrictions, suitability, retail or wealth distribution and market-data synchronization while fitting into wealth management platforms, capital markets systems, custody, product governance, client suitability controls, treasury, reporting and security tooling. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. sponsorship likely spans wealth management, investment bank, digital assets, technology, legal and operations leadership. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, UBS will likely score heavily on legal enforceability, investor-segmentation controls and integration with highly controlled distribution and product-governance processes. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by FINMA, SNB-related market infrastructure expectations, and for EU-facing services MiCA / DORA interaction points. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
For this procurement, bidders should assume that the first live use cases will be concrete rather than experimental. At minimum, the target operating model should support equity issuance or representation, corporate actions, transfer restrictions, suitability, retail or wealth distribution and market-data synchronization. The proposed platform should show how configuration, approvals, ledger updates, settlement instructions, exception handling and reporting are orchestrated across those use cases without requiring brittle one-off customization for each product or jurisdiction.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes wealth management platforms, capital markets systems, custody, product governance, client suitability controls, treasury, reporting and security tooling. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
