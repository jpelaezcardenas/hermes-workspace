...
| Field | Value |
| --- | --- |
| Reference | EMIRATES-NBD-RFP-TOKENIZED-DEPOSITS-TRADE-FINANCE-202603 |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Emirates NBD (UAE) |
| Document type | RFP |
| Subject | Tokenized Deposits And Trade Finance Liquidity Rails |
...
Emirates NBD is treating tokenized deposits and trade finance liquidity rails as a business-critical
capability that must be implemented with the same discipline applied to core regulated systems. The
solution under consideration will be expected to operate inside a control environment shaped by
business ownership, architecture standards, security review, legal interpretation, compliance sign-
off, and internal audit expectations. This is not a speculative innovation exercise. It is a
procurement intended to test whether the market can supply a dependable platform and implementation
model for the target operating state.
...
The first objective is to identify a platform and supplier capable of supporting tokenized deposits
and trade finance liquidity rails in live operation, not just in a demonstration setting. The buyer
wants confidence that the selected solution can sustain growth in users, volumes, products, and
audit scrutiny without fundamental rework.

The second objective is to understand the operating model implied by each response. Emirates NBD
needs to know which activities are automated, which are configurable, which require manual
oversight, and which depend on external partners, regulated intermediaries, or internal teams. A
response that treats those matters as afterthoughts will score poorly because the implementation
burden is part of the procurement decision.
...
References to initiatives such as Project Aber, Digital Dirham work, Project Khokha, sandbox
programmes, exchange modernisation, or trade digitisation are welcome when they sharpen the
response, but they should not be used as substitutes for explicit control design. Scope covers
design mobilisation, product setup, integration, testing, controls, training, and operational
readiness.
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for tokenized deposits and trade finance liquidity rails. |
...
wants to understand how the work actually lands inside Emirates NBD's organisation.
...
| REQ-09 | Support document-linked trade workflows, milestones, and counterparty controls. | High | Narrative + evidence reference |
| REQ-10 | Reconcile tokenized records to core trade and treasury books. | High | Narrative + evidence reference |
| REQ-11 | Support multi-entity trade limit allocation and exception handling. | High | Narrative + evidence reference |
...
Three issues will dominate this procurement. First is control integrity around tokenized deposits
and trade finance liquidity rails; the buyer must be able to identify who initiated a change or
transaction, which policy checks applied, who approved the event, and how the resulting state can be
reconstructed later. Second is coexistence with existing enterprise systems; the selected solution
cannot become a reconciliation sinkhole that generates more manual work than it removes. Third is
phased scalability; the buyer wants to move from initial launch to broader adoption without a
platform reset.

Bidders should use this spotlight section to explain why their architecture, delivery model, and
evidence posture are suitable for a regulated institution in UAE. The strongest responses will be
specific about operational pain points, implementation realities, and how the proposed design
contains risk rather than moving it elsewhere.

## 3. Regulatory & Compliance Requirements

### 3.1 Applicable Context

Respondents must address the legal and supervisory context relevant to UAE, including DFSA digital
asset regime, ADGM FSMR framework, CBUAE payment and stored value expectations, UAE SCA rules, UAE
AML/CFT law, outsourcing and cyber resilience expectations. Applicability may differ by legal
entity, product perimeter, client segment, and whether the target structure is treated as a payment,
...
treasury, customer servicing, technology support, and management oversight for the tokenized
deposits and trade finance liquidity rails programme.
...
For Emirates NBD, this appendix should be written in a way that supports architecture review,
control review, and implementation planning simultaneously. That means naming interfaces, documents,
checkpoints, and dependencies rather than using generic programme language.
...
For Emirates NBD, operational governance for Tokenized Deposits And Trade Finance Liquidity Rails
must define named control owners across product management, first-line operations, compliance
oversight, treasury or finance, information security, platform administration, and executive
escalation. The buyer expects the bidder to describe how each role interacts with the platform,
which actions require approvals, how temporary overrides are governed, and how post-event review is
performed. A credible response should explain not only the happy path but also the boundary
conditions: rejected transactions, stale approvals, mismatched data, delayed settlement, and
corrected records.
...
incident review, and management reporting. Emirates NBD is especially interested in whether those
routines can be supported through native reporting and workflow evidence or whether they rely on
external manual tracking. If offline controls are required, they should be called out explicitly so
the buyer can judge staffing impact.
...
A strong response should explain how data relating to Tokenized Deposits And Trade Finance Liquidity
Rails moves between channels, orchestration layers, policy engines, ledgers, books-and-records,
reporting stacks, and external participants. Emirates NBD expects the bidder to identify
authoritative data sources, event sequencing assumptions, idempotency treatment, reference data
ownership, and how replay or recovery is handled after processing interruptions.
...
Integration narratives should also address release management. Emirates NBD wants to know how API
changes are versioned, how consumers are notified, whether backward compatibility is maintained, and
how test environments reflect production behaviour. Responses that ignore lifecycle management of
interfaces create avoidable delivery risk.

### 8.8 Security, resilience, and assurance detail

Given the sensitivity of Tokenized Deposits And Trade Finance Liquidity Rails, the buyer expects
layered security controls rather than a single assertion of secure design. Bidders should describe
identity architecture, privileged access control, secret handling, encryption treatment, network
segmentation, logging, tamper evidence, and how operational staff interact with support or
...
Resilience discussion should go beyond backup existence. Emirates NBD wants evidence of recovery
objectives, tested restoration procedures, incident classification, communications protocols,
dependency-failure handling, and degraded-mode assumptions. For regulated programmes, the question
is not whether outages happen; it is whether the platform and supplier behave predictably when they
do. Bidders should therefore explain how they support event replay, queue draining, restart
discipline, and post-incident evidence gathering.
...
configuration, interface build, testing, training, and cutover fit together. Emirates NBD does not
expect a rigid waterfall plan, but it does expect formal acceptance criteria, issue governance,
milestone ownership, and explicit dependency management. If the bidder proposes agile delivery, the
governance model still needs to explain when decisions are frozen and what constitutes readiness to
progress.
...
to Tokenized Deposits And Trade Finance Liquidity Rails, the more disciplined post-go-live change
control must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that Emirates
NBD can build internal capability rather than remain dependent on a small external delivery team.
...
third-party services grow. Emirates NBD wants to know whether cost scales linearly, steps at
thresholds, or depends on discretionary supplier interpretation. A credible bid gives procurement
and finance enough information to model multiple operating scenarios.

Supplier transparency also includes product strategy, concentration risk, and partner dependency. If
core aspects of Tokenized Deposits And Trade Finance Liquidity Rails depend on a single custody,
exchange, payments, cloud, or identity partner, that should be disclosed along with substitution
options and contingency plans. Buyers do not object to partnerships; they object to discovering late
that the real operating model lives outside the contract they signed.
...
Emirates NBD (UAE) is not approaching **tokenized deposits and trade finance liquidity rails** as a generic blockchain exercise. Emirates NBD has public momentum in digital assets and in early 2026 issued the MENA region's first AED-denominated digital bond on Nasdaq Dubai using DLT infrastructure. That makes this procurement materially different from a theoretical exploration: the bank needs tooling that can scale from flagship issuances into repeatable treasury, trade-finance, and deposit-token workflows. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Likely integrations include Nasdaq Dubai and its post-trade stack, digital-bond issuance infrastructure, core treasury systems, transaction banking, sanctions and KYC controls, corporate channels, and internal audit/reporting platforms. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Capital-markets, treasury, corporate banking, digital-innovation, and risk leaders are the most relevant executive stakeholders for the next phase of deployment. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. Recent public issuance activity is a strong proxy for active procurement and platform selection behaviour even where a formal public RFP is not published. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For Emirates NBD (UAE), the most relevant workload pattern is **tokenized deposits, digital bonds, trade-finance collateralisation, post-trade servicing, and cross-platform reconciliation**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how Emirates NBD (UAE) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, Emirates NBD (UAE) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Strategic business fit | 22% | Scored against tokenized deposits and trade finance liquidity rails in the specific context of Emirates NBD (UAE). |
| Technical architecture and integration fit | 18% | Scored against tokenized deposits and trade finance liquidity rails in the specific context of Emirates NBD (UAE). |
| Regulatory, compliance, and control evidence | 20% | Scored against tokenized deposits and trade finance liquidity rails in the specific context of Emirates NBD (UAE). |
| Security, resilience, and operational readiness | 15% | Scored against tokenized deposits and trade finance liquidity rails in the specific context of Emirates NBD (UAE). |
| Delivery credibility and partner model | 12% | Scored against tokenized deposits and trade finance liquidity rails in the specific context of Emirates NBD (UAE). |
| Commercial transparency and TCO | 8% | Scored against tokenized deposits and trade finance liquidity rails in the specific context of Emirates NBD (UAE). |
| Supplier viability and roadmap discipline | 5% | Scored against tokenized deposits and trade finance liquidity rails in the specific context of Emirates NBD (UAE). |
...
A realistic proposal for Emirates NBD (UAE) should therefore provide a phased deployment hypothesis. Phase one should describe the minimum safe production scope, the volumes and participant types supported at launch, required client-side roles, major control gates, and objective acceptance criteria. Phase two should describe how the scope expands once controls, throughput, and reporting have been proven. If the bidder believes the requested use case should be sequenced differently, that is acceptable, but the answer must still show a credible route from contained launch to scaled operations.
...
Control assurance is an explicit selection criterion for Emirates NBD (UAE). The selected supplier will be expected to support walkthroughs with architecture, security, legal, compliance, operations, risk, and procurement stakeholders. Responses should therefore describe the artefacts available before contract signature and the deeper evidence available during implementation. Useful artefacts include configuration snapshots, role matrices, sample audit logs, example reconciliation outputs, incident-process documentation, environment diagrams, API specifications, backup/restore evidence, and sample operational reports. If these artefacts exist only in vendor-internal formats and cannot be shared with clients, that limitation should be declared clearly.

The rollout plan should distinguish between pilot value and production readiness. A pilot may validate that tokenized deposits, digital bonds, trade-finance collateralisation, post-trade servicing, and cross-platform reconciliation is technically feasible, but the buyer is making a sourcing decision for a programme that will need governance, support, and repeatability. Bidders should therefore outline the minimum controls needed before first live processing, the metrics that would indicate the platform is ready for broader rollout, and the dependencies that must be closed before additional products, entities, markets, or participant groups are added. Where later phases depend on external partner readiness, regulatory comfort, or contract expansion, that dependency should be made visible in the plan rather than left implicit.

Finally, respondents should explain how the programme remains governable after go-live. The buyer expects structured release management, formal change approval, regression testing, periodic entitlement review, exception governance, and management reporting that tracks the real health of the service rather than vanity metrics. Examples of meaningful indicators include exception ageing, reconciliation break volumes, failed-settlement rates, approval turnaround, security incident trends, backlog of unresolved defects, accuracy of regulatory reporting, and frequency of manual overrides. The suppliers that score best will be the ones that show how tokenized deposits and trade finance liquidity rails becomes an owned institutional capability rather than a clever but fragile technology deployment.



### 8.18 Institution-specific market signals and execution implications

Emirates NBD entered 2026 with tangible public momentum after issuing the MENA region’s first AED-denominated digital bond on Nasdaq Dubai using DLT infrastructure. That public signal changes the evaluation lens: this is no longer abstract innovation scouting. The bank has already shown willingness to take digitally native capital-markets infrastructure into a visible institutional setting, so bidders should assume far tougher questions on scalability, post-trade controls, and operational readiness. The UAE perimeter spans CBUAE, SCA, VARA where applicable, and financial-free-zone regimes such as ADGM FSRA and DIFC DFSA. The bank will care about outsourcing controls, evidentiary quality, and whether the solution can coexist with sophisticated treasury, transaction-banking, and capital-markets stacks without creating fragmented records.

This matters for the current procurement because For tokenized deposits and trade-finance liquidity rails, the relevant problem is how to connect treasury liquidity, trade obligations, settlement evidence, and enterprise controls so the bank can move beyond one-off showcases. Suppliers that cannot explain real integration boundaries, intraday liquidity views, and evidence for exception handling will fail credibility checks quickly. The bank's evaluation team is unlikely to reward broad claims about future market transformation unless those claims are tied back to concrete controls, delivery phases, and accountable operating roles. In practice, a credible proposal will show how legal and policy assumptions are captured early, how architecture decisions are frozen at the right governance points, and how implementation evidence can be re-used across procurement, architecture review, information-security review, and internal-audit challenge.

Bidders should also assume that adjacent enterprise programmes will influence sequencing. Budget owners for treasury modernization, transaction banking, corporate channels, payments, debt capital markets, compliance tooling, and enterprise data platforms will all want to know whether the proposed solution complements existing investment or forces a parallel stack. The safest answer is not maximal scope on day one; it is a phased design that makes initial production narrow, measured, and evidentially strong while still leaving room to extend into additional products, jurisdictions, participant classes, or settlement models later.

Finally, supplier transparency will be interpreted as a control signal. If critical functions depend on external node operators, custodians, exchanges, payment networks, cloud services, or document-processing partners, the buyer expects those dependencies to be named together with ownership of incidents, change windows, SLA consequences, and fallback procedures. The more directly the bidder explains where operational risk sits, the easier it will be for Emirates NBD (UAE) to decide whether the platform can become a durable institutional capability rather than a fragile innovation layer.

### 8.19 Institution-specific operating scenarios, assurance, and rollout expectations

For Emirates NBD (UAE), the buyer expects shortlisted suppliers to walk through at least three realistic scenarios during clarifications or demonstrations. Scenario one is a clean-path transaction or issuance journey in which onboarding data is complete, approvals are timely, and downstream integrations respond normally. Scenario two is an exception path in which sanctions or eligibility checks return inconclusive results, reference data is incomplete, or an approval expires before execution. Scenario three is an operational-stress case in which an external dependency, settlement network, or internal downstream system becomes unavailable after initiation but before final confirmation. The proposal should explain how the platform behaves in each case, what users can see, what is blocked automatically, how overrides are governed, and what evidence remains available afterward.

The buyer also wants explicit assurance on records and reporting. Responses should identify which identifiers remain stable across APIs, ledgers, workflow queues, documents, and reports; how users distinguish pending from final state; how corrections are versioned; and how management can view unresolved exceptions by age, cause, owner, and financial materiality. Especially in programmes involving tokenized instruments or digitally represented obligations, operational confidence often depends less on the elegance of the ledger model and more on the quality of reconciliation outputs, operational dashboards, and exported evidence packs.

Rollout planning should be equally concrete. The expected pattern is an initial production scope limited to a manageable set of entities, counterparties, products, and currencies with clearly defined success criteria. Expansion should occur only after the buyer has evidence on throughput, reconciliation accuracy, incident patterns, control-breach frequency, and support burden. Suppliers should therefore state which configuration choices are difficult to reverse, which governance decisions must be taken before phase one, and which later capabilities can be added safely once the operating model has proven stable. That level of honesty is especially important for Emirates NBD (UAE) because the institution is large enough that a weak phase-one design would create expensive downstream rework.