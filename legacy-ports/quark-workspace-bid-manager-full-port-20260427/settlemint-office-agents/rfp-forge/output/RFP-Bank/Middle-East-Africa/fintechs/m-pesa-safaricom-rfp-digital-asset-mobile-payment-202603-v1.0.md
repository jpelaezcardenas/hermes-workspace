...
| Issuing organization | M-Pesa Safaricom (Kenya) |
| Document type | RFP |
| Subject | Digital Asset Mobile Payment Overlays For Stored-Value Networks |
...
M-Pesa Safaricom is treating digital asset mobile payment overlays for stored-value networks as a
...
Regional conditions in Kenya also matter. The buyer expects bidders to anchor their response in
...
mobile payment overlays for stored-value networks in live operation, not just in a demonstration
...
The second objective is to understand the operating model implied by each response. M-Pesa Safaricom
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for digital asset mobile payment overlays for stored-value networks. |
...
wants to understand how the work actually lands inside M-Pesa Safaricom's organisation.
...
Three issues will dominate this procurement. First is control integrity around digital asset mobile
payment overlays for stored-value networks; the buyer must be able to identify who initiated a
...
evidence posture are suitable for a regulated institution in Kenya. The strongest responses will be
...
Respondents must address the legal and supervisory context relevant to Kenya, including CBK payment
expectations, data protection law, AML/CFT controls, consumer protection, cyber and outsourcing
controls, mobile money oversight. Applicability may differ by legal entity, product perimeter,
client segment, and whether the target structure is treated as a payment, security, custody
arrangement, deposit analogue, investment product, or supervised market activity. The bidder must
therefore state its assumptions clearly rather than implying universal applicability.
...
mobile payment overlays for stored-value networks programme.
...
For M-Pesa Safaricom, this appendix should be written in a way that supports architecture review,
...
For M-Pesa Safaricom, operational governance for Digital Asset Mobile Payment Overlays For Stored-
Value Networks must define named control owners across product management, first-line operations,
...
incident review, and management reporting. M-Pesa Safaricom is especially interested in whether
...
A strong response should explain how data relating to Digital Asset Mobile Payment Overlays For
Stored-Value Networks moves between channels, orchestration layers, policy engines, ledgers, books-
and-records, reporting stacks, and external participants. M-Pesa Safaricom expects the bidder to
...
Integration narratives should also address release management. M-Pesa Safaricom wants to know how
...
Given the sensitivity of Digital Asset Mobile Payment Overlays For Stored-Value Networks, the buyer
...
Resilience discussion should go beyond backup existence. M-Pesa Safaricom wants evidence of recovery
...
configuration, interface build, testing, training, and cutover fit together. M-Pesa Safaricom does
...
to Digital Asset Mobile Payment Overlays For Stored-Value Networks, the more disciplined post-go-
live change control must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that M-Pesa
Safaricom can build internal capability rather than remain dependent on a small external delivery
team.
...
third-party services grow. M-Pesa Safaricom wants to know whether cost scales linearly, steps at
...
core aspects of Digital Asset Mobile Payment Overlays For Stored-Value Networks depend on a single
...
M-Pesa Safaricom (Kenya) is not approaching **digital asset mobile payment overlays for stored-value networks** as a generic blockchain exercise. M-Pesa is a national-scale mobile-money platform, so any digital-asset extension must improve payment, treasury, or merchant settlement flows without compromising trust, uptime, or regulator confidence. The relevant conversation is around programmable money operations and corridor settlement, not retail crypto speculation. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Likely dependencies include CBK-supervised rails, Safaricom channels, bank partners, agent/merchant systems, AML/fraud tooling, and cross-border remittance partners. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Payments, platform operations, treasury, regulatory affairs, and cyber-security leadership are the critical stakeholders. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. No direct public digital-asset RFP was identified; assume a resilience-heavy procurement posture because any change touches nationally important payment infrastructure. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For M-Pesa Safaricom (Kenya), the most relevant workload pattern is **mobile-money settlement, merchant treasury, cross-border remittance corridors, wallet/account interoperability, and compliance reporting**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
The regulatory perimeter for M-Pesa Safaricom (Kenya) is shaped by Central Bank of Kenya (CBK), Capital Markets Authority Kenya, Data Protection Act 2019, Proceeds of Crime and Anti-Money Laundering Act, National Payment System oversight, consumer-protection obligations for mobile money and capital markets. Bidders must not treat that list as a box-ticking reference. They need to translate the perimeter into platform behaviour: data-location choices, audit-log retention periods, approval routing, customer classification, reporting outputs, outsourcing controls, incident escalation, access recertification, and evidentiary support during supervisory review.

Where the requested use case intersects capital-markets activity, payments, custody, digital-asset exchange operations, or tokenized deposit analogues, respondents should explain the assumed legal characterisation and the resulting control posture. If a proposed architecture depends on a regulated partner, foreign booking model, special-purpose issuer, bankruptcy-remote vehicle, or external wallet/custodian, that structure must be declared explicitly together with the residual obligations that remain with M-Pesa Safaricom (Kenya). If Sharia governance is relevant, the bidder should explain how product rules, approval lineage, prohibited activity restrictions, and board or committee evidence are captured and reported.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how M-Pesa Safaricom (Kenya) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, M-Pesa Safaricom (Kenya) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Use-case fit and operating leverage | 24% | Scored against digital asset mobile payment overlays for stored-value networks in the specific context of M-Pesa Safaricom (Kenya). |
| Integration and API maturity | 18% | Scored against digital asset mobile payment overlays for stored-value networks in the specific context of M-Pesa Safaricom (Kenya). |
| Compliance, controls, and auditability | 18% | Scored against digital asset mobile payment overlays for stored-value networks in the specific context of M-Pesa Safaricom (Kenya). |
| Settlement, treasury, and resilience design | 15% | Scored against digital asset mobile payment overlays for stored-value networks in the specific context of M-Pesa Safaricom (Kenya). |
| Delivery speed and configurability | 12% | Scored against digital asset mobile payment overlays for stored-value networks in the specific context of M-Pesa Safaricom (Kenya). |
| Commercial scalability | 8% | Scored against digital asset mobile payment overlays for stored-value networks in the specific context of M-Pesa Safaricom (Kenya). |
| Supplier viability and dependency transparency | 5% | Scored against digital asset mobile payment overlays for stored-value networks in the specific context of M-Pesa Safaricom (Kenya). |
...
contained innovation-lab assumption. The platform must therefore prove it can handle high concurrency, simple user journeys, agent and merchant operations, reversals, fraud controls, and degraded-mode support while preserving clean segregation between customer wallet balances and any underlying tokenized settlement mechanisms.
...
- Support telecom-scale operational monitoring, customer-support evidence, and rapid handling of reversals, fraud alerts, and mistaken transfers.
- Provide explicit segregation between consumer wallet experience and underlying tokenized settlement or liquidity mechanisms so customer protection remains clear.
- Allow phased rollout across merchant settlement, cross-border corridors, and internal treasury use cases before any broad consumer-facing proposition.

These requirements are additive to the baseline sections of this document. They are included because
M-Pesa / Safaricom (Kenya) operates with business, regulatory, and operational constraints that differ from a
