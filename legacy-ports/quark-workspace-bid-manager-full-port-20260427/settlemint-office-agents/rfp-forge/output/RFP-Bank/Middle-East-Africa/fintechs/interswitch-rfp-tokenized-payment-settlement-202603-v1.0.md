...
| Issuing organization | Interswitch (Nigeria) |
| Document type | RFP |
| Subject | Tokenized Payment Settlement And Reconciliation Network |
...
Interswitch is treating tokenized payment settlement and reconciliation network as a business-
...
Regional conditions in Nigeria also matter. The buyer expects bidders to anchor their response in
...
The first objective is to identify a platform and supplier capable of supporting tokenized payment
settlement and reconciliation network in live operation, not just in a demonstration setting. The
...
The second objective is to understand the operating model implied by each response. Interswitch
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for tokenized payment settlement and reconciliation network. |
...
wants to understand how the work actually lands inside Interswitch's organisation.
...
Three issues will dominate this procurement. First is control integrity around tokenized payment
settlement and reconciliation network; the buyer must be able to identify who initiated a change or
...
evidence posture are suitable for a regulated institution in Nigeria. The strongest responses will
...
Respondents must address the legal and supervisory context relevant to Nigeria, including CBN
guidance, SEC Nigeria digital asset rules where relevant, payment system guidelines, AML/CFT
controls, NDPC data obligations, outsourcing and cyber expectations. Applicability may differ by
legal entity, product perimeter, client segment, and whether the target structure is treated as a
payment, security, custody arrangement, deposit analogue, investment product, or supervised market
activity. The bidder must therefore state its assumptions clearly rather than implying universal
applicability.
...
treasury, customer servicing, technology support, and management oversight for the tokenized payment
settlement and reconciliation network programme.
...
For Interswitch, this appendix should be written in a way that supports architecture review, control
...
For Interswitch, operational governance for Tokenized Payment Settlement And Reconciliation Network
...
incident review, and management reporting. Interswitch is especially interested in whether those
...
A strong response should explain how data relating to Tokenized Payment Settlement And
Reconciliation Network moves between channels, orchestration layers, policy engines, ledgers, books-
and-records, reporting stacks, and external participants. Interswitch expects the bidder to identify
...
Integration narratives should also address release management. Interswitch wants to know how API
...
Given the sensitivity of Tokenized Payment Settlement And Reconciliation Network, the buyer expects
...
Resilience discussion should go beyond backup existence. Interswitch wants evidence of recovery
...
configuration, interface build, testing, training, and cutover fit together. Interswitch does not
...
to Tokenized Payment Settlement And Reconciliation Network, the more disciplined post-go-live change
control must be.
...
Interswitch can build internal capability rather than remain dependent on a small external delivery
team.
...
third-party services grow. Interswitch wants to know whether cost scales linearly, steps at
...
core aspects of Tokenized Payment Settlement And Reconciliation Network depend on a single custody,
...
Interswitch (Nigeria) is not approaching **tokenized payment settlement and reconciliation network** as a generic blockchain exercise. Interswitch sits deep in payment switching and enterprise settlement. Tokenized settlement would only be credible if it reduces reconciliation latency, improves liquidity visibility, and integrates with the existing switching and bank-partner ecosystem. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Expected dependencies include issuing/acquiring banks, switching infrastructure, merchant settlement, treasury, fraud and AML tooling, and local payment-system rails. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Switching, enterprise payments, treasury, risk, and platform engineering are likely governance owners. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. The use case is operationally plausible because of Interswitch's role in payment orchestration and settlement. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For Interswitch (Nigeria), the most relevant workload pattern is **tokenized settlement ledger, merchant and bank reconciliation, liquidity controls, and switch-to-ledger event integrity**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
The regulatory perimeter for Interswitch (Nigeria) is shaped by Central Bank of Nigeria (CBN), Securities and Exchange Commission Nigeria, Nigeria Data Protection Act, Money Laundering (Prevention and Prohibition) Act 2022, foreign-exchange and payment-system supervision, digital-asset guidance where capital-market products are involved. Bidders must not treat that list as a box-ticking reference. They need to translate the perimeter into platform behaviour: data-location choices, audit-log retention periods, approval routing, customer classification, reporting outputs, outsourcing controls, incident escalation, access recertification, and evidentiary support during supervisory review.

Where the requested use case intersects capital-markets activity, payments, custody, digital-asset exchange operations, or tokenized deposit analogues, respondents should explain the assumed legal characterisation and the resulting control posture. If a proposed architecture depends on a regulated partner, foreign booking model, special-purpose issuer, bankruptcy-remote vehicle, or external wallet/custodian, that structure must be declared explicitly together with the residual obligations that remain with Interswitch (Nigeria). If Sharia governance is relevant, the bidder should explain how product rules, approval lineage, prohibited activity restrictions, and board or committee evidence are captured and reported.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how Interswitch (Nigeria) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, Interswitch (Nigeria) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Use-case fit and operating leverage | 24% | Scored against tokenized payment settlement and reconciliation network in the specific context of Interswitch (Nigeria). |
| Integration and API maturity | 18% | Scored against tokenized payment settlement and reconciliation network in the specific context of Interswitch (Nigeria). |
| Compliance, controls, and auditability | 18% | Scored against tokenized payment settlement and reconciliation network in the specific context of Interswitch (Nigeria). |
| Settlement, treasury, and resilience design | 15% | Scored against tokenized payment settlement and reconciliation network in the specific context of Interswitch (Nigeria). |
| Delivery speed and configurability | 12% | Scored against tokenized payment settlement and reconciliation network in the specific context of Interswitch (Nigeria). |
| Commercial scalability | 8% | Scored against tokenized payment settlement and reconciliation network in the specific context of Interswitch (Nigeria). |
| Supplier viability and dependency transparency | 5% | Scored against tokenized payment settlement and reconciliation network in the specific context of Interswitch (Nigeria). |
...
A realistic proposal for Interswitch (Nigeria) should therefore provide a phased deployment hypothesis. Phase one should describe the minimum safe production scope, the volumes and participant types supported at launch, required client-side roles, major control gates, and objective acceptance criteria. Phase two should describe how the scope expands once controls, throughput, and reporting have been proven. If the bidder believes the requested use case should be sequenced differently, that is acceptable, but the answer must still show a credible route from contained launch to scaled operations.
...
Public market context reinforces that Interswitch (Nigeria / Pan-African payments) should be treated as a specific institutional
buyer, not as a placeholder for the region. It is a payments infrastructure provider operating switching, card, merchant, and enterprise settlement services across Nigeria and other African markets. Interswitch's core strength is infrastructure-scale payment processing rather than speculative asset issuance. A tokenized settlement platform would need to address enterprise-grade throughput, merchant and issuer reconciliation, and bank-grade service reliability while fitting Nigeria's cautious digital-asset perimeter.

Key context includes CBN payment and switching oversight, AML/CFT and fraud controls, card-network dependencies, merchant-settlement obligations, and the distinction between enterprise settlement utility and regulated public crypto activity.

For the target use case of **tokenized payment-settlement platform**, that public context changes the procurement lens.
...
contained innovation-lab assumption. The implementation burden is therefore likely to center on closed-loop or permissioned settlement use cases, participant-bank interfaces, and auditable reconciliation between tokenized settlement records and conventional books-and-records.
...
- Support high-volume batch and event-driven reconciliation across switch events, merchant settlement, issuer positions, and bank funding accounts.
- Provide explicit controls to ring-fence permissioned settlement tokens or obligations from prohibited or non-approved public crypto workflows.
- Allow staged rollout by participant bank, merchant segment, or settlement use case to avoid systemic operational concentration during early adoption.

These requirements are additive to the baseline sections of this document. They are included because
Interswitch (Nigeria / Pan-African payments) operates with business, regulatory, and operational constraints that differ from a
