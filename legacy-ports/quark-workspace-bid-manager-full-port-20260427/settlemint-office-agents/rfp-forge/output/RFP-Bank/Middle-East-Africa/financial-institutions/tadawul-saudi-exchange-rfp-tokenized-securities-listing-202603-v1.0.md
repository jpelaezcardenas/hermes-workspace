...
| Field | Value |
| --- | --- |
| Reference | TADAWUL-SAUDI-EXCHANGE-RFP-TOKENIZED-SECURITIES-LISTING-202603 |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Tadawul Saudi Exchange (KSA) |
| Document type | RFP |
| Subject | Tokenized Securities Listing And Market-Operations Stack |
...
Tadawul Saudi Exchange is treating tokenized securities listing and market-operations stack as a
...
The first objective is to identify a platform and supplier capable of supporting tokenized
securities listing and market-operations stack in live operation, not just in a demonstration
...
The second objective is to understand the operating model implied by each response. Tadawul Saudi
Exchange needs to know which activities are automated, which are configurable, which require manual
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for tokenized securities listing and market-operations stack. |
...
wants to understand how the work actually lands inside Tadawul Saudi Exchange's organisation.
...
Three issues will dominate this procurement. First is control integrity around tokenized securities
listing and market-operations stack; the buyer must be able to identify who initiated a change or
...
securities listing and market-operations stack programme.
...
For Tadawul Saudi Exchange, this appendix should be written in a way that supports architecture
...
For Tadawul Saudi Exchange, operational governance for Tokenized Securities Listing And Market-
Operations Stack must define named control owners across product management, first-line operations,
...
incident review, and management reporting. Tadawul Saudi Exchange is especially interested in
...
A strong response should explain how data relating to Tokenized Securities Listing And Market-
Operations Stack moves between channels, orchestration layers, policy engines, ledgers, books-and-
records, reporting stacks, and external participants. Tadawul Saudi Exchange expects the bidder to
...
Integration narratives should also address release management. Tadawul Saudi Exchange wants to know
...
Given the sensitivity of Tokenized Securities Listing And Market-Operations Stack, the buyer expects
...
Resilience discussion should go beyond backup existence. Tadawul Saudi Exchange wants evidence of
...
configuration, interface build, testing, training, and cutover fit together. Tadawul Saudi Exchange
...
to Tokenized Securities Listing And Market-Operations Stack, the more disciplined post-go-live
change control must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that Tadawul
Saudi Exchange can build internal capability rather than remain dependent on a small external
delivery team.
...
third-party services grow. Tadawul Saudi Exchange wants to know whether cost scales linearly, steps
...
core aspects of Tokenized Securities Listing And Market-Operations Stack depend on a single custody,
...
Tadawul Saudi Exchange (KSA) is not approaching **tokenized securities listing and market-operations stack** as a generic blockchain exercise. Tadawul sits at the centre of Saudi capital-market digitisation and therefore has a credible case for tokenized securities listing and post-trade modernization. The programme should be framed around market-integrity controls, issuer workflow, and compatibility with local CSD and broker infrastructure. The implication for bidders is simple: responses must explain how a controlled production programme would be governed from initial launch through scale, how policy and entitlement decisions are evidenced, and how the platform avoids creating a parallel operating stack that business, risk, and audit teams cannot actually supervise.

The buyer expects the proposed solution to align with the institution's current operating reality. Likely integrations include Edaa, Muqassa, brokers, issuers, surveillance tools, and CMA reporting processes. A strong response will therefore identify authoritative systems of record, describe event sequencing between the platform and adjacent enterprise systems, and explain how breaks are detected before they become unresolved ledger differences or customer-impacting incidents.

Executive sponsorship is also expected to be multidisciplinary. Exchange operations, technology, post-trade, issuer services, and regulatory-affairs leaders are likely stakeholders. This matters because the selected supplier will not be evaluated by innovation staff alone. It will be tested by people who care about booking-model integrity, operational resilience, information security, regulatory defensibility, and board-quality reporting.

Finally, the buyer wants commercial and sourcing realism. Saudi capital-market modernization provides a strong public context for targeted platform sourcing. Bidders should assume that any hidden dependency, vague implementation assumption, or roadmap-led answer will be surfaced quickly during architecture review and clarification sessions.

### 8.12 Company-specific use-case and technical depth

For Tadawul Saudi Exchange (KSA), the most relevant workload pattern is **tokenized listing, issuer onboarding, post-trade integration, surveillance, and reporting**. Bidders should therefore address at least five operating layers in detail. First, origination and onboarding: how clients, issuers, investors, counterparties, merchants, or participating institutions are onboarded; how eligibility rules are applied; how documentation is stored and versioned; and how approvals are routed when records are incomplete or contradictory. Second, lifecycle execution: how assets, payment obligations, investor entitlements, receivables, or tokenized instruments are created; which fields are configurable; which actions require human approval; and how rules differ by entity, corridor, investor class, or instrument type.
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how Tadawul Saudi Exchange (KSA) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, Tadawul Saudi Exchange (KSA) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Policy and market-infrastructure fit | 22% | Scored against tokenized securities listing and market-operations stack in the specific context of Tadawul Saudi Exchange (KSA). |
| Technical architecture and interoperability | 18% | Scored against tokenized securities listing and market-operations stack in the specific context of Tadawul Saudi Exchange (KSA). |
| Regulatory evidence and supervisory control support | 22% | Scored against tokenized securities listing and market-operations stack in the specific context of Tadawul Saudi Exchange (KSA). |
| Security, resilience, and data-governance posture | 15% | Scored against tokenized securities listing and market-operations stack in the specific context of Tadawul Saudi Exchange (KSA). |
| Delivery and implementation realism | 10% | Scored against tokenized securities listing and market-operations stack in the specific context of Tadawul Saudi Exchange (KSA). |
| Commercial transparency | 6% | Scored against tokenized securities listing and market-operations stack in the specific context of Tadawul Saudi Exchange (KSA). |
| Supplier capability and ecosystem alignment | 7% | Scored against tokenized securities listing and market-operations stack in the specific context of Tadawul Saudi Exchange (KSA). |
...
A realistic proposal for Tadawul Saudi Exchange (KSA) should therefore provide a phased deployment hypothesis. Phase one should describe the minimum safe production scope, the volumes and participant types supported at launch, required client-side roles, major control gates, and objective acceptance criteria. Phase two should describe how the scope expands once controls, throughput, and reporting have been proven. If the bidder believes the requested use case should be sequenced differently, that is acceptable, but the answer must still show a credible route from contained launch to scaled operations.
...
Control assurance is an explicit selection criterion for Tadawul Saudi Exchange (KSA). The selected supplier will be expected to support walkthroughs with architecture, security, legal, compliance, operations, risk, and procurement stakeholders. Responses should therefore describe the artefacts available before contract signature and the deeper evidence available during implementation. Useful artefacts include configuration snapshots, role matrices, sample audit logs, example reconciliation outputs, incident-process documentation, environment diagrams, API specifications, backup/restore evidence, and sample operational reports. If these artefacts exist only in vendor-internal formats and cannot be shared with clients, that limitation should be declared clearly.

The rollout plan should distinguish between pilot value and production readiness. A pilot may validate that tokenized listing, issuer onboarding, post-trade integration, surveillance, and reporting is technically feasible, but the buyer is making a sourcing decision for a programme that will need governance, support, and repeatability. Bidders should therefore outline the minimum controls needed before first live processing, the metrics that would indicate the platform is ready for broader rollout, and the dependencies that must be closed before additional products, entities, markets, or participant groups are added. Where later phases depend on external partner readiness, regulatory comfort, or contract expansion, that dependency should be made visible in the plan rather than left implicit.

Finally, respondents should explain how the programme remains governable after go-live. The buyer expects structured release management, formal change approval, regression testing, periodic entitlement review, exception governance, and management reporting that tracks the real health of the service rather than vanity metrics. Examples of meaningful indicators include exception ageing, reconciliation break volumes, failed-settlement rates, approval turnaround, security incident trends, backlog of unresolved defects, accuracy of regulatory reporting, and frequency of manual overrides. The suppliers that score best will be the ones that show how tokenized securities listing and market-operations stack becomes an owned institutional capability rather than a clever but fragile technology deployment.


### 8.18 Institution-specific market research and operating implications

Public market context reinforces that Tadawul Saudi Exchange (Saudi Arabia) should be treated as a specific institutional
buyer, not as a placeholder for the region. It is a Saudi Arabia's national exchange group coordinating listing, trading, clearing, depository, and capital-markets development under the Kingdom's broader financial-sector agenda. Tadawul's interest in tokenized securities should be read through the lens of Vision 2030, capital-market deepening, institutional investor participation, and coordination with Muqassa and Edaa. The exchange is likely to prioritize orderly market infrastructure, investor protection, and scalable issuance controls over crypto-native experimentation.

The key perimeter includes CMA rules, Saudi listing and disclosure obligations, post-trade dependencies, AML/CFT requirements, data and cyber controls, and where relevant Sharia-governance expectations for sukuk or other Islamic structures.

For the target use case of **tokenized securities listing platform**, that public context changes the procurement lens.
...
contained innovation-lab assumption. A credible platform must therefore support issuance and admission workflows, qualified-investor segmentation where required, corporate-action servicing, and clean interfaces with clearing, depository, and official reporting processes.
...
- Support listing and admission controls for tokenized sukuk, debt, funds, or structured instruments with strong disclosure and document-governance features.
- Integrate with Saudi post-trade infrastructure so token positions, beneficial-owner records, and settlement evidence remain consistent across Edaa, Muqassa, brokers, and custodians.
- Allow differentiated Sharia-governance evidence where instrument structure, asset backing, or profit-distribution logic requires formal review.

These requirements are additive to the baseline sections of this document. They are included because
Tadawul Saudi Exchange (Saudi Arabia) operates with business, regulatory, and operational constraints that differ from a
