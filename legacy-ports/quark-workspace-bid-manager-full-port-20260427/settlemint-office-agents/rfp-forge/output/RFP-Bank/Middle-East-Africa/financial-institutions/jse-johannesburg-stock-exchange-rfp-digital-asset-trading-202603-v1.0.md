...
| Field | Value |
| --- | --- |
| Reference | JSE-JOHANNESBURG-STOCK-EXCHANGE-RFP-DIGITAL-ASSET-TRADING-202603 |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | JSE Johannesburg Stock Exchange (South Africa) |
| Document type | RFP |
| Subject | Digital Asset Trading And Regulated Market Infrastructure |
...
JSE Johannesburg Stock Exchange is treating digital asset trading and regulated market
infrastructure as a business-critical capability that must be implemented with the same discipline
...
trading and regulated market infrastructure in live operation, not just in a demonstration setting.
...
The second objective is to understand the operating model implied by each response. JSE Johannesburg
Stock Exchange needs to know which activities are automated, which are configurable, which require
...
| WS-02 | Business and product configuration | Configure lifecycle, roles, limits, disclosures, and policy rules for digital asset trading and regulated market infrastructure. |
...
wants to understand how the work actually lands inside JSE Johannesburg Stock Exchange's
organisation.
...
Three issues will dominate this procurement. First is control integrity around digital asset trading
and regulated market infrastructure; the buyer must be able to identify who initiated a change or
...
trading and regulated market infrastructure programme.
...
For JSE Johannesburg Stock Exchange, this appendix should be written in a way that supports
...
For JSE Johannesburg Stock Exchange, operational governance for Digital Asset Trading And Regulated
Market Infrastructure must define named control owners across product management, first-line
...
incident review, and management reporting. JSE Johannesburg Stock Exchange is especially interested
in whether those routines can be supported through native reporting and workflow evidence or whether
...
A strong response should explain how data relating to Digital Asset Trading And Regulated Market
Infrastructure moves between channels, orchestration layers, policy engines, ledgers, books-and-
records, reporting stacks, and external participants. JSE Johannesburg Stock Exchange expects the
...
Integration narratives should also address release management. JSE Johannesburg Stock Exchange wants
...
Given the sensitivity of Digital Asset Trading And Regulated Market Infrastructure, the buyer
...
Resilience discussion should go beyond backup existence. JSE Johannesburg Stock Exchange wants
...
configuration, interface build, testing, training, and cutover fit together. JSE Johannesburg Stock
...
to Digital Asset Trading And Regulated Market Infrastructure, the more disciplined post-go-live
change control must be.

Training and knowledge transfer are part of scope. Bidders should state what documentation,
workshops, admin training, operator runbooks, and handover artifacts are provided so that JSE
Johannesburg Stock Exchange can build internal capability rather than remain dependent on a small
...
third-party services grow. JSE Johannesburg Stock Exchange wants to know whether cost scales
...
core aspects of Digital Asset Trading And Regulated Market Infrastructure depend on a single
...
Operational resilience is equally important. Responses should specify recovery-time and recovery-point targets, restoration testing frequency, dependency mapping, degraded-mode behaviour, and the operational plan for third-party outages, network partition, failed screening calls, stuck workflows, delayed settlement confirmations, or inconsistent state across ledgers. The strongest responses will show how JSE Johannesburg Stock Exchange (South Africa) could take the platform through architecture review, information-security review, business-continuity review, and internal-audit walkthrough without rewriting the story for each audience.

### 8.14 Company-specific evaluation overlay

In addition to the standard scoring model already defined in this document, JSE Johannesburg Stock Exchange (South Africa) expects evaluators to apply the following company-specific weighting overlay when distinguishing between suppliers:

| Criterion | Weight | Why it matters in this procurement |
| --- | --- | --- |
| Policy and market-infrastructure fit | 22% | Scored against digital asset trading and regulated market infrastructure in the specific context of JSE Johannesburg Stock Exchange (South Africa). |
| Technical architecture and interoperability | 18% | Scored against digital asset trading and regulated market infrastructure in the specific context of JSE Johannesburg Stock Exchange (South Africa). |
| Regulatory evidence and supervisory control support | 22% | Scored against digital asset trading and regulated market infrastructure in the specific context of JSE Johannesburg Stock Exchange (South Africa). |
| Security, resilience, and data-governance posture | 15% | Scored against digital asset trading and regulated market infrastructure in the specific context of JSE Johannesburg Stock Exchange (South Africa). |
| Delivery and implementation realism | 10% | Scored against digital asset trading and regulated market infrastructure in the specific context of JSE Johannesburg Stock Exchange (South Africa). |
| Commercial transparency | 6% | Scored against digital asset trading and regulated market infrastructure in the specific context of JSE Johannesburg Stock Exchange (South Africa). |
| Supplier capability and ecosystem alignment | 7% | Scored against digital asset trading and regulated market infrastructure in the specific context of JSE Johannesburg Stock Exchange (South Africa). |
...
A realistic proposal for JSE Johannesburg Stock Exchange (South Africa) should therefore provide a phased deployment hypothesis. Phase one should describe the minimum safe production scope, the volumes and participant types supported at launch, required client-side roles, major control gates, and objective acceptance criteria. Phase two should describe how the scope expands once controls, throughput, and reporting have been proven. If the bidder believes the requested use case should be sequenced differently, that is acceptable, but the answer must still show a credible route from contained launch to scaled operations.
...
contained innovation-lab assumption. For JSE, the key question is whether a platform can support tokenized instruments without breaking market transparency, surveillance, registrar integrity, and predictable post-trade operations for members and custodians.
...
- Support member and participant hierarchies suited to brokers, custodians, CSD participants, issuers, and market-operations teams.
- Integrate surveillance, market-abuse monitoring, and official reporting so digital-asset trading does not weaken market-integrity controls.
- Provide resilient lifecycle support for issuance, secondary-market transfers, corporate actions, and reconciliation back to official post-trade records.

These requirements are additive to the baseline sections of this document. They are included because
JSE Johannesburg Stock Exchange (South Africa) operates with business, regulatory, and operational constraints that differ from a
