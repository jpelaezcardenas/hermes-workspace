...
| Field | Value |
| --- | --- |
| Reference | `WISE-RFI-STABLECOIN-INFRASTRUCTURE-202603` |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Wise (United Kingdom) |
| Information request subject | Stablecoin Infrastructure |
...
| Contact point | wise-market-scan@issuer.example |
| Status | Non-binding market inquiry |

## 1. Procurement Context

Wise is conducting this RFI to understand the current vendor landscape for **stablecoin infrastructure**. This is a market and capability scan, not an award-ready procurement. The purpose is to compare what vendors can do today, what can be configured quickly, what depends on partners, and what is still roadmap.
...
| --- | --- |
| Capability maturity | How well vendors currently support stablecoin infrastructure and adjacent workflows |
...
| Extension path | Whether the same foundation can support stablecoins, tokenized payment obligations, bridge accounts, reconciliation services over time |
...
This RFI covers functional capability, technical architecture, operating model, control framework, and indicative commercials for stablecoin infrastructure. The buyer is interested in both minimum viable launch support and extension potential. Vendors should answer based on current product reality, not aspiration.
...
| --- | --- | --- |
| CQ-01 | Describe your current support for stablecoin infrastructure. | Status label + 150-word summary |
...
Wise expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Wise is most plausibly evaluating digital assets through cross-border payments, stablecoins and treasury settlement efficiency rather than capital-markets issuance. The realistic procurement question is whether tokenized payment rails can improve speed, cut trapped liquidity and preserve the transparency and compliance profile Wise already markets heavily. In practical terms, that means the proposed platform must support stablecoin funding, treasury settlement, mint/burn controls, reserve reporting, sanctions controls and integration into payment orchestration while fitting into cross-border payment engine, treasury and liquidity systems, correspondent banking connections, customer onboarding, sanctions screening, ledgers, analytics and monitoring. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.
