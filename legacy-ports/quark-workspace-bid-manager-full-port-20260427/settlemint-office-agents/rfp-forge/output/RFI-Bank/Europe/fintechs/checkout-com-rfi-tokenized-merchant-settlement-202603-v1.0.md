...
| Field | Value |
| --- | --- |
| Reference | `CHECKOUT-COM-RFI-TOKENIZED-MERCHANT-SETTLEMENT-202603` |
| Version | v1.0 |
| Issue date | March 2026 |
| Issuing organization | Checkout.com (United Kingdom) |
| Information request subject | Tokenized Merchant Settlement |
...
| Contact point | checkout-com-market-scan@issuer.example |
| Status | Non-binding market inquiry |

## 1. Procurement Context

Checkout.com is conducting this RFI to understand the current vendor landscape for **tokenized merchant settlement**. This is a market and capability scan, not an award-ready procurement. The purpose is to compare what vendors can do today, what can be configured quickly, what depends on partners, and what is still roadmap.
...
| Extension path | Whether the same foundation can support merchant wallets, stablecoin acquiring, tokenized receivables, settlement reporting over time |
...
This RFI covers functional capability, technical architecture, operating model, control framework, and indicative commercials for tokenized merchant settlement. The buyer is interested in both minimum viable launch support and extension potential. Vendors should answer based on current product reality, not aspiration.
...
| --- | --- | --- |
| CQ-01 | Describe your current support for tokenized merchant settlement. | Status label + 150-word summary |
...
Checkout.com expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Checkout.com’s digital asset relevance comes from payment processing, merchant settlement and stablecoin-enabled commerce rather than capital-markets issuance. The buyer is likely evaluating whether tokenized settlement rails can improve merchant treasury timing, cross-border settlement and cost efficiency without undermining compliance posture. In practical terms, that means the proposed platform must support merchant settlement timing, treasury optimization, payout certainty, reconciliation and regional rollout governance while fitting into merchant payments gateway, settlement and treasury stack, reconciliation, risk systems, API gateways, data platforms and merchant dashboards. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. payments product, treasury, engineering, compliance and security leadership would drive selection. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, the company will score vendors on merchant-grade throughput, reconciliation quality, treasury controls and jurisdiction-aware rollout. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by FCA and EEA payments / crypto perimeter where relevant, MiCA for EU expansion, DORA and GDPR. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes merchant payments gateway, settlement and treasury stack, reconciliation, risk systems, API gateways, data platforms and merchant dashboards. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
...
Because this is an RFI rather than a committed award process, Checkout.com is using this document to separate productized platforms from ambitious slide decks. Respondents should therefore describe the maturity of their solution for merchant settlement timing, treasury optimization, payout certainty, reconciliation and regional rollout governance in very concrete terms: what is live today, what is in pilot, what is customer-specific, and what remains roadmap. The buyer is especially interested in evidence that a platform can survive production governance, exception volume, regulator scrutiny and multi-team operating ownership.
