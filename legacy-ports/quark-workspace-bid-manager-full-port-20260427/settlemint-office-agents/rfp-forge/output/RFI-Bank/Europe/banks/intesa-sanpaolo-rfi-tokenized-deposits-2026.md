...
date: 2026-03-14
version: v1.0
reference: ISP-2026-002
---

# Request for Information: Market Sounding on Tokenized Deposit Platforms and Operating Models

## Section 1: Context, Scope, and Timeline

### 1.1 Issuing Organization
Intesa Sanpaolo is evaluating the next stage of its digital securities operating model following market activity in blockchain-based bond issuance and broader innovation in digital finance within the Italian and European regulatory perimeter. As a systemically important banking group under ECB supervision and Italian regulatory oversight, the Bank requires procurement outputs that fit a production environment, support robust control frameworks and integrate into an established banking and capital markets architecture. The Bank is motivated by the need to shorten execution cycles for debt issuance, improve transparency across allocation and settlement, support policy-driven investor eligibility controls, and create a technology base that can also inform future tokenized liability and cash-settlement initiatives. The target platform must support disciplined governance rather than experimentation for its own sake.
...
Responses shall consider ECB outsourcing and resilience expectations, Bank of Italy and CONSOB interfaces where relevant, MiFID II, AMLD requirements, GDPR, DORA, eIDAS, EBA guidance on ICT and outsourcing risk, and any implications of MiCA perimeter boundaries where respondents believe them relevant to product architecture.

### 1.2 Scope and Key Scenarios
This RFI seeks non-binding market information regarding tokenized deposit capabilities, including ledger-based representations of bank liabilities, internal transfer controls, treasury integration, payment-versus-payment and delivery-versus-payment options, intragroup and client payment scenarios, and how vendors would support policy, liquidity, reconciliation and legal documentation in an Italian and EU banking context.
...
| --- | --- | --- |
| ISP-B1 | Support controlled creation, approval and release of the target asset lifecycle with configurable maker-checker controls. | Mandatory |
| ISP-B2 | Provide investor or participant eligibility rules aligned to jurisdiction, product and distribution constraints. | Mandatory |
| ISP-B3 | Support lifecycle servicing events including distributions, redemptions, transfers, amendments and exceptions. | Mandatory |
| ISP-B4 | Provide operational dashboards, exception queues and status tracking for front-to-back teams. | Scored |
| ISP-B5 | Support configurable metadata and document linkage for legal, operational and reporting artefacts. | Scored |
| ISP-B6 | Enable scenario-based simulation or test workflows before production release of new products. | Desirable |
...
| ID | Requirement description | Priority |
| --- | --- | --- |
| ISP-T1 | Expose documented APIs and event interfaces suitable for enterprise integration and automation. | Mandatory |
| ISP-T2 | Support deployment patterns compatible with bank-controlled hosting and segregated environments. | Mandatory |
| ISP-T3 | Provide comprehensive audit logs, configuration history and environment-specific controls. | Mandatory |
| ISP-T4 | Support integration with IAM, key management, monitoring and enterprise workflow tooling. | Scored |
| ISP-T5 | Provide non-functional evidence for scalability, resilience and performance under peak event loads. | Scored |
| ISP-T6 | Offer migration tooling for reference data, positions, static data or configuration baselines. | Desirable |

Expected integration points include treasury platforms, debt capital markets workflow tools, sanctions and screening services, legal documentation repositories, core accounting, cash ledgers, payments rails, SWIFT connectivity, IAM, SOC monitoring, enterprise service bus and analytics environments used for finance and risk reporting. Where third-party managed components are required, respondents must specify which party operates them, how incidents are handled, and what evidence can be supplied to satisfy internal security review and outsourcing governance.
...
| ID | Requirement description | Priority |
| --- | --- | --- |
| ISP-S1 | Support role-based access control, least privilege and segregation of duties across administration and operations. | Mandatory |
| ISP-S2 | Provide immutable or tamper-evident audit evidence for critical actions and lifecycle events. | Mandatory |
| ISP-S3 | Support data residency, retention and deletion controls aligned to GDPR and local obligations. | Mandatory |
| ISP-S4 | Support security monitoring, incident investigation and evidence export to bank SOC tooling. | Scored |
| ISP-S5 | Demonstrate operational resilience and third-party risk controls consistent with DORA or equivalent expectations. | Scored |
| ISP-S6 | Provide independent assurance artefacts and vulnerability-management processes. | Desirable |

Responses shall consider ECB outsourcing and resilience expectations, Bank of Italy and CONSOB interfaces where relevant, MiFID II, AMLD requirements, GDPR, DORA, eIDAS, EBA guidance on ICT and outsourcing risk, and any implications of MiCA perimeter boundaries where respondents believe them relevant to product architecture. Respondents should disclose any known control gaps, roadmap dependencies or jurisdictional limitations. Evidence such as certifications, assurance reports, penetration testing summaries and secure development lifecycle documentation may be attached as appendices.
...
The Bank requires Italian and English documentation, controlled change windows, strong segregation of duties, evidence for internal model validation and audit teams, and a support structure capable of handling time-sensitive events such as coupon dates, redemptions and contingency settlement operations. Respondents shall indicate the level of support available during critical events such as issuance dates, payment windows, maturity events, period-end reporting or contingency recovery exercises. Documentation must be suitable for business users, support teams, architects, security reviewers and auditors.
...
Intesa Sanpaolo expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Intesa Sanpaolo has publicly supported digital asset activity through its digital-assets desk and high-profile participation in Italy’s digitally native bond activity, including tokenized bond issuance on public blockchain infrastructure for institutional use. That means a realistic buyer document should connect procurement requirements to issuance governance, custody, settlement and evidence production rather than merely crypto trading functionality. In practical terms, that means the proposed platform must support institutional-grade digital-asset issuance, servicing, settlement, reporting and control workflows while fitting into capital markets issuance tooling, payments and treasury, custody interfaces, investor recordkeeping, finance and general ledger, AML screening, IAM and security monitoring. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. sponsorship is likely from IMI Corporate & Investment Banking, digital-assets program leaders, operations, legal and compliance. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, the bank will favor vendors that can bridge Italian market practice with broader EU regulatory convergence under MiCA and DORA. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by Bank of Italy, CONSOB, ECB supervision, MiCA, DORA and Italian outsourcing / privacy requirements. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes capital markets issuance tooling, payments and treasury, custody interfaces, investor recordkeeping, finance and general ledger, AML screening, IAM and security monitoring. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
