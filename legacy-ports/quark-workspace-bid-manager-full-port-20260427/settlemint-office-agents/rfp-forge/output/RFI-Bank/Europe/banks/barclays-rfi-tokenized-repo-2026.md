...
document-type: rfi
asset-class: tokenized repo
date: 2026-03-14
version: v1.0
reference: BARC-2026-002
---

# Request for Information: Tokenized Repo Platform Market Scan

## Section 1: Context, Scope, and Timeline

### 1.1 Issuing Organization
Barclays is conducting a procurement for a platform supporting digital securities issuance and post-issuance servicing within institutional capital markets workflows. The Bank is separately collecting exploratory information on tokenized repo capabilities to inform future collateral and funding innovation. Solutions must fit a UK wholesale banking environment, provide strong maker-checker and operational control features, and demonstrate practical interoperability with established securities operations and collateral systems. Barclays is interested in platforms that can improve issuance efficiency, asset servicing transparency and data integrity without creating fragmented operating models or control blind spots.
...
From a market standpoint, drivers include shorter settlement expectations, demand for improved transparency over ownership and lifecycle events, pressure to reduce manual reconciliations, and the need to support future delivery-versus-payment or programmable servicing patterns. The selected platform, if any, must prove that these benefits can be realized without compromising legal certainty, investor protection, information security or operational resilience. Priority is institutional workflow efficiency, straight-through processing, collateral awareness and clear operational ownership across front office, operations, treasury and technology.

### 1.2 Scope and Key Scenarios
This RFI concerns tokenized repo as the primary asset class. The Bank is seeking structured market information and does not intend this document to create any binding procurement obligation, shortlist commitment or volume commitment. The in-scope scenarios include Exploratory market scan for repo lifecycle digitization, collateral substitution, margin process integration, eligibility rules and programmable settlement conditions.
...
|---|---|---|
| BARC-BR1 | Support configurable asset lifecycle workflows with maker-checker approval and complete audit trail | Mandatory |
| BARC-BR2 | Provide participant, holding and entitlement views suitable for operations, compliance and reporting users | Mandatory |
| BARC-BR3 | Handle scheduled and exception-based events, including corrections and reversals | Mandatory |
| BARC-BR4 | Allow business users to configure core rules, templates and approval paths without code changes | Scored |
| BARC-BR5 | Support multilingual document and notification generation where relevant to operating entities | Desirable |
...
| ID | Requirement | Priority |
|---|---|---|
| BARC-TR1 | Expose versioned APIs and event interfaces for workflow, holdings, transactions and reporting data | Mandatory |
| BARC-TR2 | Support segregated environments for development, testing, pre-production and production | Mandatory |
| BARC-TR3 | Provide high availability and documented disaster recovery objectives | Mandatory |
| BARC-TR4 | Offer deployment options compatible with the Bank's cloud and network security policies | Scored |
| BARC-TR5 | Provide infrastructure-as-code or equivalent automated deployment support | Desirable |

The current system landscape includes Collateral management, capital markets booking, SWIFT, settlement operations, corporate actions tooling, ledger and finance systems, IAM and observability stack. Bidders must state whether integration accelerators exist for these or equivalent categories, and how they would manage message translation, canonical data modelling and operational monitoring across the interface estate.

### 2.3 Security/Compliance Requirements
Security and compliance requirements are central to evaluation. The solution must support operation in a regulated banking environment subject to FCA, PRA, UK CSDR-related controls where applicable, UK GDPR, outsourcing and operational resilience expectations. Respondents should describe how the platform supports segregation of duties, privileged access management, encryption in transit and at rest, immutable or tamper-evident logs, incident response, vulnerability management and evidence generation for internal audit and regulators. The Bank expects alignment with modern control standards, including defensible software development and third-party risk practices.
...
| ID | Requirement | Priority |
|---|---|---|
| BARC-SC1 | Support role-based access control, MFA integration and privileged activity logging | Mandatory |
| BARC-SC2 | Provide immutable or tamper-evident audit logs covering configuration and operational events | Mandatory |
| BARC-SC3 | Document data residency, retention and deletion controls aligned with applicable privacy laws | Mandatory |
| BARC-SC4 | Provide security certifications, penetration test summaries and secure SDLC evidence | Scored |
| BARC-SC5 | Support policy-based export of audit evidence for regulator and internal audit requests | Scored |
...
Barclays Bank PLC expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. Barclays has explored digital assets through institutional market infrastructure partnerships, digital collateral and repo workflow modernization, and broader tokenized securities discussions within its investment banking and markets franchise. Public evidence is less about retail crypto and more about institutional workflows, post-trade efficiency, collateral mobility and digitally native capital markets issuance. In practical terms, that means the proposed platform must support repo lifecycle automation, collateral mobility, eligibility schedules, substitution, margin process integration, intraday funding and settlement orchestration while fitting into existing investment banking booking systems, collateral management platforms, treasury and liquidity tooling, SWIFT messaging, securities operations platforms, finance ledgers, identity and access management, and enterprise observability controls. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. senior sponsorship is typically expected from Barclays Investment Bank digital, markets technology, treasury, operations, procurement, cyber and compliance leadership rather than a standalone crypto business unit. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, Barclays procurement practice typically emphasizes third-party risk, operational resilience, cyber assurance, segregation of duties and clear service-accountability boundaries. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by FCA, PRA, Bank of England and UK operational resilience / outsourcing framework. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes existing investment banking booking systems, collateral management platforms, treasury and liquidity tooling, SWIFT messaging, securities operations platforms, finance ledgers, identity and access management, and enterprise observability controls. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
...
Because this is an RFI rather than a committed award process, Barclays Bank PLC is using this document to separate productized platforms from ambitious slide decks. Respondents should therefore describe the maturity of their solution for repo lifecycle automation, collateral mobility, eligibility schedules, substitution, margin process integration, intraday funding and settlement orchestration in very concrete terms: what is live today, what is in pilot, what is customer-specific, and what remains roadmap. The buyer is especially interested in evidence that a platform can survive production governance, exception volume, regulator scrutiny and multi-team operating ownership.
