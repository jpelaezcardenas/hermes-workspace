---
institution: ING Group
country: Netherlands
document-type: rfi
asset-class: digital bonds
date: 2026-03-14
version: v1.0
reference: ING-2026-002
---

# Request for Information: Digital Bonds Capability Exploration

## Section 1: Context, Scope, and Timeline

### 1.1 Issuing Organization
ING is seeking a platform to support tokenized trade finance workflows with a focus on documentation control, transferability, financing events and auditable process coordination across bank, corporate and ecosystem participants. The Bank is also conducting an exploratory review of digital bond capabilities for selected wholesale use cases. The target environment is Dutch and pan-European, with strong emphasis on compliance, client process fit and interoperability across trade systems, risk controls and reporting functions. ING will prefer vendors that understand enterprise deployment in regulated banking environments rather than narrow pilots disconnected from operations.
...
|---|---|---|
| ING-BR1 | Support configurable asset lifecycle workflows with maker-checker approval and complete audit trail | Mandatory |
| ING-BR2 | Provide participant, holding and entitlement views suitable for operations, compliance and reporting users | Mandatory |
| ING-BR3 | Handle scheduled and exception-based events, including corrections and reversals | Mandatory |
| ING-BR4 | Allow business users to configure core rules, templates and approval paths without code changes | Scored |
| ING-BR5 | Support multilingual document and notification generation where relevant to operating entities | Desirable |
...
| ID | Requirement | Priority |
|---|---|---|
| ING-TR1 | Expose versioned APIs and event interfaces for workflow, holdings, transactions and reporting data | Mandatory |
| ING-TR2 | Support segregated environments for development, testing, pre-production and production | Mandatory |
| ING-TR3 | Provide high availability and documented disaster recovery objectives | Mandatory |
| ING-TR4 | Offer deployment options compatible with the Bank's cloud and network security policies | Scored |
| ING-TR5 | Provide infrastructure-as-code or equivalent automated deployment support | Desirable |

The current system landscape includes Trade finance platforms, document management, sanctions and AML controls, ERP/client channels, treasury, finance, reference data and enterprise integration services. Bidders must state whether integration accelerators exist for these or equivalent categories, and how they would manage message translation, canonical data modelling and operational monitoring across the interface estate.

### 2.3 Security/Compliance Requirements
Security and compliance requirements are central to evaluation. The solution must support operation in a regulated banking environment subject to DNB, AFM where relevant, ECB SSM oversight, GDPR, EU trade compliance and outsourcing expectations. Respondents should describe how the platform supports segregation of duties, privileged access management, encryption in transit and at rest, immutable or tamper-evident logs, incident response, vulnerability management and evidence generation for internal audit and regulators. The Bank expects alignment with modern control standards, including defensible software development and third-party risk practices.
...
| ID | Requirement | Priority |
|---|---|---|
| ING-SC1 | Support role-based access control, MFA integration and privileged activity logging | Mandatory |
| ING-SC2 | Provide immutable or tamper-evident audit logs covering configuration and operational events | Mandatory |
| ING-SC3 | Document data residency, retention and deletion controls aligned with applicable privacy laws | Mandatory |
| ING-SC4 | Provide security certifications, penetration test summaries and secure SDLC evidence | Scored |
| ING-SC5 | Support policy-based export of audit evidence for regulator and internal audit requests | Scored |
...
ING Group expects bidders to anchor their response in the buyer's actual strategic posture rather than reuse a generic tokenization template. ING has a long track record in blockchain-enabled trade finance and capital markets pilots, including roles in Komgo, HQLAx-type collateral mobility discussions and digitally native bond issuance initiatives. ING tends to focus on enterprise workflow efficiency, trade documentation, settlement optimization and network interoperability rather than consumer crypto speculation. In practical terms, that means the proposed platform must support bond issuance, bookbuilding support, investor onboarding, settlement, coupon processing, redemption and issuer reporting while fitting into trade finance systems, corporate banking channels, capital markets infrastructure, treasury systems, client onboarding, compliance and sanctions controls, event-driven integration and data platforms. The bank or fintech will not score a response well if the digital-asset layer becomes a sidecar disconnected from existing books and records, operations controls, treasury workflows, or regulator-facing reporting.

The issuing team also expects bidders to recognize the buyer's governance model. relevant leadership would come from wholesale banking innovation, trade and treasury services, capital markets operations, risk and architecture. This has two consequences. First, proposals must explain how the platform supports board-level oversight, architecture review, risk sign-off and third-party control testing before production launch. Second, vendors must show how implementation can proceed without creating a long tail of manual exceptions that move risk into operations rather than remove it.

From a procurement standpoint, ING will likely care about interoperability across consortium-style infrastructure and pragmatic coexistence with established banking controls. Proposals should therefore describe not just target-state functionality but also the migration path from current-state processes, including coexistence with legacy systems, phased onboarding of products or jurisdictions, and rollback or containment measures if external dependencies fail.

### Regulatory and Jurisdictional Context
The regulatory perimeter for this procurement is shaped by DNB, AFM, ECB supervision, MiCA, DORA and GDPR. Where the proposed operating model involves crypto-asset issuance, trading, custody, settlement or payment functionality in the EEA, respondents must explain how MiCA obligations are allocated across the buyer, the platform provider and any custody, exchange, wallet, payment or reserve-management partners. The buyer does not want hand-waving about regulatory coverage; it wants a control map that identifies which legal and operational obligations remain with the institution and which are facilitated by the proposed platform.
...
The solution must interoperate cleanly with the buyer's established control estate. That includes trade finance systems, corporate banking channels, capital markets infrastructure, treasury systems, client onboarding, compliance and sanctions controls, event-driven integration and data platforms. Vendors should describe canonical data models, API contracts, event-taxonomy design, idempotency controls, batch and streaming interfaces, and how reconciliation breaks are surfaced and resolved. If the proposed platform maintains its own ledger, respondents must explain how golden-source boundaries are defined and how downstream finance, risk and regulatory systems consume authoritative records.
