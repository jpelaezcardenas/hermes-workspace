...
| Field | Value |
| --- | --- |
| Reference | ATTIJARIWAFA-BANK-RFI-TOKENIZED-TRADE-FINANCE-MOROCCO-202603 |
| Version | v1.0 |
...
This RFI is intended to establish market maturity for tokenized trade finance and supply-chain
document digitization, compare delivery models, and understand which suppliers can support a
...
tokenized trade finance and supply-chain document digitization, including onboarding, policy setup,
...
| DQ-09 | Support document-linked trade workflows, milestones, and counterparty controls. | Status label + explanation + evidence reference |
| DQ-10 | Reconcile tokenized records to core trade and treasury books. | Status label + explanation + evidence reference |
| DQ-11 | Support multi-entity trade limit allocation and exception handling. | Status label + explanation + evidence reference |
...
platform supports multiple deployment patterns, state which one is most appropriate for Attijariwafa
Bank and why.
...
Address the practical regulatory context for Morocco, including Bank Al-Maghrib expectations, AMMC
requirements, data and cyber obligations, AML/CFT controls, outsourcing governance, trade
digitisation considerations. If the target use case would require specific licences, partner
...
Describe a realistic delivery model for a programme comparable to tokenized trade finance and
supply-chain document digitization, covering discovery, design, configuration, integration, testing,
...
RFI responses will be reviewed for clarity, completeness, relevance to tokenized trade finance and
supply-chain document digitization, evidence quality, dependency transparency, and credibility of
...
Use this appendix to make the operating reality of tokenized trade finance and supply-chain document
digitization clear to Attijariwafa Bank. The buyer is particularly interested in evidence quality,
...
Respondents should interpret market maturity carefully. For Attijariwafa Bank, the question is not
simply whether Tokenized Trade Finance And Supply-Chain Document Digitization can be demonstrated,
...
In reviewing responses, Attijariwafa Bank will focus on how data and control travel together.
...
layer, or specialist subsystem for Tokenized Trade Finance And Supply-Chain Document Digitization,
and what that means for reconciliation and support.

### 8.7 Implementation realism interpretation

Implementation timelines should reflect actual delivery conditions in Morocco. If readiness depends
on partner onboarding, legal structuring, policy approval, cloud approval, penetration testing, non-
...
scenarios. For Attijariwafa Bank, this includes not only implementation and subscription levels, but
...
For market-discovery purposes, this means respondents should not answer as though they were pitching a generic digital-asset stack. They should explain what must already be true inside Attijariwafa Bank (Morocco) for the use case to be governable: which enterprise systems need to remain authoritative, which policies must be finalized up front, which external partners would be mandatory, and which controls would need to be proven before any live processing. Responses that blur those prerequisites will be read as immature even if the underlying product is technically capable.

The buyer is also likely to probe whether comparable deployments exist in adjacent markets, what delivery effort was required to make them work, and where the repeatable product boundary ends. In other words, Attijariwafa Bank (Morocco) is using this RFI to separate scalable institutional platforms from bespoke project assemblages. Respondents should be candid about the difference.

### 8.19 Institution-specific evaluation scenarios and rollout signals

A useful RFI response for Attijariwafa Bank (Morocco) should describe how the platform would behave in three evaluation scenarios: a straightforward happy-path transaction or issuance; an exception path involving incomplete data, stale approvals, or unresolved compliance outcomes; and a dependency failure involving external settlement, custody, identity, or messaging services. For each scenario, the buyer wants to understand visible system state, operational alerts, user actions, approval controls, and exported evidence. That is the level at which institutional readiness becomes clear.

Respondents should also indicate what a sensible phase-one deployment would look like, what metrics would determine whether phase two is justified, and which dependencies most often extend timelines in real institutions. The strongest RFI submissions will help Attijariwafa Bank (Morocco) calibrate not only feature fit but also the hidden operating effort implied by the chosen model.
