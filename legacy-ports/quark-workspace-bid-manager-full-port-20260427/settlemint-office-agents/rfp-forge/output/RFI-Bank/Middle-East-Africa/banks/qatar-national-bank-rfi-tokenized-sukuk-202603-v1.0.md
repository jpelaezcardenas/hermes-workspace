...
| Field | Value |
| --- | --- |
| Reference | QATAR-NATIONAL-BANK-RFI-TOKENIZED-SUKUK-202603 |
| Version | v1.0 |
...
This RFI is intended to establish market maturity for tokenized sukuk issuance and servicing
...
tokenized sukuk issuance and servicing platform, including onboarding, policy setup, role
...
| DQ-09 | Support Sharia-compliant structuring, periodic distributions, and asset-backing records. | Status label + explanation + evidence reference |
| DQ-10 | Support AAOIFI-aligned governance evidence and board approvals. | Status label + explanation + evidence reference |
| DQ-11 | Support investor registry, transfer restrictions, and servicing controls. | Status label + explanation + evidence reference |
...
platform supports multiple deployment patterns, state which one is most appropriate for Qatar
...
Address the practical regulatory context for Qatar, including QCB expectations, QFCRA/QFMA perimeter
issues, AML/CFT controls, outsourcing and cloud controls, Islamic finance governance, securities and
custody obligations. If the target use case would require specific licences, partner structures, or
buyer legal interpretation, say so. The buyer values clear perimeter statements over broad claims of
...
Describe a realistic delivery model for a programme comparable to tokenized sukuk issuance and
servicing platform, covering discovery, design, configuration, integration, testing, training,
...
RFI responses will be reviewed for clarity, completeness, relevance to tokenized sukuk issuance and
servicing platform, evidence quality, dependency transparency, and credibility of the implied
...
Use this appendix to make the operating reality of tokenized sukuk issuance and servicing platform
clear to Qatar National Bank. The buyer is particularly interested in evidence quality, dependency
...
Respondents should interpret market maturity carefully. For Qatar National Bank, the question is not
simply whether Tokenized Sukuk Issuance And Servicing Platform can be demonstrated, but whether it
...
In reviewing responses, Qatar National Bank will focus on how data and control travel together.
...
layer, or specialist subsystem for Tokenized Sukuk Issuance And Servicing Platform, and what that
means for reconciliation and support.

### 8.7 Implementation realism interpretation

Implementation timelines should reflect actual delivery conditions in Qatar. If readiness depends on
...
scenarios. For Qatar National Bank, this includes not only implementation and subscription levels,
...
For market-discovery purposes, this means respondents should not answer as though they were pitching a generic digital-asset stack. They should explain what must already be true inside Qatar National Bank (Qatar) for the use case to be governable: which enterprise systems need to remain authoritative, which policies must be finalized up front, which external partners would be mandatory, and which controls would need to be proven before any live processing. Responses that blur those prerequisites will be read as immature even if the underlying product is technically capable.

The buyer is also likely to probe whether comparable deployments exist in adjacent markets, what delivery effort was required to make them work, and where the repeatable product boundary ends. In other words, Qatar National Bank (Qatar) is using this RFI to separate scalable institutional platforms from bespoke project assemblages. Respondents should be candid about the difference.

### 8.19 Institution-specific evaluation scenarios and rollout signals

A useful RFI response for Qatar National Bank (Qatar) should describe how the platform would behave in three evaluation scenarios: a straightforward happy-path transaction or issuance; an exception path involving incomplete data, stale approvals, or unresolved compliance outcomes; and a dependency failure involving external settlement, custody, identity, or messaging services. For each scenario, the buyer wants to understand visible system state, operational alerts, user actions, approval controls, and exported evidence. That is the level at which institutional readiness becomes clear.

Respondents should also indicate what a sensible phase-one deployment would look like, what metrics would determine whether phase two is justified, and which dependencies most often extend timelines in real institutions. The strongest RFI submissions will help Qatar National Bank (Qatar) calibrate not only feature fit but also the hidden operating effort implied by the chosen model.
