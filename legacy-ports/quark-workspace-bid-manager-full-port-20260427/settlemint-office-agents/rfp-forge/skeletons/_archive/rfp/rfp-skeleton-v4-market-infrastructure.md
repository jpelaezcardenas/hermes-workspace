# RFP Skeleton v4 — Digital Securities Market Infrastructure

**Document ID:** RB-RFP-04-v1.0
**Codename:** EXCHANGE
**Category:** Full RFP
**Buyer type:** Exchange, CSD, market operator, or regulated venue
**Primary intent:** Formal procurement for end-to-end digital securities issuance, trading support, settlement, and registry infrastructure
**Default response model:** Structured vendor response with explicit evidence requests
**Tailoring variables:** market model, participant types, issuance-trading-settlement scope, interoperability model, regulatory perimeter
...
### 4.1 Platform Architecture Requirements (18-24 requirements)
- Write about: Core architecture expectations for issuer onboarding, participant access, market connectivity, asset registry control, settlement finality, and extensible multi-party operating models
...
- Expand requirements around participant administration, market roles, interoperability with depositories or payment rails, and operational resiliency
- Require a target operating model diagram showing issuer, intermediary, venue/CSD, cash leg provider, and regulator interaction points
- Include an appendix for participant categories, market rules assumptions, and market event reporting expectations
