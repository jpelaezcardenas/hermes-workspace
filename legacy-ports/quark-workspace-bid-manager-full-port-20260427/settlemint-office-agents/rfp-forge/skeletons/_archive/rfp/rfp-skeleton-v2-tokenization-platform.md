# RFP Skeleton v2 — Enterprise Tokenization Platform

**Document ID:** RB-RFP-02-v1.0
**Codename:** TOKEN
**Category:** Full RFP
**Buyer type:** Commercial bank or universal bank
**Primary intent:** Formal procurement for an enterprise digital asset issuance and servicing platform
**Default response model:** Structured vendor response with explicit evidence requests
**Tailoring variables:** asset classes, target client segments, cloud/on-prem posture, integration estate, jurisdiction mix
...
- Write about: Core architecture expectations for multi-asset tokenization, tenant separation, configurable product models, API-first integration, policy-driven workflows, and bank-grade operating controls
...
- Require coverage across multiple asset classes such as deposits, funds, bonds, structured products, and alternative assets where relevant
- Add a section note that the buyer may require phased rollout by business line or geography
- Ask vendors to separate platform capability from implementation assumptions and bank-owned integration work
