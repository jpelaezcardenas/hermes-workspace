...
- **DAPI (Durable API Service)**: Backend API layer with a two-endpoint architecture, `/api/rpc` for session-authenticated dApp access and `/api/v2` REST for API-key-based programmatic access.
...
- **DIDX (DALP Indexer)**: Standalone blockchain event processing system owned and operated within the DALP deployment. DIDX automatically discovers contracts as they are deployed, decodes all on-chain events into queryable PostgreSQL read models with 18+ analytics views, and handles chain reorganizations with deterministic rollback. No external indexing service dependencies. See Q2.8 for full architectural detail.
...
- **RPC endpoint**: Session-authenticated endpoint for the dApp frontend with type-safe procedure calls.
...
- **Indexer read surface**: All on-chain state is projected into PostgreSQL-backed read models through DALP's owned indexer (DIDX), with pre-decoded data served directly to API consumers without runtime transformation.
...
**How the indexing pipeline works.** DIDX fetches event logs from blockchain nodes, decodes them using a pre-built topic-to-ABI map for constant-time lookups, and routes decoded events to domain-specific handlers covering tokens, identity, compliance, addons, feeds, and system contracts. The processing uses a converging discovery loop: when handlers register new contracts (for example, when a factory deploys a new token), DIDX re-fetches events for the same block range including the newly discovered addresses. The loop terminates when no new contracts are found, ensuring complete event coverage even when contract deployment and first interaction happen in the same block.

**What this means for your read layer.** All on-chain data is decoded once during indexing, not on every API read request. Compliance module parameters, claim data, addon type identifiers, and transaction metadata are decoded from their ABI-encoded form and stored as pre-computed JSONB columns in PostgreSQL. API queries hit pre-decoded, pre-aggregated read models, 18+ analytics views across five domains, without runtime transformation overhead. This decode-at-index-time architecture replaced a prior dependency on The Graph and is now the single source of truth for all API read operations across every domain: user, settings, compliance templates, feeds, KYC, system core, token-sale, token features, transfer, recovery, fixed-yield, and identity.

**How upgrades work without downtime.** When platform updates require reprocessing historical blocks, DIDX provisions a new deployment schema alongside the active one. The new schema rebuilds from block zero while the existing schema continues serving all read traffic. When the rebuild completes, an atomic view swap redirects all reads to the new schema. The previous schema enters a grace period before retirement. Applications experience no interruption, no stale data, and no maintenance windows.

**How data integrity is protected.** DIDX detects chain reorganizations by comparing stored block hashes against the canonical chain from the RPC node. If a reorganization is detected, PostgreSQL triggers that shadow-log every mutation enable deterministic rollback: every insert, update, and delete from orphaned blocks is reversed in exact reverse chronological order. The checkpoint resets to the fork point, and re-indexing proceeds from the correct chain. The read surface never serves data from orphaned blocks, a critical property for environments where audit trail integrity must be demonstrable to regulators.
...
A useful boundary for evaluators is this: DALP natively provides the orchestration, state management, and on-chain settlement controls for DvP/XvP workflows, while bank-rail execution remains integration-dependent. In practice, DALP can trigger, track, and reconcile settlement workflows around external payment events, but it does not itself become a SWIFT gateway, RTGS operator, or fiat clearing network. Where the cash leg stays on-chain, for example with approved stablecoins or tokenized cash instruments. DALP can keep both legs inside the platform's governed workflow. Where the cash leg runs through traditional rails, DALP coordinates with the institution's existing payment stack rather than replacing it.
