...
For proposal writers, the most effective entry point is usually not the module catalog, it is the regulatory template layer. Buyers rarely start by asking how claim expressions are encoded. They ask whether a MiCA, MAS, JFSA, FCA, or Reg D style control posture can be activated quickly and governed safely from day one. DALP's pre-seeded jurisdictional templates answer that question first, then the underlying modules explain how the policy is enforced. That order matters because it frames compliance as deployment-ready operating policy rather than as smart contract plumbing.
...
## Configurable Compliance: The Composable Control Plane

DALP's compliance architecture is not a single compliance engine, it is a composable control plane where the rules themselves are configurable, combinable, and updatable.

This distinction matters because most tokenization platforms offer compliance in one of two forms:

1. **Hardcoded compliance**: The platform supports a fixed set of compliance checks (usually a whitelist and maybe a country restriction). Adding new rules requires custom smart contract development.
2. **Off-chain compliance**: The platform performs compliance checks in application logic before submitting transactions. This means compliance can be bypassed by anyone with direct blockchain access, a fundamental architectural weakness for regulated securities.

DALP takes a fundamentally different approach: **12 concrete compliance module types**, each independently deployable, per-token configurable, and composable through sequential evaluation. Every module enforces its rules at the smart contract level, on-chain, ex-ante, and atomic. The compliance rules are not advisory. They are the rules.

### What "Configurable" Actually Means

Each compliance module is a pre-audited smart contract with **type-specific parameters** that institutions configure per token:

| Module | What's Configurable |
|--------|-------------------|
| **Identity Verification** | The logical expression over claim topics (e.g., `[KYC, AML, AND]` vs. `[ACCREDITED]` vs. `[KYC, AML, AND, QII, OR]`) |
| **Country Allow List / Block List** | The specific ISO 3166-1 country codes. EU-27, GCC, Singapore-only, or any custom set |
| **Investor Count** | Global limit and per-country sub-limits (e.g., max 500 globally, max 100 per country) |
| **TimeLock** | Hold period duration, FIFO batch tracking, and exemption expressions for qualified investors |
| **Transfer Approval** | Expiry window, one-time-use flag, and exemption expressions |
| **Token Supply Limit** | Lifetime, fixed-period, or rolling-period caps, with optional base-price conversion for currency-denominated limits (e.g., EUR 8M under MiCA) |
| **Capped** | Maximum circulating supply |
| **Collateral** | Collateral ratio (in basis points), collateral claim topic, and extra trusted issuers for reserve attestation |
| **Identity Allow/Block List** | Specific OnchainID identities to include or exclude |
| **Address Block List** | Specific wallet addresses to block (OFAC SDN lists, compromised wallets) |

### What "Composable" Actually Means

Modules compose through **sequential AND evaluation**: every active module must pass for a transfer to succeed. A single module veto blocks the transfer. This fail-closed design is the correct model for regulated securities.

But composability goes deeper than AND logic across modules. **Inside individual modules**, more complex logic is available:

- **Identity Verification** supports arbitrary boolean expressions over claim topics using RPN (Reverse Polish Notation): `AND`, `OR`, `NOT` combinations that can encode any eligibility rule without custom code
- **Investor Count** can filter which investors count toward limits using its own expression logic
- **Transfer Approval** and **TimeLock** support exemption expressions for qualified investor classes

This two-level composition. AND across modules, configurable logic within modules, gives institutions fine-grained control that would otherwise require custom smart contract development for each regulatory regime.

### What "Updatable" Actually Means

Compliance modules can be **added, removed, or reconfigured after the token is deployed**: through governed administrative operations requiring `GOVERNANCE_ROLE`. This is architecturally critical because:

- **Regulations change.** MiCA's issuance caps, country lists, and investor eligibility requirements will evolve. DALP's modules adapt through parameter changes, not contract redeployment.
- **Business requirements evolve.** A token that launches with a 90-day holding period may need to extend to 180 days. A token initially restricted to one jurisdiction may expand to three.
- **Compliance postures compound.** As an institution's digital asset program grows, new compliance requirements emerge. Adding a new module to an existing token is an administrative operation, not a development project.

The three-tier compliance interface hierarchy (Global Compliance → Token Compliance → SMART V2) ensures that this updatability works across protocol versions, tokens deployed under earlier DALP versions can coexist with newer tokens without forced migration.

### Why This Is Hard to Get Right

Building a composable compliance system for regulated financial instruments is not a UI design problem. It is a smart contract engineering challenge with real security implications:

- **Module ordering matters.** Compliance modules and token features execute in a defined order. Place cheap, restrictive checks first (identity, country) to minimize gas on rejected transfers. The platform enforces this through documented ordering recommendations and validation.
- **Parameter encoding must be type-safe.** Each module type has a specific discriminated-union parameter schema. Invalid configurations are rejected at deployment time, not at runtime. This prevents misconfiguration from reaching on-chain state.
- **Post-transfer hooks must be atomic.** After a transfer executes, compliance modules update their state (investor counts, supply tracking, approval consumption, timelock records). If any post-hook fails, the entire transfer reverts, there is never a state where a transfer completed but compliance accounting is out of sync.
- **Backward compatibility across versions.** The three-tier interface hierarchy preserves backward compatibility for deployed tokens while enabling new tokens to use updated compliance hooks. This is not a nice-to-have for regulated markets, it is essential.

Most teams discover this complexity only after they've committed to building their own compliance layer. DALP provides the composable infrastructure that has already solved these problems across production deployments.
...
DALP implements ERC-3643 through the **SMART Protocol** (SettleMint Asset Regulatory Technology). This is not a proprietary departure from the standard; it follows the ERC-3643 specification while adding features required for institutional deployment: upgradeable compliance modules, multi-jurisdictional regulatory templates, deterministic parameter encoding, and richer claim-expression logic than most vanilla ERC-3643 deployments support.
...
The module supports a configurable list of **approval authorities**: specific on-chain identities (OnchainID contracts) authorized to grant transfer approvals for a given token. This maps directly to regulated roles in securities markets: a licensed transfer agent designated by the issuer, specific compliance officers within the issuing organization, or regulated intermediaries authorized to supervise secondary market transfers. Only addresses whose on-chain identity matches a configured approval authority can grant approvals, any attempt by an unauthorized identity reverts immediately with `UnauthorizedApprover`.

Each approval creates a record with full provenance: the expiry timestamp, whether the approval has been consumed (for one-time-use configurations), and the specific identity of the authority that granted it. When a regulator examines a transfer during a supervisory review, the on-chain record provides a complete chain: which authority approved it, when the approval was granted, when it expires, and whether it has been consumed.

The module prevents conflicting approvals from different authorities for the same transfer, if an unexpired approval from one authority already exists, a second authority cannot overwrite it. This ensures a clean chain of responsibility: one authority, one approval, one consumption.

Approvals are keyed on sender identity, recipient identity, and amount. They can be configured as one-time use (consumed after a single transfer, recommended for regulatory compliance), expiry-based (valid only within a configured window), and exemption-aware (investors matching specific claim expressions can bypass the approval requirement when regulations permit).

**Why this matters**: Many regulated securities workflows still require transfer-agent or compliance-officer review. DALP does not pretend that every market is fully automated. It gives institutions a programmable way to preserve human approval in a deterministic system, with designated authorities and audit-grade provenance, while allowing exceptions for pre-qualified participants.

#### 7.8 TimeLock

Enforces a holding period using FIFO (First In, First Out) batch tracking. Every incoming transfer creates an acquisition batch with the amount received, the timestamp of receipt, and the unlock timestamp (received + configured hold period).
...
**Operational boundary**: The module enforces a maximum of **500 batches per user per token** to prevent unbounded array growth and escalating gas costs. For most institutional deployments, 500 batches is more than sufficient (a quarterly distribution recipient accumulates 4 batches per year). For scenarios involving many small, frequent allocations, institutions can mitigate by consolidating positions through custodian-mediated transfers or designing allocation workflows that minimize separate incoming transfers per holder. This limit is documented proactively because institutional deployments should not discover operational boundaries during production operations.
...
That alignment is the difference between a demo and a digital asset program that holds up under real regulatory scrutiny.
