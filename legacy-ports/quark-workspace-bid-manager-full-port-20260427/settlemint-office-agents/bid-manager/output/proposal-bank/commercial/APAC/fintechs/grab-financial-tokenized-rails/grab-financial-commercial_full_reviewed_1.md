...
| All Corridors (Configuration-Based) | No per-corridor license fee; additional corridors through configuration. Adding SGD, MYR, IDR, THB, PHP, VND corridors costs €0 in incremental license fees. |
...
| GrabPay merchant settlement batch failure | Merchant settlement batch not confirming within SLA window |
...
| Corridor expansion economics | New corridors via configuration, not re-implementation. Each new SEA corridor (MYR, IDR, THB, PHP, VND) adds zero incremental DALP license cost. | High. Grab Financial operates across 7+ SEA markets; zero incremental licensing per corridor is a material multi-year saving vs per-currency licensing models |
