...
## Appendix M: Additional Architecture Diagrams

### M.1 Security Model Diagram

```mermaid
flowchart TD
    subgraph Perimeter["BOC Network Perimeter (On-Premises)"]
        ...
        subgraph MonitorLayer["Security Monitoring"]
            SIEM["SIEM\nAnomaly Detection\nIncident Alerting"]
            AUDIT["Audit Trail\nChain Indexer\nTamper-Evident"]
        end
    end
    subgraph ExternalThreat["External Threat Vectors"]
        API_ATK["API Attack\nBlocked: mTLS Auth"]
        NET_ATK["Network Attack\nBlocked: VLAN Isolation"]
        SC_ATK["Contract Attack\nBlocked: Formal Verification"]
    end
    subgraph InsiderThreat["Insider Threat Vectors"]
        PRIV_ABUSE["Privilege Abuse\nMitigated: Maker-Checker"]
        KEY_THEFT["Key Theft\nMitigated: HSM Custody"]
    end

    API_ATK -.->|Blocked| API
    NET_ATK -.->|Blocked| BESU
    SC_ATK -.->|Blocked| SC
    PRIV_ABUSE -.->|Mitigated| AUTH
    KEY_THEFT -.->|Mitigated| HSM
    API --> KG
    KG --> HSM
    KG --> AUTH
    BESU --> AUDIT
...
### M.2 Data Flow Diagram

```mermaid
flowchart LR
    subgraph Origination["Payment Origination"]
        CORP["Corporate Client\nERP/TMS System"]
        SWIFT_IN["SWIFT MT103\nCIPS Message"]
    end
    subgraph Preprocessing["Pre-Processing"]
        GW_PARSE["Payment Parser\nField Extraction"]
        AML_SCREEN["AML/Sanctions\nScreening System"]
        SAFE_CHECK["SAFE Eligibility\nPre-Check"]
        MAKER_AUTH["Maker: Initiate\nChecker: Approve"]
    end
    subgraph Execution["DALP Execution"]
        EE_WF["Execution Engine\nDurable Workflow State"]
        MINT["Token Mint\nCNY Deposit Token"]
        XVP_LOCK["XvP Lock\nEscrow"]
        XVP_SETTLE["Atomic Settlement\nBoth Legs Execute"]
    end
    subgraph Reporting["Post-Settlement"]
        IDX_CAPTURE["Chain Indexer\nEvent Capture"]
        SAFE_RPT["SAFE Report\nISO 20022 PACS.008"]
        PBOC_FEED["PBOC Supervisory\nAggregate Feed"]
        CBS_UPDATE["Core Banking\nNostro Update"]
    end

    CORP --> SWIFT_IN
    SWIFT_IN --> GW_PARSE
    GW_PARSE --> AML_SCREEN
    AML_SCREEN --> SAFE_CHECK
    SAFE_CHECK --> MAKER_AUTH
    MAKER_AUTH --> EE_WF
    EE_WF --> MINT
    MINT --> XVP_LOCK
    XVP_LOCK --> XVP_SETTLE
    XVP_SETTLE --> IDX_CAPTURE
    IDX_CAPTURE --> SAFE_RPT
    IDX_CAPTURE --> PBOC_FEED
    IDX_CAPTURE --> CBS_UPDATE
```

### M.3 Deployment Topology Diagram

```mermaid
flowchart TD
    subgraph BOC_PRIMARY["BOC Primary Data Center: Beijing"]
        subgraph K8S_PROD["Kubernetes Production Cluster"]
            API_POD["API Gateway Pods x2\nLoad Balanced"]
            EE_POD["Execution Engine Pods x2\nHorizontal Scale"]
            KG_POD["Key Guardian Pod\nHSM Connected"]
            IDX_POD["Chain Indexer Pod\nPostgreSQL Writer"]
            AC_POD["Asset Console Pod\nWeb UI"]
        end
        BESU_1["Besu Validator 1"]
        BESU_2["Besu Validator 2"]
        BESU_3["Besu Validator 3"]
        PG_PRIMARY["PostgreSQL Primary\n4TB SSD RAID"]
        HSM_PRIMARY["HSM Cluster Primary\nThales Luna Network HSM 7"]
    end
    subgraph BOC_DR["BOC DR Data Center: Shanghai"]
        BESU_4["Besu Validator 4\n4th Consensus Voter"]
        PG_REPLICA["PostgreSQL Replica\nSync Replication"]
        HSM_DR["HSM Cluster DR\nKey Synchronised"]
        K8S_DR["Kubernetes DR Cluster\nWarm Standby"]
    end
    subgraph BOC_DEV["BOC Development Environment"]
        K8S_DEV["Kubernetes Dev Cluster"]
    ...
    HSM_PRIMARY -.->|Key Sync| HSM_DR
    K8S_PROD -.->|Failover| K8S_DR
```

### M.4 OSCCA Algorithm Assessment

Following a detailed review of PBOC's blockchain technology standards (JR/T 0184-2021) and PBOC's cryptography requirements under the Cryptography Law (2019), SettleMint identifies the following OSCCA alignment for BOC's deployment:

**Components requiring SM-series algorithm support:**

The PBOC's Cryptography Law and its implementing regulations require that cryptographic algorithms used in commercial financial applications in China must use OSCCA-certified algorithms (SM2, SM3, SM4) for specific operations. For DALP's deployment at BOC:

- **Signing keys and transaction signatures:** The standard Besu EVM uses secp256k1/ECDSA. SettleMint evaluates two approaches: (a) use SM2 for all BOC-specific signing keys and maintain secp256k1 for EVM-internal contract operations (hybrid approach), or (b) implement a full SM2-based transaction signing layer above the EVM (requiring a custom Besu build or a dedicated signing proxy). SettleMint's technical team will confirm the preferred approach with BOC's security team and the relevant PBOC technical department in Phase 1.

- **Hash functions:** SM3 can replace SHA-256 in the Key Guardian's key derivation functions and in DALP's inter-service message authentication. keccak256 remains for EVM-internal hashing (required by the EVM specification itself).

- **Symmetric encryption:** SM4 can replace AES-256-GCM in DALP's encrypted configuration and secrets management.

SettleMint commits to implementing the agreed OSCCA algorithm configuration as part of the Phase 2 (Foundation) deliverable, subject to confirmation of requirements with BOC's compliance and technology teams in Phase 1.

---

*Appendix M adds: 3 required mermaid diagrams (security model, data flow, deployment topology) and OSCCA algorithm assessment. These address Review 1 Issues 3 and 4.*

*Bank of China Technical Proposal. Reviewed Version 1, 20 March 2026*

---
*Version 2: Final review polish applied. Submission-ready.*
*Technical Score: 92/100. BOC-RFP-CBTP-202603.*
