...
Under tested conditions (500 concurrent settlement instructions on a 4-node IBFT validator network), DALP achieves median settlement latency of 2.3 seconds with P99 at 4.1 seconds, providing deterministic T+0 finality that eliminates the confirmation-depth waiting required by probabilistic consensus mechanisms.
...
Having established in the preceding section how DALP's four-layer architecture provides the structural foundation for Al Rajhi Bank's enterprise integration, we now turn to how the smart contract layer specifically addresses the Sharia compliance requirements that distinguish this programme from conventional tokenization.
