...
**Atomic settlement (DvP/PvP).** DALP's XvP (Exchange versus Payment) Settlement addon provides atomic delivery-versus-payment and payment-versus-payment settlement with escrow-based approval and all-or-nothing execution. When secondary transfers occur, the token and payment legs settle simultaneously, eliminating counterparty risk. All settlement events are indexed in real time and exportable via the REST API for compliance and accounting system feeds. This is standard functionality, not an add-on cost.
...
### Multi-Tenancy for Neptune's Platform Ambition

Neptune's longer-term vision extends beyond its own fleet to operating a tokenization platform for the broader shipping industry. DALP's multi-tenancy model directly supports this ambition with capabilities validated through the questionnaire assessment:

**Tenant onboarding and configuration.** DALP supports admin-assisted tenant onboarding via the organization management UI. Each tenant receives an isolated environment with configurable settings including blockchain selection, token standards, base currency, and visual theme. When Neptune onboards a new shipping company (for example, ALX Marine or a charter operator), the DALP admin team creates a new tenant organization with the company's legal entity name, incorporation jurisdiction, regulatory licence reference, and default token standards. The onboarding process is repeatable and takes minutes, not weeks.

**Per-tenant white-label branding.** Each tenant's investor-facing portal and communications reflect the tenant's own brand, not Neptune's. DALP supports per-organization theme configuration including custom logo, color scheme, and CSS variables. Custom domain configuration (for example, tokens.alxmarine.com) is supported with per-tenant SSL certificate provisioning. This is critical for Neptune's B2B platform model: tenants' investors should see their operator's brand, not Neptune Digital's.

**Strict data isolation.** DALP enforces data isolation between tenants via organizationId scoping on all database queries and through per-tenant on-chain system contract deployments. Tenant A's cap table, investor PII, token data, and documents are architecturally inaccessible to Tenant B's users or administrators. Cross-tenant data leakage is prevented at both the application and smart contract layers. This isolation model satisfies GDPR and data protection obligations.

**Tenant-scoped role administration.** Each tenant independently manages their own internal user roles (ISSUER, AGENT, COMPLIANCE OFFICER, PAYING AGENT) and assigns them to their staff. Neptune Digital's platform administrator can create, suspend, and deactivate tenants but cannot access tenant-level data or user accounts without explicit audit-logged authorization. The principle of least privilege is enforced at the platform level.

**Per-tenant compliance configuration.** Each tenant independently configures their own compliance ruleset: KYC/AML provider selection, required claim types, permitted investor jurisdictions, investor eligibility criteria, and sanctioned country blacklists. Different tenants may operate under different regulatory regimes (Jersey, BVI, Cayman, Malta). Compliance configuration is fully parameterisable per tenant with no cross-tenant interference. This allows Neptune to serve shipping companies operating under different jurisdictions without compliance rules from one tenant bleeding into another.

**Usage tracking and billing foundations.** DALP tracks per-organization API usage and on-chain asset counts. A configurable billing engine with automated invoice generation is not natively included, but the usage data required for billing (tokens issued, active investor count, distributions processed, API call volume, storage consumed) is available via the platform admin interface and API, enabling Neptune to implement its own fee schedules (monthly SaaS subscription, per-token fee, per-distribution fee, or success fee) through an integration layer.

**Platform admin dashboard.** DALP provides a platform-admin organization listing view with on-chain asset counts and member metrics per tenant. Neptune's operations team can see tenant status, total tokens issued per tenant, active investor counts, and recent activity across all tenants from a single consolidated view. A full revenue dashboard with billing metrics and tenant lifecycle management (suspend, offboard) requires additional operator-side tooling on top of the data DALP exposes.

### SPV Registry and Lifecycle Management

Neptune's programme centres on sixty-plus SPCs, each tied to a single vessel. Managing the SPV-to-token relationship and the full lifecycle of each SPV is a core requirement. Here is how DALP addresses this:

**Asset-to-token linkage.** DALP supports associating metadata (classification, location, and identifier claims) with each token issuance. For Neptune's vessel tokens, maritime-specific fields such as vessel name, IMO number, vessel type, flag state, class society, and year of build can be stored as custom token metadata or in the token document vault. The recommended approach is to store structured vessel data as claims on the token's OnchainID identity contract, making vessel details verifiable and queryable through the same identity infrastructure used for investor claims. This links each token to its underlying physical asset and legal wrapper in a machine-readable format.

**SPV document vault.** DALP's token document vault stores and versions issuance-level legal documents with visibility-controlled access for ISSUER, COMPLIANCE, and AUDITOR roles. For each SPC, this includes articles of association, certificate of incorporation, register of members, shareholder agreements, board resolutions authorising token issuance, offering memoranda, and legal opinions. All uploads are SHA-256 hashed for integrity verification and versioned with a complete audit log. This is separate from the investor-level document vault: it is the issuer-side legal record accessible only to authorized roles.

**SPV lifecycle stages.** DALP supports the core lifecycle events of a token issuance: active, paused, maturity redemption, and token burning on wind-down. For Neptune, this maps to the SPV lifecycle as follows:

| SPV Lifecycle Stage | DALP Implementation |
|---|---|
| Formation | SPC created in corporate registry (external); token deployed via createEquity() |
| Active (token live) | Token operational with compliance enforcement, distributions, and transfers |
| Disposal (vessel sold) | Final distribution to token holders via airdrop system; token paused |
| Wind-Down | Maturity redemption executed; remaining tokens burned; compliance frozen |
| Dissolved | Token fully burned; all records archived in document vault; SPC record retained for audit |

The maturity-redemption feature burns tokens and triggers treasury payouts to holders at maturity. When a vessel is sold, Neptune can execute a final distribution to all SPC token holders (proportional to holdings at the snapshot date), followed by token burning to retire the instrument. The full on-chain audit trail is preserved permanently.

**Beneficial ownership register (JFSC).** DALP's real-time cap table records all token holders with identity, holdings, percentage ownership, and acquisition data derived from on-chain state. For Jersey-incorporated SPCs subject to the Beneficial Ownership (Jersey) Law 2017, the cap table data (token holder name, address, nationality, number of tokens held, percentage ownership, date of acquisition) provides the raw data required for the statutory beneficial ownership register. Automated generation and export in JFSC-compliant statutory filing format requires operator-side tooling or a custom integration, but the underlying data is available in real time and can be exported via CSV or the REST API.

**Ownership structure traceability.** While DALP does not render a visual multi-layer SPV-to-asset structure diagram natively, the platform records the full chain of relationships: platform tenant (Neptune Digital), issuing entity context, individual SPC tokens, and the underlying vessel metadata. The API provides programmatic access to this data, enabling MindWave to build an ownership chain visualization in the investor portal showing the path from Neptune Holding Company through the Marshall Islands sub-holding to the individual SPC and its vessel.

### Corporate Actions Capabilities

The questionnaire assessment confirmed DALP's corporate actions coverage in detail. Here is the full picture for Neptune:

**Dividend and earnings distribution.** DALP's fixed-treasury-yield feature supports scheduled ERC-20 token distributions (including USDC or other stablecoins) from a treasury to all eligible token holders. For Neptune, this means SPC vessel earnings can be distributed to token holders on a configurable schedule (monthly, quarterly, or ad hoc). The distribution schedule, amounts, and record date are all configurable per token issuance. The airdrop system provides additional flexibility: push airdrop for immediate distribution, vesting airdrop for distribution over time, and time-bound airdrop for distribution within a defined window.

**Coupon payments for debt instruments.** If Neptune issues bond-style instruments alongside equity tokens (for example, vessel financing bonds), DALP's fixed-treasury-yield feature supports scheduled fixed-rate coupon payments. The yield schedule, payment frequency, and denomination currency are configured at token creation or updated by an authorized operator.

**On-chain voting.** DALP implements ERC-5805 voting power infrastructure with checkpointed token balances and on-chain delegation. Token holders can delegate their voting power to representatives, and historical balance checkpoints provide accurate voting weight at any point in time. A full governance proposal-and-execution system (proposal submission, quorum calculation, automatic execution) is not yet shipped as a native DALP feature, but the voting power foundation enables integration with governance frameworks such as Governor contracts. For Neptune's initial needs (board-level SPC decisions, holding company governance), the voting infrastructure combined with the transfer approval workflow provides a workable governance model.

**Record date and checkpoints.** DALP's ERC-5805 voting power feature provides on-chain checkpointed snapshots of all holder balances at any historical block. These immutable snapshots underpin entitlement calculations for dividends, voting, and rights issues. Neptune can query precise holder balances at any past timestamp for accurate pro-rata calculations.

**Redemption and buyback.** DALP's maturity-redemption feature burns tokens and triggers treasury payouts to holders at maturity. For vessel disposals, the flow is: execute final earnings distribution, then trigger maturity redemption to burn all outstanding tokens and return capital to holders. Issuer buybacks can be executed via forced transfer (acquiring tokens from holders) followed by burning, with a complete on-chain audit trail documenting every step.

**Capital calls.** DALP does not include a dedicated capital call workflow for fund or SPC structures. Capital calls can be approximated using the token-sale addon to issue additional tokens to a whitelist of existing investors at a configured price, effectively requesting additional capital in exchange for new token allocations. For Neptune's SPC structure, this provides a functional workaround if additional capital is needed for vessel maintenance or acquisition costs.

**Rights issues.** DALP's token-sale addon supports presale whitelisting of existing holders with a configurable discount and per-address cap. This enables a rights-issue-style offering where current SPC token holders receive priority allocation at a preferential price before any new investor allocation.

**Issuer notices.** DALP supports on-chain document storage and metadata anchoring via the token document vault for publishing announcements and material notices (AGM notices, financial results, material events). The document vault provides permanence and verifiability. A dedicated broadcast function with automatic investor notification delivery (email or push) requires configuration of the operator's notification service.

### Capability Gap Analysis

Transparency about what DALP does and does not do natively is central to this proposal. The following table consolidates all capability gaps identified through the questionnaire assessment, with recommended approaches for each:

| Capability | DALP Status | Gap Description | Recommended Approach |
|---|---|---|---|
| Order book / trade matching | Not provided | DALP is an issuance and lifecycle platform, not a trading venue | Integrate with a licensed ATS or MTF (21X, Archax) for secondary trading |
| Fiat on/off-ramp | Not provided | No native banking rails for fiat payment processing | Integrate with licensed payment processor or banking API via DALP's REST surface |
| Automated vessel valuation | Not provided | No built-in vessel appraisal or market data models | Use external maritime valuation provider; feed into DALP via oracle or manual process |
| SPV legal entity registry | Not native | DALP manages tokens, not corporate entities | Maintain SPV registry externally; link to DALP tokens via metadata and document vault |
| Stock split / consolidation | Not provided | No native proportional supply reorganization | Not currently supported; would require a migrate-and-reissue approach if ever needed |
| Tax withholding | Not provided | Distributions are paid gross; no jurisdiction-aware tax calculation | Integrate tax service provider; operator handles withholding outside DALP |
| PEP / adverse media screening | Not native | Requires third-party screening provider | Integrate Sumsub or Refinitiv; record screening results as on-chain identity claims |
| Full governance proposal system | Partial | Voting power infrastructure exists (ERC-5805); no native proposal/execution UI | Integrate with Governor contract framework or build custom governance UI |
| Automated regulatory report generation | Partial | Data available via API and CSV export; no JFSC-format auto-generation | Build operator-side reporting tooling against DALP's API for JFSC filing format |
| Outbound webhook delivery | Partial | SSE streaming available; full webhook push delivery not yet native | Use SSE for real-time monitoring; build webhook relay for push notifications if needed |
| Tenant billing engine | Partial | Usage data tracked per-org; no automated invoicing | Build billing layer on top of DALP's usage data API |
| Dedicated investor notification system | Partial | In-platform notifications for key events; no external email delivery | Configure operator email service; use DALP event webhooks as triggers |
| E-signature integration | Not native | Digital subscription workflow does not include embedded signing | Integrate DocuSign or Adobe Sign via MindWave's investor portal |

Each "Not provided" or "Partial" item has a clear integration path. None of these gaps are architectural blockers. They represent boundaries where DALP's platform scope ends and Neptune's integration scope begins.
...
**API key scoping and rate limiting.** DALP's API gateway provides API key authentication with HTTP-method-based scope enforcement. Keys are differentiated between read-only and read-write access, with configurable per-key rate limiting. This enables Neptune to issue scoped API keys to MindWave (read-write for portal operations), Nathek (read-only for position synchronisation), and reporting systems (read-only for data extraction) with appropriate access boundaries.

**Multi-chain support.** DALP operates on any EVM-compatible chain including Ethereum, Polygon, Avalanche C-Chain, Arbitrum, Optimism, Base, BNB Smart Chain, and Hyperledger Besu. Multi-chain deployment is supported with per-chain identity, compliance, and indexer isolation. For Neptune, this means the fleet could operate on a Layer 2 for cost efficiency while maintaining the option to deploy specific high-value instruments on Ethereum mainnet. Non-EVM chains are not supported.

**Stablecoin payment rails.** DALP integrates natively with ERC-20 stablecoins including USDC for on-chain distribution payments, subscription funding, and trade settlement. Payment currency is configurable per token or per primary offering. For Neptune's dividend distributions, USDC or a EUR-denominated stablecoin can be used as the distribution currency, providing instant on-chain payment confirmation without fiat settlement delays.

**Oracle and data feed integration.** DALP includes an exchange rate feeds system for ingesting external FX data. Integration with on-chain oracle networks such as Chainlink or API3 for NAV pricing and real-world asset valuation can be configured as an extra. For Neptune's Phase 3, when SPC vessel valuations need to feed into token pricing, this oracle integration path is available.
