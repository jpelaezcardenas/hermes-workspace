...
### Why This Programme Is Technically Hard

The difficulty of Neptune's programme does not lie in token issuance. Minting an ERC-20 token is a well-understood operation. The difficulty lies in three areas that most tokenization platforms treat as afterthoughts:

**Multi-entity compliance at scale.** Each SPC may operate under different jurisdictional rules depending on the vessel's flag state, the SPC's domicile, and the residency of its investors. With sixty-plus SPCs, this creates a combinatorial compliance challenge: each entity needs its own compliance configuration, its own investor eligibility rules, and its own transfer approval workflows. A platform that treats compliance as a single global ruleset cannot serve Neptune's needs.
...
SettleMint proposes its Digital Asset Lifecycle Platform (DALP) as the foundational infrastructure for Neptune's programme. DALP is a production-grade, standards-based digital asset stack built on ERC-3643 and OnchainID, with configurable token features, modular compliance enforcement, role-based access control, and API-first integration surfaces. It is designed specifically for regulated financial institutions, sovereign entities, and capital markets participants who need more than a token contract: they need an operating system for digital asset issuance, servicing, and governance.

The technical proposal centres on a phased rollout aligned to Neptune's Posidonia deadline:

**Phase 0 (Foundation, 6 weeks, March to April 2026)** deploys the DALP system instance, configures the identity and compliance framework, and issues 3 to 5 pilot SPC equity tokens representing a cross-section of vessel types and flag states. This phase proves the control model and creates the baseline configuration that all subsequent SPC deployments will inherit.

**Phase 1 (SPC Fleet Deployment, 6 weeks, April to May 2026)** extends the validated model to the remaining 55 to 57 SPCs using scripted deployment of individual `createEquity()` calls. Each SPC receives its own equity token with jurisdiction-specific compliance module configuration, its own identity registry, and its own access control hierarchy. MindWave delivers the investor portal MVP during this phase.
...
**Production credentials in regulated environments.** SettleMint holds ISO 27001 and SOC 2 Type II certifications and has maintained continuous production deployments at regulated banks for more than seven years. Deployments span bonds, equities, deposits, stablecoins, real estate, and funds across Europe, the Middle East, and Asia-Pacific. This is not a company running pilots. This is a company running production infrastructure for institutions where downtime and compliance failures carry material consequences.

**A platform built for multi-entity complexity.** DALP's architecture supports multiple asset issuances per entity, entity-specific compliance rules, and cross-entity reporting, all from a single platform deployment. The factory pattern (`createEquity()`) with CREATE2 deterministic addressing means Neptune can predict every SPC token's address before deployment. Each SPC gets its own proxy contract, access manager, identity registry, and compliance module set, with no shared state that would create cross-entity risk.
...
The company exists to solve the complexity of doing digital assets right. Tokenization technology is increasingly accessible, but the hard problems remain genuinely difficult: regulatory compliance across multiple jurisdictions, key management for institutional custody requirements, asset lifecycle operations from issuance through servicing to retirement, settlement logic that eliminates counterparty risk, and auditability that satisfies regulators and auditors. Most institutions underestimate this complexity until they are deep in implementation. SettleMint absorbs that infrastructure complexity so institutions can focus on their business.
...
This geographic breadth is relevant to Neptune because the programme involves entities domiciled in Jersey, the Marshall Islands, and potentially flag states across multiple continents. SettleMint's experience navigating multi-jurisdictional regulatory complexity is directly applicable.
...
Neptune Maritime Leasing operates a corporate structure that is more complex than most tokenization programmes encounter. The hierarchy consists of three distinct corporate layers:
...
The programme presents several technical challenges that directly inform platform selection:

| Challenge | Description | Implication for Platform |
|---|---|---|
...
| Jurisdictional compliance complexity | Jersey, Marshall Islands, plus flag states across multiple continents | Must support per-token compliance module configuration without code changes |
...
The implementation follows an accelerated four-phase approach aligned to Neptune's Posidonia deadline. The phases are designed to prove the model early, scale with confidence, and deliver a credible demonstration by early June 2026.
...
gantt
    ...
    Discovery and Requirements     :p0a, 2026-03-19, 1w
    Environment Provisioning       :p0b, after p0a, 1w
    Identity and Compliance Config :p0c, after p0b, 1w
    Pilot SPC Deployment (3-5)     :p0d, after p0c, 2w
    MindWave Onboarding            :p0e, 2026-03-19, 4w

    section Phase 1: Fleet Deployment
    Deployment Script Development  :p1a, after p0d, 1w
    Bulk SPC Deployment (55-57)    :p1b, after p1a, 2w
    Compliance Config per SPC      :p1c, after p1a, 3w
    Yield Distribution Setup       :p1d, after p1b, 1w
    Investor Onboarding (5)        :p1e, after p1b, 2w
...
### Phase 0: Foundation (6 weeks, March to April 2026)

Phase 0 establishes the foundational infrastructure, governance model, and first pilot SPC tokens. The objective is to prove that DALP can support Neptune's requirements before scaling to the full fleet.
...
- Define compliance requirements per jurisdiction (Jersey, Marshall Islands, flag states for pilot vessels)
...
- Define compliance module compositions for pilot SPCs
- Configure role hierarchy templates

**Weeks 4 to 5: Pilot SPC Deployment.**

- Deploy 3 to 5 pilot SPC equity tokens representing a cross-section of vessel types and flag states
- For each pilot SPC: deploy via `createEquity()`, configure compliance modules, set role assignments, register investor identities
- Execute end-to-end lifecycle tests: mint tokens to investor addresses, attempt compliant and non-compliant transfers, demonstrate compliance enforcement
- Validate dividend distribution mechanism using historical balance snapshots and airdrop system

**Week 6 (parallel): MindWave Onboarding.**
...
| 3 to 5 Pilot SPC Tokens | Live equity tokens with full lifecycle capability |
| SPC Asset Template | Validated template for standard SPC equity issuance |
| Role Hierarchy Template | Reusable permission structure for SPC governance |
| Phase 0 Completion Report | Results, lessons learned, template refinements |

#### Success Criteria

- Pilot SPC tokens are deployed and operational on the target network
...
- SPC asset template is validated and ready for fleet-scale deployment
- MindWave has begun development on the investor portal

### Phase 1: SPC Fleet Deployment (6 weeks, April to May 2026)

Phase 1 extends the validated Phase 0 model to the full SPC fleet, deploying the remaining 55 to 57 SPC equity tokens using the standardised template and scripted deployment.

#### Activities

**Week 1: Deployment Script Development.**

- Build deployment script that executes sequential `createEquity()` calls for remaining SPCs
...
**Weeks 2 to 3: Bulk SPC Deployment.**

- Execute deployment script on production network
- Deploy remaining 55 to 57 SPC equity tokens in controlled batches (10 to 15 per batch)
...
**Week 4: Yield Distribution Setup.**
...
| Deliverable | Description |
|---|---|
| Fleet Deployment Script | Parameterised script for SPC equity token deployment |
| 55 to 57 Additional SPC Tokens | Full fleet of 60+ SPC equity tokens operational |
...
Phase 2 is a short validation and preparation phase ensuring everything is ready for a credible Posidonia announcement.
...
This scope is credible and achievable within the timeline. A claim of "Neptune has tokenized its entire SPC fleet on DALP" is accurate even if only a subset of SPCs have active investor allocations, because the tokens are deployed and operational with compliance enforcement active.
...
The Posidonia deadline creates a 12-week window from programme start to announcement readiness. This timeline is tight but technically feasible if key dependencies are resolved promptly.

| ID | Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|---|
| TR-1 | Legal structuring delay | HIGH | CRITICAL | Papandreou's analysis of Marshall Islands SPC token structure and Jersey regulatory requirements is the critical path. If legal clarity is not achieved by end of March, the engineering timeline is compressed beyond what is safe. Mitigation: begin pilot deployment with provisional legal assumptions; adjust compliance configuration when legal structure is confirmed. |
...
**Challenge.** The engagement demanded institutional-grade custody integration (DFNS for transaction signing, with Fireblocks integration path), structured approval workflows, and cloud-agnostic infrastructure deployable in the UAE/GCC region using Terraform and Helm.
...
**Relevance to Neptune.** The Saudi RER engagement demonstrates SettleMint's capability at the largest possible scale: country-wide infrastructure serving an entire asset class. For Neptune, this provides evidence that DALP can handle the multi-entity scale (60+ SPCs, scaling to 200-300 vessels), the integration complexity (multiple external systems), and the governance requirements (sovereign-grade controls) that Neptune's programme demands. If SettleMint can deliver country-scale real estate tokenization infrastructure, it can deliver fleet-scale maritime tokenization infrastructure.
...
|---|---|---|---|---|---|
| R-01 | **Legal structuring delay.** Papandreou's analysis of Marshall Islands SPC token structure and Jersey JFSC notification requirements extends beyond March 2026. | HIGH | CRITICAL | Begin pilot deployment with provisional legal assumptions; design compliance modules for reconfiguration without redeployment. Engage legal counsel on parallel track with engineering. | Neptune / Papandreou |
