...
## 14a. Target Operating Model (BAU)
...
- Sharia Advisory Board attestation review: confirming on-chain Sharia attestations are current
- AAOIFI reporting: Islamic finance reporting data extraction from DALP events

**Bank Mandiri Technology owns:**
- Hyperledger Besu validator operations: node health, consensus monitoring, block production
...
### 14a.2 Daily Operations Procedures

**Start of Day (08:00 WIB):**
1. Confirm Hyperledger Besu validator network health: all validators producing blocks, IBFT consensus operating
...
5. Review OJK eligibility monitoring: investors with expiring QI claims requiring renewal
...
- OJK investor count limit monitoring: alert when approaching configured limits

**End of Day (16:00 WIB):**
- PPATK daily report: transaction activity summary for LTKT calculation
- OJK reporting data: digital securities activity export
- KSEI reconciliation: on-chain positions versus KSEI depository closing records
- Sukuk profit distribution: confirm any scheduled distributions completed

### 14a.3 Bank Indonesia Observer Node

The private network architecture includes an optional Bank Indonesia observer node, a significant governance feature that deserves operational clarity:

**Observer node capability:** Bank Indonesia's observer node receives a full copy of all block data from the private network. As an observer (not a validator), BI cannot influence consensus but has direct visibility into all transaction activity without requiring Bank Mandiri to generate separate regulatory reports.

**BI supervisory access:** Bank Indonesia's payment system oversight team can query the observer node directly for: real-time transaction volumes, settlement status for specific transactions, IDR deposit token reserve compliance, and BI-FAST coordination event logs. This transforms regulatory reporting from a batch reporting obligation into continuous supervisory visibility.

**Technical operation:** The observer node runs in Bank Indonesia's own infrastructure, maintained by BI's technical team with support from Bank Mandiri's technology team. SettleMint provides the genesis configuration and network connection parameters during Phase 2. Ongoing operation is BI's responsibility with Bank Mandiri's network support.

**Governance implications:** The observer node architecture is a demonstration of Bank Mandiri's commitment to regulatory transparency. Rather than treating BI oversight as a compliance obligation, the architecture embeds supervisory visibility into the infrastructure design. This positions Bank Mandiri favorably with Bank Indonesia for future digital asset approvals and programme expansions.

---

## 14b. Scenario Narratives
...
At 08:00 WIB, operations team confirms the yield schedule addon is queued for execution. The Fixed Treasury Yield feature has pre-calculated each investor's proportional coupon based on their Historical Balance snapshot at the record date. The IDR deposit token treasury is pre-funded with sufficient IDR for all coupon payments.

Bank Mandiri's operations manager activates the coupon distribution from the corporate actions console. DALP executes the distribution: each investor's wallet receives their IDR deposit token coupon allocation in sequence. The process runs without manual intervention for each of the 150 investors.
...
By 10:00 WIB, all 150 coupon payments are distributed and confirmed on-chain. The operations team reviews the distribution summary in the DALP dashboard and confirms reconciliation against the expected coupon schedule. Bank Mandiri's treasury team confirms IDR deposit token reserves are reduced by the total distribution amount.
...
The compliance officer calls DALP's freeze API for the investor's wallet address. Within 30 seconds, the wallet is frozen on-chain: the investor cannot transfer, receive, or redeem any digital asset tokens. The freeze event is recorded on-chain with the compliance officer's identity (their wallet address associated with the Custodian role), the timestamp, and the compliance reference number.

The investor's relationship manager is notified automatically through the alert system: "Client wallet [address] has been frozen by compliance as of [timestamp]. Reference: [PPATK case number]."

Bank Mandiri's financial crime team prepares the Suspicious Transaction Report (LTKM) for PPATK. The on-chain freeze event and the complete transaction history for the investor's wallet are exported through DALP's events API, providing the evidentiary basis for the LTKM submission.

If the investigation clears the investor, the compliance officer calls DALP's unfreeze API. The unfreeze event is recorded on-chain with the authorization reference. The investor's wallet immediately resumes normal operation.

If the investigation confirms the suspicious activity, Bank Mandiri can escalate to a forced transfer (Custodian role: forced transfer to a Bank Mandiri control account) or maintain the freeze while regulatory proceedings continue. All actions are permanently recorded on-chain.
...
The sukuk profit rate is configured as the underlying asset's return (the Musyarakah asset pool's quarterly performance), not as a fixed interest rate. BSI's Sharia Finance team calculates the profit rate based on the asset pool performance and updates the yield schedule configuration through DALP's governance interface. This configuration change requires the Governance role holder's wallet verification plus the Sharia Advisory Board's on-chain attestation that the profit rate reflects the actual asset pool return (not disguised interest).
