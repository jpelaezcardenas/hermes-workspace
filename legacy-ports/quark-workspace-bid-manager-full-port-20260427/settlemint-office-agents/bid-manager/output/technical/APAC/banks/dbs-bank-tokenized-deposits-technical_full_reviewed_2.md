...
DBS Bank is moving digital asset operations from controlled exploration to institutional deployment. The institution's direct involvement in DBS Digital Exchange, Singapore's publicly visible participation in Project Guardian, and the progressive maturation of trade finance digitalization initiatives under TradeTrust collectively create a set of demands that this programme must now satisfy, not in a proof-of-concept sandbox, but in a production environment that operates under MAS regulatory oversight, survives internal audit, and integrates into DBS Bank's existing enterprise infrastructure. Those expectations are not limited to technology; they extend to governance, auditability, operational sustainability, and the ability to withstand scrutiny from MAS, internal audit, risk committees, and senior management simultaneously.
...
First, proven production deployments in directly comparable environments. SettleMint has operated DALP in production with OCBC Bank in Singapore for a security token engine covering securitization, tokenization, and fractionalization of off-chain assets for HNWI investment products, the same regulatory environment, the same institutional control expectations, and the same MAS compliance framework that governs this procurement. The OCBC deployment demonstrates that DALP's compliance architecture satisfies Singapore's regulatory requirements in production, not just in sandbox conditions. For DBS Bank's evaluation committee, this is the most directly relevant production reference in the market: same regulator, same jurisdiction, same compliance framework, multi-year live operation. SettleMint is not proposing to build a Singapore-compliant digital asset platform for the first time; it is proposing to extend a proven deployment pattern to DBS Bank's programme. Additional APAC references include Standard Chartered Bank's Digital Virtual Exchange for fractional tokenization of securities across Asia, and Maybank's Project Photon for FX tokenization and cross-border XvP settlement aligned with Bank Negara Malaysia's Digital Asset Innovation Hub.
...
## 14a. Target Operating Model (Business-as-Usual Run State)

### 14a.1 Operational Ownership Model

After the hypercare period concludes, DALP operates under DBS Bank's day-to-day management with SettleMint providing platform support per the Enterprise Support SLA. The run-state model distributes responsibility clearly between DBS Bank operational teams and SettleMint.

**DBS Bank First-Line Operations Team owns:**
- Daily start-of-day and end-of-day reconciliation checks
- Approval queue management for pending transfers and issuance requests
- Exception triage and routing to second-line functions
- Monitoring dashboard review and alert response
- Investor onboarding coordination (claim issuance workflow)
- Routine reporting pack production from DALP event exports

**DBS Bank Second-Line Risk and Compliance owns:**
- Review of compliance exception reports
- AML/CFT case management triggered by DALP events
- Policy change approval for compliance module reconfiguration
- Regulatory reporting sign-off
- Periodic review of trusted issuer status

**DBS Bank Technology Team owns:**
- Platform infrastructure monitoring (in shared responsibility with SettleMint for managed cloud)
- Integration health monitoring across enterprise system connectors
- Release management coordination with SettleMint for platform updates
- Security patch review and approval

**SettleMint Support Team owns:**
- Platform-level incident response per Enterprise SLA
- Platform software updates and patch management
- Root cause analysis for P1/P2 incidents
- Quarterly architecture reviews and optimization recommendations

### 14a.2 Daily Operational Procedures

**Start-of-Day Checks (08:00-09:00 SGT):**
1. Review DALP operations dashboard for overnight events: failed transactions, expired settlements, reconciliation breaks
2. Confirm Chain Indexer health: block height, head age, indexer lag
3. Review approval queue: pending transfers, pending issuance requests, transfers approaching SLA
4. Confirm AML integration health: webhook delivery success rate from overnight batch
5. Review settlement queue: any XvP settlements approaching expiry requiring counterparty action
6. Confirm blockchain network health: node connectivity, gas pricing within normal range

**Intraday Monitoring:**
- Approval queue review every 2 hours during business hours
- Reconciliation mismatch alerts acted upon within 30 minutes of alert generation
- Failed transaction investigation: distinguish transient (network, gas) from permanent (compliance, policy) failures
- Sanctions screening hold review: immediate escalation to compliance team for new holds

**End-of-Day Checks (17:30-18:30 SGT):**
1. Reconciliation sign-off: confirm digital asset positions align with core banking sub-ledger
2. Settlement close: confirm all intraday XvP settlements reached final state (executed or cancelled)
3. Yield schedule review: confirm any scheduled distributions completed successfully
4. Exception log review: categorize and route any unresolved exceptions
5. Daily reporting pack: generate event export for management information and regulatory reporting systems

### 14a.3 Reconciliation Procedures

The reconciliation framework operates in three layers:

**Layer 1. Real-time event reconciliation:** DALP webhooks deliver events to DBS Bank's core banking system and sub-ledger in real time. The reconciliation engine in the core banking system matches DALP events to internal position records and flags mismatches immediately.

**Layer 2. End-of-day position reconciliation:** DBS Bank's operations team compares DALP's aggregated position (total supply, holder distribution, pending events) against the internal sub-ledger closing balance. DALP's API provides deterministic balance queries at any block height, enabling precise point-in-time reconciliation.

**Layer 3. Period-end reporting reconciliation:** For regulatory reporting periods, DALP's event export API provides a complete event log for the reporting period. The report is reproducible: the same query at the same block height range always produces the same output, supporting auditor verification.

**Break Investigation Procedure:**
If a reconciliation break is identified:
1. Operations team identifies the break category (timing, amount, instrument, counterparty mismatch)
2. DALP event API queried for the relevant transaction with full context
3. Break cause identified: network delay, failed webhook delivery, duplicate event, or genuine discrepancy
4. Resolution: replay webhook manually, adjust internal record, or escalate to SettleMint support for investigation
5. Break resolution documented with root cause, resolution, and preventive action
6. All break investigations retained for audit access

### 14a.4 Change Management Procedures

**Routine Configuration Changes (Compliance Module Updates, Role Assignments):**
- Change request submitted through DBS Bank's internal change management process
- SettleMint provides impact assessment within 2 business days
- Change validated in staging environment before production deployment
- Production deployment within approved change window (Saturday 02:00-06:00 SGT)
- Post-change validation by DBS Bank operations team

**Emergency Configuration Changes (Sanctions Hold, Regulatory Order):**
- Compliance team raises emergency change request with DBS Bank's change authority
- SettleMint support escalated to Enterprise tier response (15-minute response)
- Emergency change executed in production with simultaneous documentation
- Post-emergency review within 5 business days

**Platform Updates (SettleMint-Initiated):**
- SettleMint notifies DBS Bank of upcoming updates with minimum 10 business days advance notice for major updates
- DBS Bank reviews update in staging environment (provided by SettleMint's staged rollout process)
- DBS Bank approves production deployment (Enterprise tier: approval gate before production)
- Post-update validation by DBS Bank technology team

### 14a.5 Knowledge Continuity

The operational model is designed to survive personnel turnover without creating knowledge dependency on specific individuals:

**Documentation Set (maintained by SettleMint and DBS Bank jointly):**
- Platform administration guide (SettleMint-maintained, version-controlled)
- Integration runbooks (DBS Bank-maintained, reviewed quarterly)
- Compliance configuration register (DBS Bank-maintained, approved by Compliance team)
- Incident response playbooks (jointly maintained, tested quarterly via tabletop exercises)
- Role and permission register (DBS Bank-maintained, reviewed at each personnel change)

**Training Programme:**
- New operations team members: 2-day platform onboarding covering dashboard navigation, approval workflows, reconciliation procedures, and exception handling
- New compliance team members: 1-day compliance module training covering ex-ante enforcement, module configuration, and audit evidence generation
- Annual refresher: 1-day tabletop exercise covering control event scenarios (sanctions alert, settlement failure, key rotation, network outage)

**Support Continuity:** The SettleMint Enterprise Support team maintains familiarity with DBS Bank's deployment through bi-weekly operational reviews and quarterly architecture reviews. DBS Bank does not depend on a single named SettleMint contact; the entire named support team shares institutional knowledge of the deployment configuration and integration patterns.

---

## 14b. Scenario Responses

### Scenario 1: Normal Day at Standard Volume

A standard business day for DBS Bank's tokenized deposits and trade finance programme involves the following workflow:

DBS Bank's operations team begins with the start-of-day checklist at 08:00 SGT, confirming overnight event status and approval queue state. During the morning, a batch of tokenized deposit minting requests arrives from DBS Bank's treasury team for a new tranche issuance. The supply manager role holder executes the minting through the DALP console: each recipient's OnchainID is validated automatically, compliance checks confirm all recipients are within the MAS allowlisted jurisdictions with current KYC claims, and the mint transactions execute. The operations team confirms the minting events appear in the webhook feed to the core banking system and reconciles the new token supply against the treasury record.

During the afternoon, several secondary market transfers process between institutional counterparties. Each transfer triggers the compliance engine: identity check passes, country allowlist check passes, investor count remains within limits, and the XvP settlement for a deposit-versus-trade-finance instrument exchange executes atomically. The operations team monitors the settlement queue through the DALP console, confirming all settlements reach the executed state before end-of-day.

At 17:30, end-of-day reconciliation confirms digital asset positions match the core banking sub-ledger to the unit level. The daily reporting extract is generated from the DALP events API and delivered to DBS Bank's data warehouse for management information processing.

### Scenario 2: Peak Day at Elevated Volume

On a peak day following a market event, transfer volumes are three to five times the normal level. The following differences apply:

The approval queue for discretionary transfers accumulates faster. DBS Bank's operations team receives automatic alerts from the DALP monitoring dashboard when queue age exceeds configured thresholds. Additional approvers are activated using their pre-configured approver roles, processing the queue in parallel without any system changes or emergency configuration.

AML webhook delivery volume increases proportionally. DALP's async transaction pipeline maintains webhook delivery reliability at elevated volumes through its configurable retry logic and dead-letter rescue system. The AML platform's processing latency may increase, but DALP's transfer approval module ensures that transfers requiring screening hold remain in the approval queue until the screening response is received, regardless of volume.

Settlement processing volume increase is absorbed transparently: DALP's Kubernetes deployment scales automatically (managed cloud) or within the pre-provisioned capacity headroom (private cloud). The Chain Gateway's multi-RPC provider configuration ensures blockchain connectivity remains stable even under elevated transaction submission volumes.

End-of-day reconciliation may take longer due to increased event volume, but the deterministic nature of DALP's event log ensures reconciliation accuracy is not affected by volume.

### Scenario 3: Control Event: Sanctions Alert and Failed Settlement

**Scenario:** At 14:30 SGT, DBS Bank's sanctions screening platform identifies a potential sanctions match on an investor wallet that holds tokenized deposit tokens. Simultaneously, an XvP settlement for a trade finance instrument transfer fails due to the counterparty not confirming their leg before expiry.

**Sanctions Alert Response:**
1. The sanctions screening platform calls DALP's freeze API for the identified wallet address
2. DALP immediately freezes the wallet: the investor's unfrozen balance drops to zero, preventing any outbound transfers
3. The compliance dashboard surfaces the freeze event with the wallet address, frozen amount, and the timestamp
4. DBS Bank's AML/Compliance team reviews the screening match: if confirmed, the freeze is maintained and the matter is escalated per DBS Bank's AML procedures; if cleared, the compliance team calls DALP's unfreeze API with a documented clearance reference
5. All freeze and unfreeze events are recorded permanently on-chain with the operator identity, timestamp, and action reference, auditable evidence regardless of outcome
6. The entire workflow from alert to freeze executes in under 5 minutes. No engineering intervention required.

**Failed Settlement Response:**
1. The expired XvP settlement appears in the DALP settlement queue as "Expired"
2. DALP's HTLC mechanism returns both parties' assets automatically: neither party loses their asset due to the other's failure to confirm
3. The operations team is notified via the monitoring dashboard and webhook alert
4. DBS Bank's operations team contacts the counterparty to understand the reason for non-confirmation
5. If the counterparty is ready to re-settle, a new XvP settlement is created through the DALP console with a new hashlock and expiry date
6. All events, original settlement creation, expiry, asset return, new settlement, are recorded on-chain with complete context for post-event review

---

## 14c. Singapore Domestic Payment Rail Integration

### 14c.1 MAS SORA (SGD RTGS) Connectivity

Singapore's SORA (Scrips and Securities Operations Centre) system serves as the SGD high-value payment settlement infrastructure. For DBS Bank's tokenized deposits programme, the integration pattern between DALP and SORA supports the cash leg of deposit token operations:

**Cash leg for deposit issuance:** When DBS Bank issues tokenized deposits against SGD fiat funding, the SGD funding transfer through SORA generates a payment confirmation that triggers the token minting workflow. DALP's Transfer Approval module can be configured to require the SORA payment confirmation event (delivered via DBS Bank's treasury system webhook) before the token minting executes. This ensures that SGD funding and token issuance are operationally coordinated, even though they occur on different rails.

**Cash leg for deposit redemption:** At maturity redemption, the atomic DvP mechanism burns the deposit tokens and transfers the denomination asset (on-chain SGD stablecoin or deposit token) to the redeeming investor simultaneously. The off-chain SGD cash payment through SORA is orchestrated by DBS Bank's treasury system, triggered by the on-chain redemption event webhook. The reconciliation framework confirms that the on-chain and off-chain legs are both completed before the redemption is considered settled.

### 14c.2 PayNow and FAST Integration

For retail or SME investor segments that may require faster SGD settlement, DALP's webhook architecture supports integration with PayNow and FAST (Fast and Secure Transfers):

**Investor onboarding funding:** PayNow can serve as the funding channel for retail investors purchasing tokenized deposit products. The PayNow payment confirmation, delivered via DBS Bank's core banking system, triggers the investor onboarding workflow in DALP: the investor's OnchainID is activated and the initial token allocation is minted. This end-to-end flow (PayNow payment to token allocation) can complete within minutes.

**Distribution payments:** For yield distributions to individual investors, DALP's pull-based yield claim mechanism generates a claim event. DBS Bank's treasury system can consume this event and initiate PayNow or FAST payment to the investor's registered bank account, creating an automated distribution workflow from on-chain yield event to off-chain SGD credit.

### 14c.3 Data Residency for Payment Rail Integration

All payment rail integration data (payment references, transaction amounts, timestamps, counterparty identifiers) remains within Singapore infrastructure. The integration pattern uses DBS Bank's internal messaging bus as the intermediary between DALP and external payment rails, ensuring no payment data transits outside DBS Bank's network perimeter.
...
| Assumption ID | Category | Assumption | Owner | Invalidation Action |
|--------------|----------|------------|-------|---------------------|
| A-001 | Regulatory | Tokenized deposits are classified as deposits (not capital market instruments) under PSA for the instrument structures proposed | DBS Bank Legal | Re-scope compliance template if classification differs |
| A-002 | Technical | DBS Bank operates or has access to an EVM-compatible blockchain network (private Besu or Project Guardian network) | DBS Bank Technology | Evaluate alternative network options in Phase 1 |
| A-003 | Integration | DBS Bank's AML platform supports real-time webhook consumption with sub-5-second processing latency | DBS Bank Technology | Adjust transfer approval module configuration to accommodate AML processing latency |
| A-004 | Integration | DBS Bank's core banking system has an available API for position reconciliation and sub-ledger event consumption | DBS Bank Technology | Define alternative reconciliation approach (file-based batch) if API is not available |
| A-005 | Integration | DBS Bank's KYC/AML infrastructure can act as a Trusted Issuer for OnchainID claims with a programmatic claim issuance API | DBS Bank Operations | Alternative: SettleMint provides managed Trusted Issuer service during initial phase |
| A-006 | Security | DBS Bank's InfoSec team can accommodate cloud deployment within AWS ap-southeast-1 for this programme | DBS Bank InfoSec | Evaluate on-premises deployment if cloud is not approved |
| A-007 | Delivery | DBS Bank project team is available full-time for the specified roles during the 19-week implementation | DBS Bank Programme | Re-scope timeline if team availability is constrained |
| A-008 | Commercial | Contract is denominated in EUR with annual upfront payment; no requirement for SGD denomination or monthly billing | DBS Bank Finance | Arrange currency hedging and payment structure alignment at contract execution |
| A-009 | Technical | Private blockchain network for DBS Bank's deployment does not require MAS network registration or licensing | DBS Bank Legal | Engage MAS on network regulatory status if required |
| A-010 | Scope | Initial programme scope is Singapore-only (no multi-jurisdiction deployment in Year 1) | DBS Bank Programme | Scope multi-jurisdiction expansion as a Year 2 configuration exercise |

### Dependency Log

| Dependency ID | External Dependency | Owner | Required By | Risk if Delayed |
|--------------|---------------------|-------|-------------|-----------------|
| D-001 | Fireblocks enterprise agreement (if selected as custody provider) | DBS Bank Procurement | Phase 2 start | Delay in Key Guardian configuration; fallback to AWS KMS temporarily |
| D-002 | MAS guidance confirmation on tokenized deposit instrument classification | DBS Bank Legal | Phase 1 completion | Risk of compliance reconfiguration if guidance arrives after Phase 2 |
| D-003 | DBS Bank InfoSec approval for AWS ap-southeast-1 deployment | DBS Bank InfoSec | Phase 2 start | Deployment model change; timeline impact |
| D-004 | DBS Bank core banking system API specification and sandbox access | DBS Bank Technology | Phase 3 start | Integration development cannot commence without API documentation |
| D-005 | DBS Bank AML platform API or webhook endpoint for DALP event consumption | DBS Bank Technology | Phase 3 start | AML integration delayed; manual compliance workflow as interim |
| D-006 | Project Guardian network access credentials (if applicable) | DBS Bank / MAS | Phase 3 | Cross-network interoperability deferred to Phase 2 of programme |
| D-007 | DBS Bank security review completion and penetration test scheduling | DBS Bank InfoSec | Phase 4 start | Testing phase delay; overall go-live risk |
| D-008 | TradeTrust framework API access and documentation (if TradeTrust integration in scope) | TradeTrust programme | Phase 3 | Optional work package deferred if access not available in time |
