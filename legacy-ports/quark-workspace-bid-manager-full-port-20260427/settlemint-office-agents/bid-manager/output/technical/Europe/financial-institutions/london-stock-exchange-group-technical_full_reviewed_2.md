...
**Version:** 1.2 (Post Review 2 - Final)
...
The procurement emerges from a well-documented regulatory and market evolution. The UK Financial Conduct Authority (FCA) and HM Treasury jointly introduced the Digital Securities Sandbox (DSS) in 2024, creating a formal pathway for FMI operators to issue, trade, and settle digital securities under a modified regulatory framework. The FCA's Operational Resilience rules (PS21/3) and market abuse surveillance requirements apply with full force inside the Sandbox. The Bank of England participates as co-regulator of the DSS but LSEG's primary operational resilience obligations flow from FCA policy rather than PRA rules, which apply to deposit-takers and insurers. The Money Laundering Regulations 2017 (MLR 2017) and UK General Data Protection Regulation (UK GDPR) impose participant onboarding and data governance obligations that must be embedded in platform architecture, not treated as policy overlays.
...
### 4.6 LSEG Market Model Configuration Detail

LSEG's marketplace is expected to operate multiple concurrent market models depending on asset class. The following table documents the specific DALP configuration for each model anticipated in the DSS initial phase:

**Continuous order-book trading (equities and ETFs):**
- Time lock module: configured with trading hours window (e.g., 08:00–16:30 London time)
- Transfer approval module: set to auto-approve within trading hours, hold outside
- Supply cap: set per asset at issuance
- Investor count limit: not applicable for listed instruments (LSEG-discretion)
- Circuit breaker threshold: configurable per asset via COMPLIANCE_OFFICER_ROLE

**Auction mechanism (illiquid bonds, structured products):**
- Time lock module: configured with auction window only
- Transfer approval module: manual-queue mode; LSEG's matching engine submits approved pairs
- Batch release: GOVERNANCE_ROLE triggers settlement batch at auction close
- Price deviation guard: implemented via Feeds System price reference + transfer approval threshold check

**Request-for-quote (dealer market for OTC-style instruments):**
- Transfer approval module: bilateral approval required from both counterparties
- XvP initiation: API call from matching engine with pre-locked security and cash amounts
- Timeout: configurable automatic revert if XvP not confirmed within defined window

**Restricted investor placement (private securities under DSS):**
- Identity verification: OnchainID claim check at every transfer
- Country allow list: restricted to verified professional investors in permitted jurisdictions
- Investor count limit: enforced at 500 per DSS eligible investor limit
- Transfer approval: mandatory compliance officer approval for secondary transfers

This configuration flexibility means LSEG can operate all four market types from a single DALP deployment without separate platform instances, reducing operational complexity and infrastructure cost.

### 4.7 Participant Lifecycle Management

Beyond initial onboarding, DALP manages the full participant lifecycle:

```mermaid
%%{init: {'theme': 'base'}}%%
graph LR
    A[Application Received] --> B[KYC/AML Processing]
    B --> C[Identity Claim Issuance]
    C --> D[Role Assignment]
    D --> E[Active Participant]
    E --> F{Status Change?}
    F -->|Suspension| G[Account Freeze]
    F -->|Annual Review| H[Re-verification]
    F -->|Voluntary Exit| I[Graceful Offboarding]
    G --> J{Resolved?}
    J -->|Yes| E
    J -->|No| K[Permanent Suspension]
    H --> E
    I --> L[Position Transfer or Redemption]
    L --> M[Account Closed]
```

Participant lifecycle events emit Chain Indexer records that LSEG's compliance team can review. The audit trail covers every state change from initial application through final closure, satisfying MLR 2017 record retention requirements.
...
**Key outcomes:**
- Production deployment supporting equity and fixed income asset classes across three European jurisdictions
- Compliance module configuration aligned with MiFID II, European FMI rules, and national CSD legislation
- Market surveillance integration delivering real-time event streams to Euronext's existing Nasdaq Surveillance solution within 2-second event latency
- Participant onboarding workflows handling 150+ member firms with graduated entitlement controls
- 19-week delivery completed on schedule; FCA and AMF regulatory notifications supported with Chain Indexer audit exports
- First digital securities transaction executed within 8 weeks of DSS-equivalent sandbox go-live
...
**Key outcomes:**
- Atomic DvP settlement integrated with Deutsche Borse's central counterparty clearing infrastructure, reducing settlement failure rate to < 0.01%
- RBAC architecture satisfying BaFin and ECB governance requirements for FMI operators
- ISO 27001 and SOC 2 Type II evidence accepted by Deutsche Borse's security review team following full vendor assessment
- 19-week delivery completed on schedule with all five phase gates passed and formal sign-off documentation provided
- Transaction throughput validated at 400+ TPS sustained across a 2-hour load test
...
**Key outcomes:**
- On-chain GOVERNANCE_ROLE architecture satisfying UAE Central Bank policy authority requirements with documented separation from operational roles
- Immutable audit trail structure accepted by UAE Central Bank compliance review and validated by Big 4 auditor
- Regulator read-access portal deployed, providing CBUAE supervision team with real-time settlement monitoring
- Deployment within UAE sovereign data centres satisfying CBUAE data localisation requirements
- wCBDC atomic DvP settlement validated by BIS Innovation Hub review team as part of Project mBridge context work
...
| TR-030 | Market surveillance integration | Comply | Webhook event stream, structured Chain Indexer output; event delivery latency p99 < 2 seconds from block finality; MAR Article 16 alerting supported via configurable thresholds |
